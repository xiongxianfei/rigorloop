...
for (const level of ['off', 'unknown_value'])
  for (const extra of [
    ['--no-file-log'],
    ['--console-log-level', 'unknown_value'],
    ['--file-log-level', 'off'],
  ])
    test(`ER-M4-004 public recorder rejects historical logging flags ${extra[0]} with environment ${level}`, (t) => {
      const root = setup(t);
      const result = spawnSync(
        ...
        [
          ...
          'record-store',
          ...args(root, 'record', extra),
        ],
        {
          encoding: 'utf8',
          input: 'payload-must-not-appear',
          env: { ...process.env, RIGORLOOP_CONSOLE_LOG_LEVEL: level },
        },
      );
...
test('ER-M4-004 public recorder ignores historical logging environment', (t) => {
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        RIGORLOOP_CONSOLE_LOG_LEVEL: 'unknown_value',
        RIGORLOOP_FILE_LOG_LEVEL: 'unknown_value',
      },
    },
  );
...
function args(root, op, extra = []) {
  return [
    ...
    '--format',
    'json',
    ...(['record', 'check'].includes(op) ? ['--input', '-'] : []),
    ...extra,
  ];
}
function run(root, op, input, options = {}, extra = []) {
  const execution = executeRecordStoreCli(args(root, op, extra), {
    ...options,
...
function update(root, status = 'completed') {
  const current = run(root, 'inspect').result,
    r = request();
  r.expected_revision = current.revision;
  r.writes[0].expected_identity = current.files.find((f) => f.path === manifest).identity;
...
function recover(root, result, action, options = {}) {
  return run(root, 'recover', undefined, options, [
    '--transaction',
    result.transaction.id,
    '--expected-recovery',
    result.transaction.recovery_identity,
    '--action',
    action,
  ]);
}

for (const phase of ['after-preparation', 'after-commit'])
  test(`ER-M2-002 pending ${phase} journal survives lock acquisition`, (t) => {
    const root = setup(t);
    run(root, 'record', request());
    const pending = update(root),
      original = RecordFiles.prototype.write;
    ...
    RecordFiles.prototype.write = function (path, ...rest) {
      if (!injected && path.endsWith('/lock')) {
        injected = true;
        assert.equal(
          run(root, 'record', pending, { fault: (p) => (p === phase ? 'crash' : undefined) }).result
            .status,
          'recovery-required',
        );
    ...
    let result;
    try {
      result = run(root, 'record', pending).result;
    } finally {
      RecordFiles.prototype.write = original;
    }
...
for (const action of ['record', 'complete', 'restore'])
  test(`ER-M2-003 late third state survives ${action}`, (t) => {
    const root = setup(t);
    run(root, 'record', request());
    const pending = update(root);
    const interrupted =
      action === 'record'
        ? null
        : run(root, 'record', pending, {
            fault: (p) => (p === 'after-replace:0' ? 'crash' : undefined),
          }).result;
    const original = RecordFiles.prototype.hash;
    let injected = false;
    RecordFiles.prototype.hash = function (path) {
      ...
      if (!injected && path === manifest && caller.includes('Store.publish')) {
        injected = true;
        writeFileSync(join(root, manifest), 'external-third-state\n');
      }
    ...
    try {
      result =
        action === 'record'
          ? run(root, 'record', pending).result
          : recover(root, interrupted, action).result;
    } finally {
      RecordFiles.prototype.hash = original;
    }
    assert.equal(injected, true);
    assert.equal(result.status, 'recovery-required');
...
for (const operation of ['write', 'exclusive', 'remove', 'mkdir', 'removeDirectory'])
  test(`ER-M2-004 late parent substitution cannot redirect ${operation}`, (t) => {
    const root = setup(t),
      outside = setup(t),
      parent = join(root, 'records'),
      moved = join(root, 'aside');
    mkdirSync(parent);
    writeFileSync(join(parent, 'file'), 'before');
    writeFileSync(join(outside, 'file'), 'outside');
    mkdirSync(join(parent, 'empty'));
    mkdirSync(join(outside, 'empty'));
    const access = new RecordFiles(root),
      directory = fs.statSync(join(parent, 'empty'));
    const method = {
      write: 'renameSync',
      exclusive: 'openSync',
      remove: 'unlinkSync',
    ...
    try {
      try {
        if (operation === 'write')
          access.write('records/file', Buffer.from('candidate'), { expected: hash('before') });
        if (operation === 'exclusive')
          access.write('records/new', Buffer.from('candidate'), { exclusive: true });
        ...
        if (operation === 'removeDirectory')
          access.removeDirectory('records/empty', `${directory.dev}:${directory.ino}`);
      } catch (e) {
        assert.ok(e.recordStoreCode || e.code);
      }
    } finally {
      fs[method] = original;
      syncBuiltinESMExports();
...
test('TG-03 inspect/check are read-only; record is exact and has observations without eligibility', (t) => {
  ...
  assert.deepEqual(absent.snapshot, { records: [] });
  ...
  const updated = update(root),
    saved = run(root, 'record', updated).result;
  assert.equal(saved.status, 'saved');
  assert.equal(saved.claim, 'storage-only');
  ...
  assert.equal(
    readFileSync(join(root, 'docs/changes/example/unrelated.txt'), 'utf8'),
    'preserve\n',
  );
...
test('TG-04 exact revision/write/read preconditions and stale retry', (t) => {
  ...
  const u = update(root);
  assert.equal(run(root, 'record', u).result.status, 'saved');
  const same = update(root);
  assert.equal(run(root, 'record', same).result.status, 'unchanged');
  writeFileSync(join(root, 'basis'), 'before');
  same.reads = [{ path: 'basis', expected_identity: hash('before') }];
  writeFileSync(join(root, 'basis'), 'after');
  assert.equal(run(root, 'record', same).result.status, 'conflict');
  same.reads = [];
  same.writes[0].expected_identity = null;
  assert.equal(run(root, 'record', same).result.status, 'conflict');
});
...
test('ER-M2-001 malformed selectors produce truthful safe rejections before IO', (t) => {
  ...
  for (const [argv, op, id, format] of [
    ...
    [
      ['check', '--change', 'example', '--change', 'example', '--format', 'json'],
      'check',
      null,
      'json',
    ],
    ...
    [
      ['inspect', '--change', 'example', '--format', 'json', '--format', 'json'],
      'inspect',
      'example',
      'text',
    ],
...
test('TG-03 historical roots, unknown flags, unsafe paths and existing roots reject unchanged', (t) => {
  ...
  assert.equal(
    run(root, 'inspect', undefined, {}, ['--unknown_value', 'sensitive']).result.errors[0].code,
    'invalid-input',
  );
  unlinkSync(join(root, manifest));
  symlinkSync(join(root, 'outside'), join(root, manifest));
  assert.equal(run(root, 'inspect').result.errors[0].code, 'unsafe-path');
  unlinkSync(join(root, manifest));
  writeFileSync(join(root, 'outside'), r.writes[0].content);
...
test('TG-04 interruption has explicit complete/restore and never exposes a mixed snapshot', (t) => {
  for (const action of ['complete', 'restore']) {
    const root = setup(t),
      r = request();
    const interrupted = run(root, 'record', r, {
      fault: (p) => (p === 'after-replace:0' ? 'crash' : undefined),
    }).result;
...
test('TG-04 third-state edits and stale recovery identity stop without overwrite', (t) => {
  const root = setup(t),
    interrupted = run(root, 'record', request(), {
      fault: (p) => (p === 'after-replace:0' ? 'crash' : undefined),
    }).result;
  ...
  const stale = structuredClone(interrupted);
  stale.transaction.recovery_identity = hash('stale');
...
test('TG-04 committed transaction refuses rollback and permits cleanup', (t) => {
  const root = setup(t),
    r = request();
  const interrupted = run(root, 'record', r, {
    fault: (p) => (p === 'after-commit' ? 'crash' : undefined),
  }).result;
  assert.equal(interrupted.status, 'recovery-required');
  assert.equal(recover(root, interrupted, 'restore').result.status, 'recovery-required');
  assert.equal(readFileSync(join(root, manifest), 'utf8'), r.writes[0].content);
...
test('TG-03 real public dispatcher supports text and JSON recording', (t) => {
  const root = setup(t),
    launcher = new URL('../dist/bin/rigorloop.js', import.meta.url).pathname;
  run(root, 'record', request()); // Supported fixture, then real public update.
  for (const format of ['text', 'json'])
    for (const op of ['inspect', 'check', 'record']) {
      const input =
        op === 'inspect'
          ? undefined
          : JSON.stringify(existsSync(join(root, manifest)) ? update(root) : request()) + '\n';
      const argv = args(root, op);
      argv[argv.indexOf('json')] = format;
      const child = spawnSync(process.execPath, [launcher, 'record-store', ...argv], {
        input,
        encoding: 'utf8',
      });
...
test('ER-M4-005 public text names available and absent record identities', (t) => {
  const root = setup(t),
    launcher = new URL('../dist/bin/rigorloop.js', import.meta.url).pathname;
  const r = request(),
    change = JSON.parse(r.writes[0].content),
  ...
  r.writes.push({
    ...
    content: JSON.stringify(fixture().evidence) + '\n',
  });
  run(root, 'record', r);
  const seeded = run(root, 'inspect').result;
  r.expected_revision = seeded.revision;
  for (const w of r.writes) w.expected_identity = hash(w.content);
  const argv = args(root, 'record');
  argv[argv.indexOf('json')] = 'text';
  const saved = spawnSync(process.execPath, [launcher, 'record-store', ...argv], {
    encoding: 'utf8',
  ...
  for (const file of current.files)
    assert.ok(saved.stdout.includes(`${file.path}: ${file.identity}\n`));
  unlinkSync(join(root, path));
  const inspect = args(root, 'inspect');
  inspect[inspect.indexOf('json')] = 'text';
  const missing = spawnSync(process.execPath, [launcher, 'record-store', ...inspect], {
    encoding: 'utf8',
  });
...
test('TG-03 missing registered records are visible and repairable; registry removal is rejected', (t) => {
  ...
  r.writes[0].content = JSON.stringify(change) + '\n';
  r.writes.push({
    path: change.records[0].path,
    expected_identity: null,
    content: JSON.stringify(f.evidence) + '\n',
  });
  assert.equal(run(root, 'record', r).result.status, 'saved');
  unlinkSync(join(root, change.records[0].path));
  const inspected = run(root, 'inspect').result;
  assert.equal(
    inspected.snapshot.records.find((e) => e.path.endsWith('evidence.json')).content,
    null,
  );
  r.expected_revision = inspected.revision;
  r.writes = [r.writes[1]];
  assert.equal(run(root, 'record', r).result.status, 'saved');
  ...
  const result = run(root, 'record', r, {
    fault: (point) => {
      if (point === 'after-preparation')
        competitor = spawnSync(
          process.execPath,
          [launcher, 'record-store', ...args(root, 'record')],
          { input: JSON.stringify(r) + '\n', encoding: 'utf8' },
        );
    },
  });
  assert.equal(result.result.status, 'saved');
  assert.equal(competitor.status, 4);
...
test('TG-04 drift after replacement restores before bytes and leaves decision basis untouched', (t) => {
  ...
  const before = readFileSync(join(root, manifest), 'utf8'),
    r = update(root);
  writeFileSync(join(root, 'basis'), 'before');
  r.reads = [{ path: 'basis', expected_identity: hash('before') }];
  const result = run(root, 'record', r, {
    fault: (point) => {
      if (point === 'before-commit') writeFileSync(join(root, 'basis'), 'external');
    },
  });
...
test('TG-04 current v3 creation crash and public recover subprocesses preserve complete/restore in both formats', (t) => {
  ...
  for (const format of ['text', 'json'])
    for (const action of ['complete', 'restore']) {
      ...
      const blocked = run(root, 'inspect').result;
      assert.equal(blocked.status, 'recovery-required');
      const argv = args(root, 'recover', [
        '--transaction',
        blocked.transaction.id,
        '--expected-recovery',
        blocked.transaction.recovery_identity,
        '--action',
        action,
      ]);
      argv[argv.indexOf('json')] = format;
      const recovery = spawnSync(process.execPath, [launcher, 'record-store', ...argv], {
        encoding: 'utf8',
      });
...
test('TG-04 interrupted recovery is repeatable and missing/tampered/unknown_value journals stop', (t) => {
  for (const fault of ['after-preparation', 'after-replace:0']) {
    const root = setup(t),
      interrupted = run(root, 'record', request(), {
        fault: (p) => (p === fault ? 'crash' : undefined),
      }).result;
    const again = recover(root, interrupted, 'complete', {
      fault: (p) => (p === 'after-replace:0' ? 'crash' : undefined),
    }).result;
  ...
  for (const mutate of [
    (j) => (j.phase = 'unknown_value'),
    (j) => (j.candidate[manifest].content += ' '),
    (j) => (j.extra = 'unknown_value'),
  ]) {
    const root = setup(t),
      stopped = run(root, 'record', request(), {
...
test('TG-04 multi-record partial publication restores exact prior bytes', (t) => {
  const root = setup(t),
    r = request(),
    f = fixture(),
    evidence = 'docs/changes/example/evidence.json';
  const change = JSON.parse(r.writes[0].content);
  ...
  for (const w of r.writes) w.expected_identity = hash(w.content);
  change.activity.status = 'completed';
  r.writes[0].content = JSON.stringify(change) + '\n';
  f.evidence.checks[0].summary = 'new proof';
  r.writes[1].content = JSON.stringify(f.evidence) + '\n';
  ...
  for (const file of before.snapshot.records)
...
test('TG-03 public rejection is bounded and ordinary symlink launcher retains existing behavior', (t) => {
  const root = setup(t),
    launcher = new URL('./helpers/record-store-launcher.mjs', import.meta.url).pathname;
  for (const argv of [
    ['unknown_value', '--format', 'json'],
    ['inspect', '--change', '../sensitive', '--format', 'json'],
    ['inspect', '--root', root, '--change', 'example', '--input', 'secret', '--format', 'json'],
  ]) {
    const child = spawnSync(process.execPath, [launcher, 'record-store', ...argv], {
      encoding: 'utf8',
    });
  ...
  const alias = join(root, 'rigorloop');
  symlinkSync(new URL('../dist/bin/rigorloop.js', import.meta.url).pathname, alias);
  const version = spawnSync(process.execPath, [alias, 'version'], {
    cwd: root,
    encoding: 'utf8',
  ...
  assert.equal(
    version.stdout.trim(),
    `@xiongxianfei/rigorloop ${JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version}`,
  );
});
...
test('TG-04 pre-journal failure changes no authoritative bytes', (t) => {
  const root = setup(t),
    r = request();
  const result = run(root, 'record', r, {
    fault: (p) => (p === 'after-lock' ? 'fail' : undefined),
  });
  assert.equal(result.result.status, 'rejected');
  assert.equal(existsSync(join(root, manifest)), false);
...
test('TG-04 unexpected IO failures are safe io-failure results', (t) => {
  const root = setup(t);
  const result = run(root, 'record', request(), {
    fault: (p) => {
      if (p === 'after-lock')
        throw Object.assign(new Error('private failure payload'), { code: 'ENOSPC' });
    },
  });
...
test('TG-04 concurrent ancestor substitution stops publication without escaped bytes', (t) => {
  ...
  mkdirSync(outside);
  writeFileSync(join(outside, 'change.json'), 'external\n');
  let substituted = false;
  const result = run(root, 'record', r, {
    fault: (point) => {
      if (point === 'before-replace:0' && !substituted) {
        substituted = true;
        renameSync(join(root, 'docs/changes/example'), join(root, 'docs/changes/preserved'));
...
test('TG-04 inspection detects an overlapping completed writer', (t) => {
  ...
  const r = update(root);
  let wrote = false;
  const inspected = run(root, 'inspect', undefined, {
    fault: (point) => {
      if (point === 'during-inspect' && !wrote) {
        wrote = true;
        assert.equal(run(root, 'record', r).exitCode, 0);
      }
    },
  });
  assert.equal(inspected.result.status, 'conflict');
  assert.equal(inspected.result.snapshot, null);
  assert.equal(run(root, 'inspect').exitCode, 0);
});
