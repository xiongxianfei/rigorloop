...
function fileSet() {
  const f = fixtures();
  return {
    [root + 'change.json']: encode('change', f.change),
    ...Object.fromEntries(f.change.records.map((r) => [r.path, encode(r.kind, f[r.kind])])),
  };
}
test('ER-M1-002 distinct diagnostics may share a path or no path', () => {
  for (const path of [root + 'evidence.json', null]) {
    ...
    result.observations = ['failed-evidence', 'subject-drift'].map((code) => ({
      code,
      path,
      message: 'Observed condition',
    }));
    assert.doesNotThrow(() => validateAdvancedResult(result));
    result.status = 'rejected';
    result.errors = ['invalid-input', 'broken-reference'].map((code) => ({
      code,
      path,
...
function inspected(files = fileSet()) {
  ...
  const identities = records.map(({ path, content }) => ({
    path,
    identity: content === null ? null : digest(content),
  }));
  return {
    ...fixtures().result,
    operation: 'inspect',
    ...
    revision: records.length
      ? digest(JSON.stringify(identities.map((f) => [f.path, f.identity])))
      : null,
    observations: records.length
      ? records
          .filter((r) => r.content === null)
          .map((r) => ({ code: 'subject-drift', path: r.path, message: 'Missing record' }))
...
test('ER-M1-003 inspect represents complete, absent and missing-sidecar snapshots', () => {
  const missing = fileSet();
  missing[root + 'evidence.json'] = null;
...
test('ER-M1-003 inspect rejects invalid content, membership and absent-root fields', () => {
  for (const content of [
    'not a manifest\n',
    null,
    ' '.repeat(1024 * 1024) + encode('change', fixtures().change),
  ]) {
    assert.throws(() =>
      validateAdvancedResult(inspected({ ...fileSet(), [root + 'change.json']: content })),
    );
  }
  const extra = {
    ...fileSet(),
    [root + 'reviews/extra.json']: encode('review', { ...fixtures().review, id: 'extra' }),
  };
  const omitted = fileSet();
  delete omitted[root + 'evidence.json'];
  const wrongChange = {
    ...fileSet(),
    [root + 'evidence.json']: encode('evidence', { ...fixtures().evidence, change_id: 'another' }),
  };
  const wrongReview = {
    ...fileSet(),
    [root + 'reviews/design-review.json']: encode('review', {
      ...fixtures().review,
      id: 'another',
    }),
  };
  for (const files of [extra, omitted, wrongChange, wrongReview])
    assert.throws(() => validateAdvancedResult(inspected(files)));
  const missing = fileSet();
  missing[root + 'evidence.json'] = null;
  const noObservation = inspected(missing);
  noObservation.observations = [];
  ...
  const wrongRevision = inspected();
  wrongRevision.revision = digest('wrong');
...
test('ER-M2-001 unavailable result selectors have only the empty input-rejection shape', () => {
  ...
  for (const selectors of [
    ...
    for (const mutate of [
      (r) => (r.status = 'saved'),
      (r) => (r.errors = []),
      (r) => (r.errors[0].code = 'io-failure'),
      (r) => (r.revision = digest('x')),
      (r) => (r.observations = fixtures().result.observations),
    ]) {
  ...
  for (const key of ['operation', 'change_id'])
    assert.throws(() => validateAdvancedResult({ ...rejected, [key]: 'unknown_value' }));
});
...
test('TG-01 exact ID, path, registry, write and read count limits', () => {
  ...
  change.records = Array.from({ length: 64 }, (_, i) => ({
    path: root + `reviews/r-${i}.json`,
    kind: 'review',
  }));
  change.applicability = change.records.map((r) => ({
    ...fixtures().change.applicability[0],
    path: r.path,
  }));
  assert.doesNotThrow(() => validateV3Record('change', change));
  const request = fixtures().request;
  request.writes = [
    { ...request.writes[0], content: encode('change', change) },
    ...change.records.map((r, i) => ({
      path: r.path,
      expected_identity: null,
      content: encode('review', { ...fixtures().review, id: `r-${i}` }),
    })),
  ];
  request.reads = Array.from({ length: 256 }, (_, i) => ({
    path: `docs/basis-${i}.json`,
    expected_identity: null,
  }));
