...
const actor={id:'reviewer',role:'review'},app={value:'current',actor,reason:'Explicit interaction declaration'};
...
function setup(){const root=mkdtempSync(join(tmpdir(),'recording-interactions-'));mkdirSync(join(root,'docs/changes'),{recursive:true});mkdirSync(join(root,dirname(basisPath)),{recursive:true});writeFileSync(join(root,basisPath),engineeringBasis);const f=structuredClone(fixture),subject={path:basisPath,identity:digest(engineeringBasis)};function subjects(x){if(!x||typeof x!=='object')return;if(Object.hasOwn(x,'path')&&Object.hasOwn(x,'identity')){Object.assign(x,subject);return;}for(const v of Object.values(x))subjects(v);}for(const k of ['change','review','evidence','decisions','verify'])subjects(f[k]);f.review.findings=Array.from({length:24},(_,i)=>({...structuredClone(f.review.findings[0]),id:`neighbor-${i}`,evidence:`Observed case ${i}: ${engineeringBasis}`,required_outcome:`Preserve case ${i}'s stated invariant.`}));f.change.blockers=[];f.decisions.decisions[0].source_refs=[{path:rp,id:'neighbor-0'}];f.evidence.checks=Array.from({length:12},(_,i)=>({...structuredClone(f.evidence.checks[0]),id:i?'check-'+(i+1):'check-1',result:'passed',summary:`Check ${i}: ${engineeringBasis}`}));const records={[mp]:f.change,[rp]:f.review,[ep]:f.evidence,[prefix+'material-decisions.json']:f.decisions,[vp]:f.verify};const writes=Object.entries(records).map(([path,value])=>({path,expected_identity:null,content:encode(value)}));const p=executeRecordStore({root,changeId:'example',operation:'record',request:{schema_version:2,contract:'rigorloop-records-v3',change_id:'example',expected_revision:null,reads:[],writes}});if(p.status!=='saved')throw Error(JSON.stringify(p));return{root,subject,records};}
const own=(object,keys)=>Object.fromEntries(keys.map(k=>[k,structuredClone(object[k])]));
function decisions(scenario,records,subject){const finding={reporter:actor,owner:{id:'implementer',role:'implement'},subjects:[subject],evidence:'Observed preservation defect in the selected engineering basis.',required_outcome:'Demonstrate unchanged neighboring entries.',state:'open',resolution:null,basis:{rationale:'The failed preservation guarantee requires correction.',supporting_judgment:{from_review:'design-review',rationale:'The selected independent review supplies the original assessed subject.'}}};if(scenario==='finding-with-neighbors'){delete finding.basis;return[{op:'finding.add',target:{review:'design-review',id:'new-finding'},values:finding}];}if(scenario==='failed-verify-correction')return[{op:'evidence.record',target:{id:'final-failure'},values:{actor:{id:'verifier',role:'verify'},subjects:[subject],result:'failed',procedure:'Compare neighboring records against the preservation requirement.',summary:'A neighbor changed during the simulated failed check.'}},{op:'blocker.add',target:{id:'verify-defect'},values:{...finding,reporter:{id:'verifier',role:'verify'},basis:{rationale:findingsRationale(),supporting_judgment:null}}}];if(scenario==='applicability-reassessment')return[{op:'applicability.set',target:{path:rp},values:{value:'stale',actor,reason:'Explicitly withdraw reliance pending reassessment.'}},{op:'review.record',target:{id:'design-review'},values:{...own(records[rp],['target','reviewer','contributors','independence_basis','subjects','judgment','summary','assessment_scope','rationale','limitations']),judgment:'approved',summary:'The selected current subjects were independently reassessed against the preservation requirement.'},applicability:{...app,value:'current',reason:'Explicit reassessment against the current subjects.'}}];return[];}
function findingsRationale(){return 'Verify observed the defect; correction remains required despite recorded completion.';}
function baselineCandidate(records,operations){const copy=structuredClone(records),touched=new Set();for(const op of operations){if(op.op==='finding.add'||op.op==='blocker.add'){const {basis,...values}=op.values;let supporting=basis?.supporting_judgment;if(supporting?.from_review){const r=copy[rp];supporting={...own(r,['reviewer','contributors','independence_basis','subjects','judgment']),rationale:supporting.rationale};}const entry=op.op==='finding.add'?{id:op.target.id,...values}:{id:op.target.id,...values,origin:{...own(values,['reporter','subjects','evidence','required_outcome']),rationale:basis.rationale,supporting_judgment:supporting}};const path=op.op==='finding.add'?rp:mp;copy[path][op.op==='finding.add'?'findings':'blockers'].push(entry);touched.add(path);}else if(op.op==='evidence.record'){copy[ep].checks.push({id:op.target.id,...op.values});touched.add(ep);}else if(op.op==='applicability.set'){Object.assign(copy[mp].applicability.find(a=>a.path===op.target.path),op.values);touched.add(mp);}else{Object.assign(copy[rp],op.values);Object.assign(copy[mp].applicability.find(a=>a.path===rp),op.applicability);touched.add(rp);touched.add(mp);}}return{copy,touched};}
export function runInteraction(scenario,mode){
 if(!SCENARIOS.includes(scenario)||!['targeted','advanced'].includes(mode))throw Error('Unknown interaction selection');
 const state=setup(),{root,subject,records}=state,calls=[],checkpoints=[];
 function call(words,payload,extra=[]){const args=[...words,'--root',root,'--change','example',...(payload?['--input','-']:[]),'--format','json',...extra];if(words[0]==='subject'){const i=args.indexOf('--change');args.splice(i,2);}const r=spawnSync(process.execPath,[cli,...args],{input:payload?encode(payload):undefined,encoding:'utf8',maxBuffer:16*1024*1024});calls.push(words.slice(0,2).filter(w=>!w.startsWith('--')).join('.'));if(r.status)throw Error(r.stdout+r.stderr);return JSON.parse(r.stdout);}
 try{
  ...
  if(scenario==='final-explanation-read'){
   ...
   return{scenario,mode,calls,semantic:report,basis:report.subjects};
  }
  if(mode==='targeted')call(['subject','inspect','--path',basisPath,'--content','full']);
   ...
   execFileSync(process.execPath,['-e',script,basisPath],{cwd:root,encoding:'utf8'});calls.push('subject.read-and-hash');
  }
  const groups=scenario==='applicability-reassessment'?operations.map(op=>[op]):[operations];
  let currentRecords=records;
  for(const operations of groups){
  if(mode==='advanced'){
   const {copy,touched}=baselineCandidate(currentRecords,operations),request={schema_version:2,contract:'rigorloop-records-v3',change_id:'example',expected_revision:snapshot.revision,reads:[{path:subject.path,expected_identity:subject.identity}],writes:[...touched].map(path=>({path,expected_identity:snapshot.files.find(f=>f.path===path).identity,content:encode(copy[path])}))};
   ...
   for(const op of operations){const words=op.op.split('.');const help=spawnSync(process.execPath,[cli,...words,'--help'],{encoding:'utf8'});if(help.status)throw Error(help.stderr);calls.push(words.join('.')+'.help');}
  ...
  return{scenario,mode,calls,semantic,checkpoints,basis:[subject]};
