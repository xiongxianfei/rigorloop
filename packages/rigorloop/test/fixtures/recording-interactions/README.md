...
Advanced operations inspect the complete store, construct full replacement records, check when preview is selected, record, then inspect the complete store again. Targeted operations select review/activity context, inspect the subject, load operation help, submit targeted edits and inspect affected entries. Final-report reads compare full advanced inspection with an explicit full Verify context selector. Each path is also checked independently against the required added values, original neighboring records and complete final report. Unrelated files must retain their actual pre-operation bytes; separate source-preservation cases cover omitted tokens within edited files. Fixture cleanup covers setup failures as well as completed operations.
