...
test('TG-02 v3 finding current fields can change while identity and blockers retain their own invariants', () => {
  ...
  const finding = read('v3-finding-correction/stored-review.json').findings[0];
  edit(before, 'reviews/final-code-review.json', (r) => (r.findings = [finding]));
  const after = structuredClone(before);
  edit(after, 'reviews/final-code-review.json', (r) => {
    r.findings[0].evidence = 'Corrected evidence';
    r.findings[0].reporter.id = 'correct-reporter';
    r.findings[0].subjects = [];
    r.findings[0].state = 'resolved';
    r.findings[0].resolution = {
      actor: { id: 'reviewer-a', role: 'review' },
      rationale: 'Withdraw mistaken report',
      evidence_refs: [],
    };
  });
  ...
  for (const fn of [
    (r) => (r.findings = []),
    (r) => (r.findings[0].id = 'renamed'),
    (r) => (r.findings[0].origin = {}),
    (r) => (r.findings[0].state = 'open'),
  ]) {
    const bad = structuredClone(after);
    edit(bad, 'reviews/final-code-review.json', fn);
