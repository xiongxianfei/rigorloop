...
import {
  RECORDS_V3_SCHEMA, validateV3Record, parseV3Record, validateV3Set,
  validateV3Preservation, validateV3Creation, v3PathKind,
...
test("TG-01 v3 complete fixtures and packaged templates validate without mutation", () => {
  ...
  const templates=JSON.parse(readFileSync(new URL("../../../templates/rigorloop-records-v3/records.json",import.meta.url)));
  for(const [kind,value] of Object.entries(templates)) validateV3Record(kind,value);
  for(const path of ["schemas/rigorloop-records-v3.schema.json","templates/rigorloop-records-v3/records.json"]) {
...
test("TG-01 unknown_value closed vocabularies fail before consistency", () => {
  ...
  const cases=[
    ...
    ["request",x=>x.schema_version=1],["review",x=>x.reviewer.role="unknown_value"],
  ];
  for(const [kind,mutate]of cases) {
    const value=structuredClone(f[kind]);mutate(value);
    assert.throws(()=>validateV3Record(kind,value),e=>["invalid-input","unsupported-contract"].includes(e.recordStoreCode));
  }
...
test("TG-01 plain JSON rejects front matter trailing narrative duplicate keys and invalid encoding", () => {
  ...
  for(const body of [null,"",4]) {const r=fixture().review;r.summary=body;reject(()=>validateV3Record("review",r));}
  const r=fixture().review;r.summary='Quoted "text"\nUnicode 确认 and \\ escapes';
  assert.equal(parseV3Record("review",encode(r)).summary,r.summary);
  r.extra=true;reject(()=>validateV3Record("review",r));
});

test("TG-01 byte/depth/path limits and JSON domain reject", () => {
  const r=fixture().review;r.summary="a".repeat(1024*1024);
  ...
  const r2=fixture().review;r2.summary="\ud800";reject(()=>validateV3Record("review",r2));
});
...
test("TG-01 namespaces reject wrong extensions dual manifests mixed versions and wrong review id", () => {
  ...
  r.schema_version=3;r.id="other";f[root+"reviews/design-review.json"]=encode(r);
...
test("TG-01 disjoint referenceable ids reject even without a reference", () => {
  ...
  const r=fixture().review;r.findings[0].id=r.id;reject(()=>validateV3Record("review",r));
});
...
test("TG-01 registry applicability duplicates and final candidate references", () => {
  for(const mutate of [c=>c.applicability.pop(),c=>c.records[0].kind="evidence",c=>c.records.push(c.records[0])]) {
    const c=fixture().change;mutate(c);reject(()=>validateV3Record("change",c));
  }
  const map=files();delete map[root+"evidence.json"];reject(()=>validateV3Set("example",map),"broken-reference");
  map[root+"evidence.json"]=encode(fixture().evidence);assert.doesNotThrow(()=>validateV3Set("example",map));
});
...
test("TG-01 v3 advanced request validates candidate bytes and explicit absent creation", () => {
  ...
  request.reads=[{path:request.writes[0].path,expected_identity:null}];reject(()=>validateV3Record("request",request));
});
...
test("TG-01 unknown_value regression reaches every stored and request vocabulary occurrence", () => {
  ...
  function walk(schema,value,path,kind) {
    if(schema.$ref) return walk(RECORDS_V3_SCHEMA.$defs[schema.$ref.split("/").at(-1)],value,path,kind);
    if(schema.anyOf) {
    ...
    if(schema.enum || Object.hasOwn(schema,"const")) {
      ...
      assert.throws(()=>validateV3Record(kind,candidate),e=>["invalid-input","unsupported-contract"].includes(e.recordStoreCode),`${kind}.${path.join(".")}`);
      covered.add(kind+"."+path.join("."));
    }
    if(schema.type==="object") for(const [key,child] of Object.entries(schema.properties)) if(Object.hasOwn(value,key))walk(child,value[key],[...path,key],kind);
    if(schema.type==="array") value.forEach((entry,index)=>walk(schema.items,entry,[...path,index],kind));
  }
  for(const [kind,value] of Object.entries(cases)) walk(RECORDS_V3_SCHEMA.$defs[kind],value,[],kind);
...
test("TG-01 nested optional references retain unsafe-path diagnostics", () => {
  ...
  const change=fixture().change;change.blockers[0].origin.supporting_judgment.subjects[0].path="../escape";
...
test("TG-01 either manifest entry rejects dual manifests live and at an exact Git snapshot", () => {
  ...
  try {
    ...
    writeFileSync(join(dir,root+"change.json"),encode(JSON.parse(readFileSync(new URL("../../../templates/rigorloop-records-v3/records.json",import.meta.url))).change));
...
test("TG-01 unknown_value version and contract pairs reject unsupported-contract", () => {
  for(const [kind,value] of Object.entries(fixture())) {
    value.schema_version=99;reject(()=>validateV3Record(kind,value),"unsupported-contract");
    value.schema_version=1;reject(()=>validateV3Record(kind,value),"unsupported-contract");
  }
