...
for(const contract of ['rigorloop-records-v3'])test(`TG-FINAL-01 public correction, recovery and explanation: ${contract}`,t=>{
 ...
 const historical={schema_version:3,contract,change_id:'example',...initial,records:[],applicability:[]};assert.equal(executeRecordStore({root,changeId:'example',operation:'record',request:{schema_version:2,contract,change_id:'example',expected_revision:null,reads:[],writes:[{path:mp,expected_identity:null,content:encode(historical)}]}}).status,'saved');
 ...
 mutate({op:'review.record',target:{id:'final-review'},values:{target:'code',reviewer,contributors:[implementer],independence_basis:'Separate reviewer assesses this complete fixture implementation.',subjects:[subject],judgment:'approved',summary:'The corrected subject meets the stated requirement.\n',assessment_scope:'Correction fixture.',rationale:['The corrected check and independent review cover the requirement.'],limitations:[]},applicability:app(reviewer)});
...
test('TG-FINAL-01 public correction receipt with a densely filled evidence record',t=>{
 ...
 const evidence={schema_version:3,change_id:'example',checks:[]};let size=Buffer.byteLength(encode(evidence));
 ...
 assert.equal(executeRecordStore({root,changeId:'example',operation:'record',request:{schema_version:2,contract:'rigorloop-records-v3',change_id:'example',expected_revision:null,reads:[],writes:[{path:mp,expected_identity:null,content:encode(change)},{path:ep,expected_identity:null,content:encode(evidence)}]}}).status,'saved');
