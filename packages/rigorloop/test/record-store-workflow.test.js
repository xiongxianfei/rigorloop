...
const author = { id: 'author', role: 'design' },
  reviewer = { id: 'reviewer', role: 'review' },
  verifier = { id: 'verifier', role: 'verify' };
const encode = (value) => JSON.stringify(value) + '\n';
const withFixtureSummary = (value) =>
  encode({
    ...value,
    summary: 'Fixture reasoning; actor labels alone do not establish independent review.\n',
  });
...
test('TG-05 actors explicitly invalidate, reopen, report Verify failure, rereview and complete', (t) => {
  ...
  for (const dir of ['docs/changes', 'docs/design/skill', 'docs/design/cli', 'docs/proposals'])
    mkdirSync(join(root, dir), { recursive: true });
  const wf = 'docs/design/skill/workflow.md',
    cli = 'docs/design/cli/cli.md',
    proposal = 'docs/proposals/example.md';
  for (const path of [wf, cli])
  ...
  const manifest = 'docs/changes/example/change.json',
    reviewPath = 'docs/changes/example/reviews/design-review.json',
    evidencePath = 'docs/changes/example/evidence.json',
    verifyPath = 'docs/changes/example/verify-report.json';
  ...
  Object.assign(change, {
    ...
    models: [
      { id: 'workflow', subject: subject(wf) },
      { id: 'cli', subject: subject(cli) },
    ],
    blockers: [],
  });
  change.activity = {
    ...
    status: 'completed',
    owner: author,
    reason: 'Explicit prior completion',
  };
  change.work[0].status = 'completed';
  const launcher =
    process.env.RIGORLOOP_TEST_PACKAGED_BIN ??
    fileURLToPath(new URL('../dist/bin/rigorloop.js', import.meta.url));
  function invoke(operation, input) {
    const result = spawnSync(
      ...
      { input: input ? encode(input) : undefined, encoding: 'utf8' },
    );
    assert.equal(result.status, 0, result.stdout + result.stderr);
    return JSON.parse(result.stdout);
  }
  function save(sidecars = {}) {
    const before = invoke('inspect'),
      identities = new Map(before.files.map((f) => [f.path, f.identity]));
    ...
    const request = {
      ...
      writes: Object.entries(contents).map(([path, content]) => ({
        path,
        content,
        expected_identity: identities.get(path) ?? null,
      })),
  ...
  const review = {
    ...
    reviewer,
    contributors: [author],
    subjects: [subject(wf), subject(cli)],
    judgment: 'approved',
    findings: [],
  };
  const evidence = {
    ...fixture.evidence,
    checks: [
      { ...fixture.evidence.checks[0], subjects: [subject(wf), subject(cli)], result: 'passed' },
    ],
  };
  change.records = [
    { path: reviewPath, kind: 'review' },
    { path: evidencePath, kind: 'evidence' },
  ];
  change.applicability = change.records.map((r) => ({
    path: r.path,
    value: 'current',
    actor: reviewer,
  ...
  const oldReview = readFileSync(join(root, reviewPath), 'utf8'),
    oldActivity = structuredClone(change.activity);
  ...
  assert.deepEqual(
    JSON.parse(drift.snapshot.records.find((r) => r.path === manifest).content).activity,
    oldActivity,
  );
  assert.equal(readFileSync(join(root, reviewPath), 'utf8'), oldReview);
  change.models[0].subject = subject(wf);
  change.applicability = change.applicability.map((a) => ({
    ...a,
    value: 'stale',
    actor: author,
    reason: 'Reviewed model changed; do not rely on old proof',
  }));
  save();
  assert.equal(readFileSync(join(root, reviewPath), 'utf8'), oldReview);
  ...
  change.activity = {
    ...
    status: 'in-progress',
    owner: author,
    reason: 'Route explicitly reopens completed owner',
  };
  ...
  blocker.origin = {
    ...Object.fromEntries(
      ['reporter', 'subjects', 'evidence', 'required_outcome'].map((k) => [
        k,
        structuredClone(blocker[k]),
      ]),
    ),
    rationale: 'Synthetic Verify finding.',
  ...
  change.activity = {
    ...
    status: 'blocked',
    owner: verifier,
    reason: 'New defect found at Verify',
  };
  evidence.checks[0] = {
    ...evidence.checks[0],
    actor: verifier,
  ...
  change.activity = {
    ...
    status: 'in-progress',
    owner: author,
    reason: 'Route selects correction owner for Verify blocker',
  };
  save();
  ...
  change.applicability = change.applicability.map((a) => ({
    ...a,
    value: 'current',
    actor: a.path === reviewPath ? reviewer : verifier,
    reason: 'Explicit new assessment',
  }));
  save({ [reviewPath]: withFixtureSummary(review), [evidencePath]: encode(evidence) });
  ...
  blocker.resolution = {
    actor: verifier,
    rationale: 'Correction reassessed',
    evidence_refs: [{ path: evidencePath, id: evidence.checks[0].id }],
  };
  change.work[0].status = 'completed';
  change.activity = {
    stage: 'verify',
    status: 'completed',
    owner: verifier,
    reason: 'Explicit fixture final decision',
  };
  change.records.push({ path: verifyPath, kind: 'verify' });
  change.applicability.push({
    path: verifyPath,
    value: 'current',
    actor: verifier,
    reason: 'Explicit successful assessment',
  });
