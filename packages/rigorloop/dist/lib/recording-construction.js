...
import {V1_FORMAT,V2_FORMAT} from './record-store-format.js';
...
export function validateMutationRequest(request,batch=false){
 ...
 const version=request.contract===V1_FORMAT.contract?1:request.contract===V2_FORMAT.contract?2:null;
 ...
 for(const [index,operation]of operations.entries()){
  try{
   const schema=RECORDING_SCHEMA.$defs[`v${version}-operation-${operation?.op}`];if(!schema)stop('invalid-input');
   if(operation.op==='change.create'&&(batch||version===1))stop('invalid-input');
   ...
   if(version===2&&(Object.hasOwn(operation.values,'origin')||(operation.op.endsWith('.set')&&Object.hasOwn(operation.values,'basis'))))stop('immutable-origin');
   for(const field of schema.properties.values.required??[])if(!Object.hasOwn(operation.values,field))missing(field);
   if(operation.op.endsWith('.set')&&!Object.keys(operation.values).length)missing('values');
 ...
 return version===1?V1_FORMAT:V2_FORMAT;
}
...
export function constructMutation(request,before){
 const format=request.contract===V1_FORMAT.contract?V1_FORMAT:V2_FORMAT;
 ...
 function newDoc(path,kind,data,body){edits++;const source=format.version===1&&['review','decisions','verify'].includes(kind)?`---\n${canonicalJSON(data)}\n---\n${body}`:canonicalJSON({...data,...(body!==undefined?{body}: {})})+'\n';const doc=new RecordDocument(format,kind,source,format.version===1?data:{...data,...(body!==undefined?{body}: {})});docs.set(path,doc);touched.add(doc);return doc;}
 function concern(values,id){const {basis,...current}=structuredClone(values);if(format.version===1)return{id,...current};let supporting_judgment=basis.supporting_judgment;if(supporting_judgment?.snapshot)supporting_judgment=supporting_judgment.snapshot;else if(supporting_judgment?.from_review){const review=use(prefix+`reviews/${supporting_judgment.from_review}.json`).data;supporting_judgment={...Object.fromEntries(['reviewer','contributors','independence_basis','subjects','judgment'].map(k=>[k,structuredClone(review[k])])),rationale:supporting_judgment.rationale};}return{id,...current,origin:{...Object.fromEntries(['reporter','subjects','evidence','required_outcome'].map(k=>[k,structuredClone(current[k])])),rationale:basis.rationale,supporting_judgment}};}
 ...
 for(const op of operations){try{
  ...
  if(op.op==='change.create'){
   if(change)stop('target-exists');const blockers=values.blockers.map(b=>{const {id,...v}=b;return concern(v,id);});change=newDoc(manifest,'change',{schema_version:2,contract:format.contract,change_id:request.change_id,...values,blockers,records:[],applicability:[]});doc=change;
  }else{
   ...
   if(op.op==='change.link'){
   ...
   else if(op.op.startsWith('finding.')){path=prefix+`reviews/${target.review}.${format.version===1?'md':'json'}`;doc=use(path);entry(doc,'findings',target.id,op.op.endsWith('.add')?Object.fromEntries(Object.entries(concern(values,target.id)).filter(([k])=>k!=='id')):values,op.op.split('.')[1]);}
    ...
    path=prefix+(kind==='review'?`reviews/${target.id}.${format.version===1?'md':'json'}`:kind==='evidence'?`evidence.${format.version===1?'yaml':'json'}`:kind==='decision'?`material-decisions.${format.version===1?'md':'json'}`:`verify-report.${format.version===1?'md':'json'}`);
    doc=support(path,recordKind,op);const {body,...fields}=values;
    if(kind==='evidence'||kind==='decision')entry(doc,kind==='evidence'?'checks':'decisions',target.id,fields,'record');
    else{claim(path+':assessment');for(const [key,value]of Object.entries(fields)){if(Object.hasOwn(doc.data,key))set(doc,[key],value);else{ // New-file scaffolds are encoded once after explicit fields are installed.
       doc.data[key]=value;edits++;touched.add(doc);
      }}}
    if(body!==undefined){claim(path+':body');if(Object.hasOwn(before,path)){const prior=doc.source;doc.body(body);if(doc.source!==prior)edits++;}else{if(format.version===2)doc.data.body=body;doc.newBody=body;}touched.add(doc);}
   }
  }
 ...
 const writes=[];for(const [path,doc]of docs)if(touched.has(doc)){let content=doc.source;if(!Object.hasOwn(before,path)){content=format.version===1&&['review','decisions','verify'].includes(doc.kind)?`---\n${canonicalJSON(doc.data)}\n---\n${doc.newBody}`:canonicalJSON(doc.data)+'\n';}writes.push({path,expected_identity:Object.hasOwn(before,path)?digest(before[path]):null,content});}
