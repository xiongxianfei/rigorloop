...
npx @xiongxianfei/rigorloop@0.3.4 init claude --json
npx @xiongxianfei/rigorloop@0.3.4 init opencode --json
```

Preview the write plan without mutating files:

```bash
npx @xiongxianfei/rigorloop@0.3.4 init opencode --dry-run --json
```
...
```bash
npx @xiongxianfei/rigorloop@0.3.4 init codex --from-archive ./rigorloop-adapter-codex-v0.3.4.zip --json
npx @xiongxianfei/rigorloop@0.3.4 init claude --from-archive ./rigorloop-adapter-claude-v0.3.4.zip --json
npx @xiongxianfei/rigorloop@0.3.4 init opencode --from-archive ./rigorloop-adapter-opencode-v0.3.4.zip --json
```
...
```bash
npx @xiongxianfei/rigorloop@0.3.4 new-change my-change --title "Describe the change" --json
```

Preview the scaffold first:

```bash
npx @xiongxianfei/rigorloop@0.3.4 new-change my-change --title "Describe the change" --dry-run --json
```
...
Use `@latest` for manual exploration. Use an explicit version such as `@0.3.4` for CI, onboarding docs, and repeatable agent setup.
