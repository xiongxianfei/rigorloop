...
def write_archive_metadata(root: Path, output: Path, tag: str, commit: str, package_root: Path) -> list[Path]:
    ...
    from lib.packaging.adapter_distribution import _local_release_candidate_metadata
...
def script_identity(directory: Path) -> str:
    if directory.is_symlink():
        raise CandidateError('tooling source directory must not be a symlink')
    # Missing roots must fail instead of acquiring an empty-source identity.
    directory.stat()
    files = []
    for path in sorted(directory.rglob('*')):
        ...
        if '__pycache__' in relative.parts:
            continue
        if path.is_symlink():
            raise CandidateError('tooling source must not contain symlinks')
        if path.is_file() and (path.suffix in {'.py', '.sh', '.mjs'}
                               or relative.parts[0] == 'resources'):
            files.append(path)
    return hashlib.sha256(canonical_bytes({p.relative_to(directory).as_posix(): file_identity(p)
...
def prepare_candidate(source: Path, source_commit: str, merged_ref: str, published_version: str,
    ...
    from lib.release.release_transaction import prepare_release, release_preflight
    ...
    with tempfile.TemporaryDirectory(prefix='rigorloop-candidate-') as temporary:
        ...
        if script_identity(root / 'scripts') != LOADED_SCRIPT_IDENTITY or script_identity(Path(__file__).resolve().parents[2]) != LOADED_SCRIPT_IDENTITY:
        ...
        if script_identity(Path(__file__).resolve().parents[2]) != LOADED_SCRIPT_IDENTITY:
...
LOADED_SCRIPT_IDENTITY = script_identity(Path(__file__).resolve().parents[2])
