...
class CurrentRecordBoundaryTests(unittest.TestCase):
    def test_reserved_residue_without_manifest_is_validated_not_skipped(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            path = root / "docs/changes/example/evidence.json"
            path.parent.mkdir(parents=True)
            path.write_text("{}\n")
            result = validate_repository(root, mode="explicit-paths", paths=[path.relative_to(root).as_posix()])
            self.assertTrue(result.blocking_findings)
            self.assertEqual(path.read_text(), "{}\n")

    def test_explicit_retired_root_rejects_without_parsing_archive(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            path = root / "docs/changes/example/change.yaml"
            path.parent.mkdir(parents=True)
            path.write_bytes(b"private invalid archive\xff")
            result = validate_repository(root, mode="explicit-paths", paths=[path.relative_to(root).as_posix()])
            self.assertTrue(result.blocking_findings)
            self.assertEqual(path.read_bytes(), b"private invalid archive\xff")
