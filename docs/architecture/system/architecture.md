...
- canonical code-state provider in `scripts/workflow_code_state.py`: derives the complete base-to-reviewed Git change set, including additions, modifications, deletions, and renames, and rejects unapproved post-review commits, dirty tracked files, and untracked files; only exact change-local or plan lifecycle-evidence paths may be exempted after review, code paths cannot be exempted, and non-Git fixtures must inject an independently owned provider rather than letting branch evidence select its own identity domain;
