...
14. The parent resolves and identity-binds the complete `scenario_ref` outside
   ...
   `expected_branch`, and `corrected_role` never enter a child workspace or
   serialized lifecycle request. After the complete event trace exists, the
   parent derives observed branch and corrected role before reading and
   comparing the two expectations. Changing the outer prompt or any scenario
   byte changes the input-set identity, but changing only an expectation can
   affect only that final comparison. Parent-observed sandbox audit is
