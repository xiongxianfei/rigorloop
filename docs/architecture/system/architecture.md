...
- reduce redundant post-planning implementation prompts through a separately armed, phase-gated `implementation-through-verify` profile that preserves reviewer-owned auto-fix authority, bounded correction loops, fresh verification evidence, and the human-controlled PR boundary.
- reduce repetitive proposal-side review-fix routing through a separately armed, bounded `review_fix` profile that preserves direct-review isolation, durable review evidence, driver-owned safe-fix classification, same-review reruns, and target-stage boundaries.
...
- `authoring-through-plan-review` is a separately armed workflow-managed profile. It is not a repository-wide default and cannot silently widen to `test-spec`, implementation, PR, release, deploy, merge, or review-fix behavior outside the separate `bounded-review-fix` profile.
- `implementation-through-verify` is a separate workflow-managed profile. It is not authorized by authoring-profile authorization, remains change-local, uses persisted phase gating, and must stop before PR or other external publication actions.
- `bounded-review-fix` is a separately armed workflow-managed profile persisted under `workflow.autoprogression.review_fix`. It is proposal-side only, uses the closed target stages `proposal-review`, `spec`, `spec-review`, `architecture`, `architecture-review`, `plan`, `plan-review`, `test-spec`, and `test-spec-review`, and cannot invoke implementation, code-review, verify, PR, release, publication, network, or other external-state operations.
- Review-fix activation requires durable user authorization and a clean current-stage gate. Proposal-start activation additionally requires an accepted proposal, approved recorded proposal-review, no open findings, and unambiguous change and artifact placement. Direct review invocations never create, activate, resume, or advance the profile, even when persisted review-fix state exists.
- Review-fix policy state records authorization, target, profile-local cursor, stop reason, and evidence pointers for audit and safe resume. It does not own active-plan live state, branch readiness, PR readiness, or review verdicts. Unknown, malformed, missing, contradictory, cancelled, off, completed, or target-reached transitions are resolved deterministically in `change.yaml` or the approved fallback policy surface.
- Review-fix auto-fix classification belongs to the workflow driver. The driver may apply only deterministic auto-safe findings after review evidence is recorded, must record disposition in `review-resolution.md`, must rerun the same review, and must stop on owner decisions, ambiguity, unsafe edits, stale evidence, generated-output ownership conflicts, or loop and edit budget exhaustion.
- After approved recorded `spec-review`, review-fix routing records exactly one architecture assessment value: `architecture-required`, `architecture-not-required`, or `architecture-ambiguous`. `architecture-required` routes through `architecture` and `architecture-review`; `architecture-not-required` skips those conditional stages and continues only to an applicable in-bound target; if the requested target is skipped, the driver stops with `target-not-applicable`; `architecture-ambiguous` stops for owner decision.
...
- Implementation-profile auto-fix authority belongs to code-review findings, not the orchestrator. The orchestrator enforces reviewer-declared classifications, affected paths, recipes, command boundaries, phase boundaries, shrinking loops, and audit records.
...
13. When `bounded-review-fix` is armed, activation proves durable authorization plus the clean gate for the current stage. The driver invokes existing authoring and review skills, records review evidence before mutation or continuation, classifies material findings itself, applies only deterministic auto-safe fixes within budget, records `review-resolution.md` disposition, reruns the same review, and proceeds only after the current review is clean and the target boundary allows the transition.
14. Direct review-only invocations remain isolated from `bounded-review-fix`: they do not create, activate, resume, or advance persisted review-fix state. Unknown or contradictory persisted review-fix state pauses before mutation or continuation.
15. After approved recorded `spec-review`, the review-fix driver records exactly one architecture assessment. `architecture-required` routes through `architecture` and `architecture-review`; `architecture-not-required` skips those conditional stages and continues only toward the next applicable in-bound target; `architecture-ambiguous` stops for owner decision. If the requested target is a skipped conditional architecture stage, the profile stops with `target-not-applicable`.
16. When `implementation-through-verify` is separately armed after clean planning, activation proves explicit ordered milestones, complete test-spec inputs, durable implementation-profile authorization, working-tree baseline, approved commands, clean governing artifacts, and state synchronization.
17. Phase A records audit-only transition decisions. Phase B may settle the test spec, execute milestones in order, run independent code-review, and apply only reviewer-classified mechanical or declared-safe corrections under bounded loop guardrails. Phase C may run `explain-change` and fresh actual-run `verify` after final full code-review.
18. Implementation-profile correction loops pause on unclassified findings, owner decisions, governing-artifact edit needs, non-shrinking rounds, new finding IDs or classes, unauthorized paths, scope expansion, command-boundary violations, round-cap exhaustion, stale settlement identities, verify failure, or external-boundary actions.
...
- review-fix profile evidence: nested `workflow.autoprogression.review_fix` authorization, target-stage, cursor, stop-reason, and update evidence in change-local policy metadata; it is audit and resume evidence for the bounded review-fix profile, not live plan state or review verdict authority;
- implementation-profile audit evidence: activation baseline, phase, milestone, review round, settlement identities, finding IDs, reviewer classifications, correction recipes, affected paths, actual paths changed, before/after unresolved-finding sets, commands, validation results, review-context reset evidence, final verify evidence, and PR-boundary stop record;
...
### Review-fix autoprogression policy

Review-fix autoprogression is opt-in, change-local workflow policy. The closed profile is `bounded-review-fix`, persisted only under `workflow.autoprogression.review_fix`, and the target-stage enum is limited to proposal-side lifecycle stages through `test-spec-review`.

The profile composes existing authoring and review skills instead of creating a new workflow engine. Review skills remain gates. The workflow driver may mutate artifacts only after review evidence is recorded, the finding is driver-classified as auto-safe, the edit stays within the current artifact or approved bookkeeping surface, and the same review can be rerun immediately.

The correction loop is bounded for convergence and reviewability: at most two auto-fix/rereview cycles per review gate, five material findings per cycle, three files changed per cycle, ten files changed per chat invocation, and no transition past the armed target stage. Budget exhaustion is a terminal or paused stop reason, not permission to partially hide unresolved work.

The profile does not have a dry-run or separate apply-mode state. Explicit arming is the authorization for auto-safe fixes. Direct review-only requests remain isolated and never create, activate, resume, or advance review-fix state.

The architecture assessment after clean `spec-review` is part of the profile boundary. It records one closed value, routes required architecture through `architecture` and `architecture-review`, skips non-required architecture stages, stops with `target-not-applicable` when the skipped conditional stage was the requested target, and pauses on ambiguity for owner decision.
...
### Implementation autoprogression policy boundary

`implementation-through-verify` is a repository workflow policy profile, not a new service, background worker, external scheduler, CLI deployment boundary, hosted PR actor, or release mechanism. It executes only inside an active workflow-managed interaction after clean planning and separate durable authorization.

The implementation-profile policy record is independent from authoring-profile authorization. It carries the profile name, state, phase, authorizing user, authorization timestamp, change ID, cancellation data, activation baseline, and audit pointers. It does not carry secrets, credentials, branch-readiness claims, PR-readiness claims, or active-plan live state.

Phase gating is part of the architecture boundary. Phase A records audit-only decisions and stop-condition simulations, Phase B runs through final clean code-review only, and Phase C may run `explain-change` and fresh `verify` only after promotion evidence is linked. The orchestrator refuses transitions outside the persisted phase.

Reviewer-owned material finding metadata is the authority for automatic corrections. Missing `auto_fix_class` means `none`; the orchestrator cannot infer auto-fix eligibility from severity, wording, file count, or apparent simplicity. `mechanical` fixes are restricted to closed deterministic kinds, while `declared-safe` fixes require a complete deterministic recipe and production-code proof when applicable.

The correction-loop architecture is convergence-first. Each automatic round is bounded per milestone, must shrink unresolved findings, must not introduce a new finding ID or class, must remain path-local, must avoid new scope and substantive governing-artifact edits, and must use approved commands. New findings pause even when the fresh reviewer may simply have been more careful, because new discovery requires human decision.

The verify and PR boundary stays explicit. Phase C verify records fresh actual-run evidence for correctness-bearing and release-sensitive checks. A verify failure pauses without automatic repair. A successful verify computes branch readiness from recorded evidence through workflow-state synchronization, reports `pr` next, and stops before hosted PR creation, branch push, package publication, deployment, remote review request, or other external effects.
...
- `docs/adr/ADR-20260624-implementation-through-verify-autoprogression.md`: closed `implementation-through-verify` profile, persisted rollout phases, reviewer-owned correction authority, fresh verify evidence, and stop-before-PR boundary.
...
- `docs/adr/ADR-20260630-bounded-review-fix-autoprogression.md`: closed `bounded-review-fix` profile, nested `workflow.autoprogression.review_fix` persistence, driver-owned auto-safe classification, bounded rereview loops, direct-review isolation, and architecture-assessment target routing.
...
ADR `docs/adr/ADR-20260624-implementation-through-verify-autoprogression.md` is required because this change introduces a durable implementation orchestration and review-authority decision: a separately armed implementation profile, phase-gated rollout, reviewer-owned auto-fix classification, bounded correction loops, fresh verify evidence, and a formal stop boundary before PR or other external effects.
...
ADR `docs/adr/ADR-20260630-bounded-review-fix-autoprogression.md` is required because this change introduces a durable workflow orchestration and persistence decision: a separately armed proposal-side review-fix profile, nested change-local policy state, driver-owned auto-safe classification, bounded same-review rereview loops, direct-review isolation, and architecture-assessment target routing.
...
| Implementation autoprogression safety | A user arms `auto-through: verify` after clean planning. | The workflow activates only after separate durable authorization and synchronized clean planning, runs only the persisted phase, accepts auto-fixes only from reviewer-owned classifications, requires shrinking path-local correction loops, uses fresh verify evidence, and stops before PR or other external effects. |
...
| Profile scope could creep into implementation | `authoring-through-plan-review` stops after clean `plan-review`; `bounded-review-fix` is separately armed and stops no later than `test-spec-review`; implementation, verification, PR, release, deploy, and external effects require separate profiles or spec amendments. |
| Implementation authorization could be implied by authoring authorization | `implementation-through-verify` has a separate change-local policy record, timestamp, authorizer, phase, and cancellation path; authoring-profile authorization never activates it. |
...
| Final verify could reuse stale evidence | Phase C verify requires fresh actual-run evidence for correctness-bearing, security-sensitive, release-sensitive, lifecycle, review closeout, change metadata, generated-output, and required test-suite checks. |
| Implementation autoprogression could cross the PR boundary | Successful Phase C completion reports `pr` next and records human authorization required; it does not open hosted PRs, push branches, publish packages, deploy, post to external systems, or request remote review. |
...
- implementation autoprogression profile: closed workflow policy value that authorizes bounded automatic test-spec settlement, milestone implementation, independent code-review, reviewer-declared corrections, explain-change, and fresh verify for one change according to persisted phase.
...
- profile phase: persisted implementation-profile rollout phase selected from `A`, `B`, or `C`.
...
- Architecture-review for the bounded review-fix autoprogression architecture update and ADR.
...
- Implementation Autoprogression Through Verify: accepted proposal and approved spec amendments define a separately armed `implementation-through-verify` profile, persisted phase gating, deterministic test-spec settlement, reviewer-owned auto-fix classification, bounded correction loops, fresh verify evidence, and stop-before-PR boundary.
