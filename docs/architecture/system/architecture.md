...
| Runtime projection registry and validator | Immutable constants and pure functions in `scripts/boundary_proof_model.py` | Projects complete reviewed runtime rows from the workflow spec, owns canonical projection identities and uniqueness checks, selects exactly one row by runtime implementation and declaration identities, validates the complete runtime-attestation observation set including effective-tool, file-change policy, handler-conformance, and diagnostic evidence, and returns only typed validated evidence for attestation assembly. It fails closed on unknown, duplicate, incomplete, or inconsistent values and performs no runtime I/O. |
