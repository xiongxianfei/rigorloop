...
5. In network mode, the CLI fetches official release metadata and the Codex adapter archive from GitHub Releases for `xiongxianfei/rigorloop`.
6. In local archive mode, `--from-archive <path>` verifies the local archive against bundled official adapter metadata shipped with the installed CLI package version.
...
| CLI init safety | A user runs `rigorloop init --adapter codex` in a project with existing files. | The CLI builds a write plan, refuses user-file overwrites by default, verifies release metadata and archive contents before extraction, writes no durable lockfile, and reports success/block/error through the stable command contract. |
...
- RigorLoop CLI Package and Codex Init: accepted proposal and approved spec define the first CLI slice: one package candidate, one `rigorloop` binary, help/version, `init --adapter codex`, dry-run JSON, non-destructive write planning, generated `rigorloop.yaml`, bundled metadata for local Codex archive verification, planned lockfile output only, and no public npm publication until release hardening.
