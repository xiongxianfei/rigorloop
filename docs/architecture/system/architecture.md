...
20. A derived `stop` from native `changes-requested` does not uniformly pause automation. The orchestrator may route to `review-resolution` only when an active workflow profile independently authorizes that route and the independence, evidence, and correction-loop gates pass; otherwise it pauses with a specific unroutable stop reason. Native `blocked` and `inconclusive` pause regardless of profile authorization.
21. Required or sampled second-review disagreement is not majority-voted away. A second reviewer material finding, blocked result, or inconclusive result prevents automatic continuation and routes to review-resolution, owner decision, or another authorized review.
22. The implementation profile requires a final holistic code review over the complete final diff, governing artifacts, review resolutions, validation selection, generated artifacts, and cross-milestone scope before `explain-change` and `verify`.
23. Final closeout outside the implementation profile runs `ci-maintenance` when triggered, then `explain-change`, `verify`, and `pr`.
24. `explain-change`, `verify`, and `pr` use the change-local evidence pack, plan state, validation output, and review closeout state before claiming readiness.
...
- Architecture-review for the independent adversarial review gates architecture update and ADR.
