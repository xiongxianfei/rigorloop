...
| Runtime projection registry and validator | Immutable constants and pure functions in `scripts/boundary_proof_model.py` | Projects complete reviewed runtime rows from the workflow spec, owns canonical projection identities and uniqueness checks, selects exactly one row by runtime implementation and declaration identities, validates the complete effective-tool projection and handler-conformance result shape, and fails closed on unknown, duplicate, incomplete, or inconsistent values. It performs no runtime I/O. |
