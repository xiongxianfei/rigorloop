...
16. Final closeout outside the implementation profile runs `ci-maintenance` when triggered, then `explain-change`, `verify`, and `pr`.
17. `explain-change`, `verify`, and `pr` use the change-local evidence pack, plan state, validation output, and review closeout state before claiming readiness.
...
- Architecture-review for the implementation autoprogression architecture update and ADR.
