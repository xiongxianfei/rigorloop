...
| Correction-authority classifier and stop recorder | Pure parsing plus bounded state functions in `scripts/boundary_proof_behavior.py` | Derives the exact stable-ID-sorted finding projection and identity from bound review-record bytes, computes the closed correction eligibility, permits attempt 2 only for `automatic-eligible`, and durably records `correction-authorization-required` before returning a non-retryable stop. It cannot reinterpret findings, synthesize owner consent, or publish a stopped run. |
| Invariant oracle evaluator | Pure projection and comparison functions in `scripts/boundary_proof_model.py`, coordinated by `scripts/boundary_proof_behavior.py` | Normalizes candidate and stage-authored feature/test-spec records independently, validates each complete R28s-R28w record, and compares only the closed version, scope, requirement, core-dimension, extension, and governing-requirement projection. After the event trace is complete, it independently derives the observed correction branch and corrected role and compares those observations with the scenario expectations; expectation values never enter lifecycle-stage requests or invocation selection. It returns `boundary-oracle-mismatch` for deterministic structural, invariant, or expectation failure and cannot compare stage-owned IDs, rationales, applicability choices, decomposition, examples, interactions, automation levels, proof grouping, or test-case IDs to a golden record. |
| Isolated workspace assembler | Functions in `scripts/boundary_proof_behavior.py` | Copies only manifest-bound skills, mapped resources, applicable instructions, and contracts into a fresh child-readable workspace. The parent retains the complete scenario record and candidate oracles outside child-readable roots; only the scenario's non-empty `request` value may enter the serialized lifecycle request, while the scenario path, bytes, `allowed_branches`, and `allowed_corrected_roles` remain parent-only. The assembler provisions a parent-owned behavior-output root; presents every scenario workspace root read-only to the child with no writable child root; permits writes only through the parent exact-byte materializer after the unchanged-workspace gate; and keeps the private runtime home and credentials outside sandbox-readable roots. |
| Immutable run publisher and reconciler | Functions in `scripts/boundary_proof_behavior.py` | Acquires the persistent publisher lock, validates global publication/recovery state, creates a run-bound publisher lease, builds a deterministic non-authoritative staged run, writes the durable exclusive receipt before installation or pointer mutation, and reconciles every interruption without reinvoking lifecycle skills. It treats a valid correction-stop receipt as working-orphan evidence, permits only explicit discard recovery, and rejects unchanged-input regeneration after that recovery. Completed recovery history is preserved but excluded from active candidacy. |
...
Successful v3 attestations are assembled only from the pure model's validated
success-evidence result and contain no diagnostic field.
Validated diagnostic decisions flow only to bounded failure responses and
failure evidence.
...
7. The parent adapter records runtime version and executable identity and
   ...
   report the exact model, model provider, active named permission profile,
   runtime workspace roots, instruction sources, working directory, and CLI
...
8. The named permission profile denies `:root`, allows only `:minimal`, the
   ...
   `PATH`. Runtime-owned `config/read`, `configRequirements/read`, fully
   ...
   The executable registry contains one complete immutable
   `codex-0.145.0-readonly-boundary-v1` projection. Its selection key binds
...
9. Before app-server output is accepted, the parent invokes the same identified
   ...
   Before either file-change capability branch, the parent installs the
   production deny-only dispatcher and freshly runs the complete
   invocation-owned handler-conformance policy against that same dispatch and
   response-validation implementation. Preflight and generation each perform
   this common gate independently.
   File-change enforcement then follows the selected projection.
   An `exposed-live-probe-required` projection runs a separate fresh
   app-server probe using the exact shared
   `stage-file-change-authorization-policy-v1`, requires one correlated
   request/decline/terminal-`declined` trace, and proves the workspace
   unchanged.
   ...
   is projection drift. Generic failure, mutation, missing or widened decline,
   prohibited acceptance, invalid conformance, or incomplete stop/reap fails
   before the canary.
   ...
   identity-bound on both paths. A transient random control-plane canary plus
   an exact spawned-command environment-name allowlist prove that credentials
   cannot enter command environment, argv, stdin, readable paths, or process
...
12. Generation creates a fresh isolated home and workspace, re-runs the
   ...
   fresh `boundary-runtime-attestation-v3` for the then-current exact eleven-row runtime
   ...
   selected projection and content identity, effective-tool projection,
   file-change authorization-policy identity, fresh handler-conformance
   identity, and closed capability state.
13. The generation attestation is a required member of
   the `boundary-behavior-implementation-v3`
   ...
   every unregistered v1 record fails closed. No durable v2 evidence was
   published; every v2-labeled record is unsupported historical evidence and
   cannot satisfy a current selector or be silently upgraded.
14. The parent resolves and identity-binds the complete `scenario_ref` outside
   child-readable roots, extracts only its non-empty `request`, and combines
   that value with the deterministic outer prompt. The scenario file, path,
   `allowed_branches`, and `allowed_corrected_roles` never enter a child
   workspace or serialized lifecycle request. After the complete event trace
   exists, the parent derives observed branch and corrected role before
   reading and comparing the two outcome-envelope expectations. Changing the
   outer prompt or any scenario byte changes the input-set identity, but
   changing only an expectation can affect only that final comparison.
   Parent-observed sandbox audit is
   transient enforcement state: the
...
15. Each lifecycle event records one or two exact transport-attempt rows.
    ...
    After an accepted formal review output, a separate correction-authority
    classifier derives the normalized finding projection from the exact
    review-record bytes, binds its identity into the review bundle, and
    computes `not-applicable`, `automatic-eligible`, or
    `owner-decision-required`. Only the automatic value may schedule authoring
    attempt 2. The owner-decision value writes and fsyncs the exact
    `correction-stop.json` receipt inside the lease-bound working root, returns
    the in-turn non-retryable stop, and performs no staging or publication.
...
17. Publication acquires the persistent `publisher.lock`, validates global
    ...
    A correction-stop working orphan is never adopted or automatically
    resumed. The existing explicitly authorized discard transaction
    quarantines it and completes lease removal. Before any later allocation or
    stage invocation, the publisher derives current inputs and rejects equality
    with the preserved stop receipt's input identity; only clarified unequal
    inputs may start a fresh run.
...
The v3 implementation manifest is the complete generation selector for harness
components, skill packages, instructions, contracts, invocation profile,
transport policy, lifecycle artifact policy, and fresh v3 runtime attestation.
The attestation binds the complete selected runtime projection, exact launcher
and package bytes, effective-tool projection, shared file-change authorization
policy, fresh handler conformance, capability-state-appropriate denial proof,
direct and descendant command denial, workspace integrity, canary
materialization, and credential-isolation results.
...
current authority or satisfy any v3 selector. No v2 record is registered;
v2-labeled data is unsupported history and cannot be upgraded.
...
The selected first-version runtime interface is Codex app-server over stdio,
not `codex exec` JSONL. The app-server `thread/start` response is the
...
Runtime-owned `config/read`, `configRequirements/read`, fully paginated
...
| Sandboxed file change | projection-negotiated, always deny-only | Both states first require fresh invocation-owned conformance against the installed production dispatcher. `exposed-live-probe-required` additionally requires a correlated live decline trace. `not-exposed-projection` additionally requires exact reviewed runtime bytes, disabled file-change-bearing features, the complete effective-tool projection, and rejection of any observed file-change event. Every governed request is still decline-only. |
...
| Browser, computer, image, web search | prohibited | Every browser/computer/image-generation/search feature is disabled and web search is `disabled`. |
| Realtime, remote control, and external environments | prohibited | Related feature rows are disabled and no environment is selected. |

The harness contains a closed version/schema-bound classification for every
row returned by `experimentalFeature/list`: permitted built-in tool,
permitted non-tool runtime behavior, or must-be-disabled tool-bearing
behavior. Every returned row must be classified; every required row must be
present; unknown or newly enabled rows return `environment-unavailable`.
The complete generated protocol vocabulary is identity-bound and every item
...
The parent does not infer enforcement from the response or event absence alone. It runs direct
...
managed requirements are identity-bound on both paths. A live app-server
file-change probe runs only when the selected projection declares it exposed.
Fresh handler conformance against the installed production dispatch is a
common pre-branch gate.
For a non-exposed projection, exact runtime bytes, the complete feature and
effective-tool projections, and the absence of contradictory observed events
join that common conformance evidence to establish the control.
...
unexpected instruction source or capability, successful forbidden probe, or
unstable executable, config, or schema identity fails closed.

Authentication isolation also uses a transient parent canary. The control
plane receives it, while the exact spawned-command environment-name allowlist,
argv/stdin probe, private-path denial, and unreadable process metadata prove it
cannot reach child commands. Only typed pass/fail results survive; no canary,
credential, environment value, raw config, managed requirement, or private
path is serialized.

The first supported Codex version is 0.138.0 because named permission profiles
and managed permission-profile selection are not supported by earlier
versions. The harness still validates the actual current CLI version and
generated app-server protocol fields rather than treating the version floor as
proof of capability.
...
Credentials are absent from child tool environment, the request-only scenario
projection,
...
No additional ADR is required for the boundary-proof correction-authority and scenario-expectation refinement because it projects the approved driver-owned correction classification, discard-only publication recovery, and post-observation proof comparison into the existing boundary harness. It adds no source of truth, resumable mid-run owner-decision mechanism, or new durable system boundary.
...
| Isolated child write denial | A governed child command, detached descendant, or app-server file-change operation attempts to mutate the scenario workspace. | Direct and descendant command writes fail, and both capability states first pass fresh conformance against the installed production dispatcher. An exposed capability additionally produces a correlated request/decline/terminal-`declined` probe. A reviewed non-exposed capability additionally matches exact runtime bytes plus disabled-feature and effective-tool projections; any observed file-change event is drift. Mutation, acceptance, missing correlation, invalid conformance, or incomplete cleanup stops before canary, materialization, or lifecycle publication. |
...
| Correction authority isolation | A formal upstream review returns `changes-requested`. | The classifier derives and identity-binds the complete finding projection; only all-eligible findings may reach authoring attempt 2. Any needs-decision finding durably records one correction stop, publishes nothing, requires explicit discard recovery, and rejects an unchanged-input fresh run. |
| Scenario expectation non-influence | A fixture declares an expected correction branch or corrected role. | The child receives only the authoritative scenario request. The parent derives branch and role from the completed event trace, compares those observations with the expectations after execution, and fails on mismatch without using either expectation to select a request, stage, retry, or correction. |
...
| Runtime evidence compatibility | Current selection encounters v1 or v2 runtime evidence. | Only the registry-listed exact v1 path and raw-byte identity are recognized as opaque read-only history. Every current preflight, generation, manifest, run, pointer, validation, report, capability, and activation role requires v3; unknown v1 and every v2 record fail closed. |
...
| A command descendant or app-server file-change path could bypass parent-only materialization | The child profile has no writable root; direct and detached-descendant command probes are always required. Fresh conformance against the installed deny-only production dispatcher is common to both capability states. File-change proof is then selected by an exact-runtime-bound immutable projection: exposed capability adds a live decline trace; non-exposure adds disabled-feature and effective-tool projections plus drift monitoring. The same handler governs canary, lifecycle, and retry turns. |
...
| Reused historical labels could select an incorrect parser or become current authority | Current roles require v3. The sole supported v1 record is recognized only by exact path, file kind, and raw-byte identity as opaque history; every other v1 input and all v2-labeled data fail closed. |
...
- runtime capability projection: an immutable typed compatibility row selected
  by exact runtime implementation, schema, protocol-classification, and
  feature-classification identities; it states whether file-change control
  requires a live decline probe or reviewed non-exposure proof.
- handler-conformance result: fresh invocation-owned value-free evidence that
  the installed file-change dispatcher declines one correctly bound request
  and fails closed for the complete malformed, mismatched, widened, accepted,
  and session-accepted case set.
...
- Codex permission-profile attestation: the conjunction of an app-server
  `thread/start` response bound to the expected named profile, model, roots,
  instructions, and CLI plus parent-run allow/deny probes through the same
  runtime and profile.
