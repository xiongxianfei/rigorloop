...
- Validation idempotency starts with `validate-artifact-lifecycle.py --mode explicit-paths` only. Cache hits require a previous pass, identical normalized command, identical input-surface hash, identical implementation manifest hash, and identical policy/config manifest hash.
...
| Lifecycle artifacts and ADRs | Carry proposal, spec, architecture, ADR, plan, test-spec, queryable change metadata, validation cache-hit evidence, validation cache measurement, and registered change-record evidence states | Markdown/YAML in `docs/proposals/`, `specs/`, `docs/architecture/`, `docs/adr/`, `docs/plans/`, `docs/changes/` |
...
- validation idempotency: cache helpers compute normalized argv, repository-relative explicit paths, input-surface hashes, implementation manifest hashes, and policy/config hashes for eligible `validate-artifact-lifecycle.py --mode explicit-paths` invocations; unsupported or uncertain manifests disable caching and run the validator;
...
1. A contributor or CI wrapper requests `python scripts/validate-artifact-lifecycle.py --mode explicit-paths ...`.
2. Cache eligibility checks confirm the command family is first-slice eligible and not a stage or milestone closeout full-bundle gate.
3. The cache helper normalizes the command as an ordered argv vector, normalizes `--path` values as repository-relative POSIX paths, rejects unsafe or duplicate explicit paths, and computes the command hash.
4. The cache helper computes the input-surface hash from every explicit path's content hash or missing-file marker.
5. The cache helper computes the validator implementation manifest hash from the entrypoint, resolved repository-local imports/helpers, and manifest-generation logic, and computes the policy/config manifest hash from declared lifecycle policy/spec/config files.
6. If a matching local execution cache entry exists for the same branch, worktree, change ID, command hash, input-surface hash, implementation hash, policy hash, and previous `pass`, the command may emit bounded `[CACHE HIT]` output and record formal cache-hit evidence when workflow evidence cites the skip.
7. If any cache component is missing, malformed, unsupported, changed, unsafe, non-local, expired, or non-passing, the validator actually runs and preserves existing pass/fail behavior and exit semantics.
8. Formal cache-hit evidence in `validation-cache-evidence.yaml` is reviewable proof that a prior pass still applies. It is not a new pass.
9. Closeout validation ignores cache hits as pass evidence. A closeout bundle is satisfied only by compact `schema_version: 2` validation events with `result: pass` and `evidence_kind: actual-run-pass`.
10. Workstream A measurement in `validation-cache-measurement.yaml` records eligible commands, cache hits, misses, disabled evaluations, actual runs, time saved, remaining cost, and Workstream B recommendation state.
...
Validation idempotency is a proof-preserving optimization layer inside validation execution. It may skip work only when the eligible validator's complete input surface, normalized command, implementation manifest, policy/config manifest, and previous passing result are unchanged. It does not change selector routing, selected check IDs, validator semantics, failure detection, exit codes, or closeout requirements.
...
Validation cache measurement lives under `docs/changes/<change-id>/validation-cache-measurement.yaml` for the implementing change. It records Workstream A cache-hit value and remaining validation cost. That measurement is the required evidence gate before any future edit-scoped validation proposal can argue that riskier scope narrowing is worth pursuing.
...
- Architecture-review for the Validation Idempotency and Cache-Hit Safety architecture update.
