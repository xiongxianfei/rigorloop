...
The file sink owns `rigorloop.jsonl`, `rigorloop.1.jsonl` through `rigorloop.4.jsonl`, and `.rigorloop-log.lock` beneath the selected root. Every append acquires the lock for at most 10 attempts and 1,000 milliseconds total. Under the lock, the sink revalidates the root and affected owned paths with non-following filesystem inspection immediately before each pathname mutation, rotates before an append that would cross 5 MiB, renames archives from highest to lowest, and publishes one complete candidate. It never steals or removes a stale or unverifiable lock; later attempts may continue to degrade until the user removes that lock. Contention, unsafe paths, permissions, disk failure, or malformed owned state degrade observability and preserve the semantic result.

Portable Node built-ins expose pathname-based `rename` and `unlink`, not a cross-platform directory-handle-relative conditional mutation API. The revalidation above therefore detects unsafe state at defined operation boundaries but is not an atomic authorization mechanism against another same-user or privileged process replacing a root or owned pathname after a successful check. That actor can also replace the CLI and retained logs and is outside the first-release local-diagnostics threat model. The implementation and documentation must not claim race-proof containment; a future requirement for that guarantee needs a separate architecture decision and native platform abstraction.
...
- Add native `openat`/`renameat`/`unlinkat` or platform-handle helpers: rejected for the first release because Node does not expose one portable built-in contract, native packaging would materially enlarge the support and security surface, and the approved local-diagnostics threat model does not include a same-user or privileged pathname-replacement adversary.
...
- Failed lock verification is safety-first: descriptors are closed, the unverifiable fixed lock is retained, and subsequent attempts degrade rather than risking removal of another process's lock.
- Path checks reject observed symlinks, replacements, and unsafe permissions, but cannot make a later pathname mutation atomic with that check. This is an explicit trust limitation, not a security guarantee.
