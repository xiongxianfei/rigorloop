...
No new findings. The five exact-path selector deferrals preserve mandatory direct proof, add no generic bypass, and keep the durable rationale current. The complete branch routes back to verify.
...
No new findings. R2 confirms WFSIM-CR1 through WFSIM-CR3 are resolved and closes implementation milestone M2.
