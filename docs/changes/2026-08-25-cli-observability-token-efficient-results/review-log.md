# Review Log: Local CLI Observability and Token-Efficient Results

## Scope

This ledger records formal review evidence for the governed local CLI observability and token-efficient results proposal. Review recording and settlement do not advance workflow routing.

## Clean review receipts

| Review ID | Stage | Round | Reviewed artifact | Record | Status | Material findings | Recording |
| --- | --- | --- | --- | --- | --- | ---: | --- |
| `architecture-review-r2` | `architecture-review` | `r2` | `docs/adr/ADR-20260825-local-cli-observability-and-result-projection-boundary.md` at `sha256:5e98900b19ff15a759dd59923c80d6a052281d345eec477d1814d82953a5a19e` | `reviews/architecture-review-r2.md` | `approved` | 0 | `recorded` |
| `spec-review-r3` | `spec-review` | `r3` | `specs/cli-observability-and-token-efficient-results.md` at `sha256:7693844003af6bd1b270d6dede9405c64b976afe838aaf4ab6444208710608ba` | `reviews/spec-review-r3.md` | `approved` | 0 | `recorded` |
| `spec-review-r2` | `spec-review` | `r2` | `specs/cli-observability-and-token-efficient-results.md` at `sha256:de9ec40c11d33b4d199e79fea74374199d94133c8eed651546ed04d664bc1029` | `reviews/spec-review-r2.md` | `approved` | 0 | `recorded` |
| `architecture-review-r1` | `architecture-review` | `r1` | canonical architecture and ADR-20260825 at recorded identities | `reviews/architecture-review-r1.md` | `approved` | 0 | `recorded` |
| `plan-review-r2` | `plan-review` | `r2` | `docs/plans/2026-08-25-cli-observability-token-efficient-results.md` at `sha256:fef931d84d84a7ba3b16a164f2dd16cdd37180b428098803d6eddc8cbc01fe0a` | `reviews/plan-review-r2.md` | `approved` | 0 | `recorded` |
| `plan-review-r3` | `plan-review` | `r3` | `docs/plans/2026-08-25-cli-observability-token-efficient-results.md` at `sha256:004a4aceadd1a4dcbb9ab5a4e4a1eca075cad4dd4fd84617d1972d476cb403a2` | `reviews/plan-review-r3.md` | `approved` | 0 | `recorded` |
| `test-spec-review-r2` | `test-spec-review` | `r2` | `specs/cli-observability-and-token-efficient-results.test.md` at `sha256:2c407aeff91b44a7ee39b8eaed162f46755483f75b4cb54379abaec86b319c73` | `reviews/test-spec-review-r2.md` | `approved` | 0 | `recorded` |
| `test-spec-review-r3` | `test-spec-review` | `r3` | `specs/cli-observability-and-token-efficient-results.test.md` at `sha256:4af3e383ffc9cc13522e0a71b953b3d3b3272ced34a9758a8335bd591e170622` | `reviews/test-spec-review-r3.md` | `approved` | 0 | `recorded` |
| `test-spec-review-r4` | `test-spec-review` | `r4` | `specs/cli-observability-and-token-efficient-results.test.md` at `sha256:8c509aeb9adf3f0b329f235fa729934210919fdbb93b24bb5d29e57d2af80e8a` | `reviews/test-spec-review-r4.md` | `approved` | 0 | `recorded` |
| `code-review-m1-r7` | `code-review` | `r7` | branch diff at `sha256:76ec33a33888a7dabbd44c958646d82a47797d320a18cc6bb007ca45515109a5` | `reviews/code-review-m1-r7.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m1-r10` | `code-review` | `r10` | branch diff at `sha256:cd54b37898a02b431a55a54151b5492c192d3f97d871e0e3d5b9d21bbe78216c` | `reviews/code-review-m1-r10.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m1-r12` | `code-review` | `r12` | branch diff at `sha256:76280fae6c040fe3b5a2e8e7b9fa83ffee58e1c403cbf599446973afaf451998` | `reviews/code-review-m1-r12.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m1-r14` | `code-review` | `r14` | branch diff at `sha256:90b1938660e967267a3a842646ca7ad81a3698c407d3b26212c8ecc037468bb4` | `reviews/code-review-m1-r14.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m1-r17` | `code-review` | `r17` | branch diff at `sha256:dabae974131766446abe7626d665ea1d57d2db09058147f026efe2e9b19bbbd2` | `reviews/code-review-m1-r17.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m1-r18` | `code-review` | `r18` | branch diff at `sha256:dabae974131766446abe7626d665ea1d57d2db09058147f026efe2e9b19bbbd2` | `reviews/code-review-m1-r18.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m2-r7` | `code-review` | `r7` | M2 implementation/test/evidence diff bundle at `sha256:a8ccc19505fef9e6243859dba71da3ecfd803575d43db2170b71922cb9c3d3f8` | `reviews/code-review-m2-r7.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m2-r9` | `code-review` | `r9` | M2 implementation/test/evidence diff bundle at `sha256:bcaca1334372260838357d8a4d3401886bfaa51a77e105de2fdd9b5453002190` | `reviews/code-review-m2-r9.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m2-r10` | `code-review` | `r10` | M2 implementation/test/evidence diff bundle at `sha256:bcaca1334372260838357d8a4d3401886bfaa51a77e105de2fdd9b5453002190` | `reviews/code-review-m2-r10.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m2-r12` | `code-review` | `r12` | M2 implementation/test/evidence diff bundle at `sha256:841c0e493c27f76981964a5a123b868846d86e8b9c716f03d9ba3f686d5bcfff` | `reviews/code-review-m2-r12.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m2-r14` | `code-review` | `r14` | M2 implementation/test/evidence diff bundle at `sha256:e6de92ecf6a84f9b20b05d28d0773d1b99b9c0c0060c4d882c05473e601c907f` | `reviews/code-review-m2-r14.md` | `clean-with-notes` | 0 | `recorded` |
| `code-review-m2-r15` | `code-review` | `r15` | M2 implementation/test/evidence diff bundle at `sha256:e6de92ecf6a84f9b20b05d28d0773d1b99b9c0c0060c4d882c05473e601c907f` | `reviews/code-review-m2-r15.md` | `clean-with-notes` | 0 | `recorded` |

## Detailed review entries

### Review entry

Review ID: code-review-m3-r1
Stage: code-review
Round: r1
Reviewed artifact path: M3 implementation/test/evidence bundle
Reviewed artifact identity: sha256:b2ef0a7fb4d2cf548d06e636145d361d58c4ca8455ded0d1862b97834df28303
Status: changes-requested
Detailed record: reviews/code-review-m3-r1.md
Resolution: review-resolution.md#code-review-m3-r1
Material findings: CLIOBS-M3-R1-F1, CLIOBS-M3-R1-F2, CLIOBS-M3-R1-F3, CLIOBS-M3-R1-F4
Finding ID: CLIOBS-M3-R1-F1
Finding ID: CLIOBS-M3-R1-F2
Finding ID: CLIOBS-M3-R1-F3
Finding ID: CLIOBS-M3-R1-F4
Open findings: CLIOBS-M3-R1-F1, CLIOBS-M3-R1-F2, CLIOBS-M3-R1-F3, CLIOBS-M3-R1-F4
Recording status: recorded

### Review entry

Review ID: code-review-m2-r13
Stage: code-review
Round: r13
Reviewed artifact path: M2 implementation/test/evidence diff bundle
Reviewed artifact identity: sha256:841c0e493c27f76981964a5a123b868846d86e8b9c716f03d9ba3f686d5bcfff
Status: changes-requested
Detailed record: reviews/code-review-m2-r13.md
Resolution: review-resolution.md#code-review-m2-r13
Material findings: CLIOBS-M2-R13-F1
Finding ID: CLIOBS-M2-R13-F1
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r11
Stage: code-review
Round: r11
Reviewed artifact path: M2 implementation/test/evidence diff bundle
Reviewed artifact identity: sha256:bcaca1334372260838357d8a4d3401886bfaa51a77e105de2fdd9b5453002190
Status: changes-requested
Detailed record: reviews/code-review-m2-r11.md
Resolution: review-resolution.md#code-review-m2-r11
Material findings: CLIOBS-M2-R11-F1
Finding ID: CLIOBS-M2-R11-F1
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r8
Stage: code-review
Round: r8
Reviewed artifact path: M2 implementation/test/evidence diff bundle
Reviewed artifact identity: sha256:a8ccc19505fef9e6243859dba71da3ecfd803575d43db2170b71922cb9c3d3f8
Status: changes-requested
Detailed record: reviews/code-review-m2-r8.md
Resolution: review-resolution.md#code-review-m2-r8
Material findings: CLIOBS-M2-R8-F1
Finding ID: CLIOBS-M2-R8-F1
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r6
Stage: code-review
Round: r6
Reviewed artifact path: M2 implementation/test/evidence diff bundle
Reviewed artifact identity: sha256:a8ccc19505fef9e6243859dba71da3ecfd803575d43db2170b71922cb9c3d3f8
Status: inconclusive
Detailed record: reviews/code-review-m2-r6.md
Resolution: review-resolution.md#code-review-m2-r6
Material findings: none
Finding ID: none
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r5
Stage: code-review
Round: r5
Reviewed artifact path: M2 implementation/test diff bundle
Reviewed artifact identity: sha256:0d3dd7524ac022763f7284e8892161c9a15ed2e62e9b7edd58b7d4c95c76ba35
Status: changes-requested
Detailed record: reviews/code-review-m2-r5.md
Resolution: review-resolution.md#code-review-m2-r5
Material findings: CLIOBS-M2-R5-F1
Finding ID: CLIOBS-M2-R5-F1
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r4
Stage: code-review
Round: r4
Reviewed artifact path: M2 implementation/test diff bundle
Reviewed artifact identity: sha256:a44b241a35c3c12082d5a47e7c71e61ac8893db624f778a271860d1e3168ba98
Status: changes-requested
Detailed record: reviews/code-review-m2-r4.md
Resolution: review-resolution.md#code-review-m2-r4
Material findings: CLIOBS-M2-R4-F1, CLIOBS-M2-R4-F2, CLIOBS-M2-R4-F3, CLIOBS-M2-R4-F4
Finding ID: CLIOBS-M2-R4-F1, CLIOBS-M2-R4-F2, CLIOBS-M2-R4-F3, CLIOBS-M2-R4-F4
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r3
Stage: code-review
Round: r3
Reviewed artifact path: M2 implementation/test diff bundle
Reviewed artifact identity: sha256:c762477f97884546a4d41575bd0104579f179ca3634787f7478b500720501024
Status: changes-requested
Detailed record: reviews/code-review-m2-r3.md
Resolution: review-resolution.md#code-review-m2-r3
Material findings: CLIOBS-M2-R3-F1, CLIOBS-M2-R3-F2, CLIOBS-M2-R3-F3, CLIOBS-M2-R3-F4
Finding ID: CLIOBS-M2-R3-F1, CLIOBS-M2-R3-F2, CLIOBS-M2-R3-F3, CLIOBS-M2-R3-F4
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r1
Stage: code-review
Round: r1
Reviewed artifact path: packages/rigorloop/dist/lib/log-sink.js
Reviewed artifact identity: sha256:7fc64f763c3121af4f473875cab16c8fc1a739b082f88f00f801412700d29b38
Status: changes-requested
Detailed record: reviews/code-review-m2-r1.md
Resolution: review-resolution.md#code-review-m2-r1
Material findings: CLIOBS-M2-L1-F1, CLIOBS-M2-L1-F2, CLIOBS-M2-L1-F3, CLIOBS-M2-L1-F4
Finding ID: CLIOBS-M2-L1-F1, CLIOBS-M2-L1-F2, CLIOBS-M2-L1-F3, CLIOBS-M2-L1-F4
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m2-r2
Stage: code-review
Round: r2
Reviewed artifact path: packages/rigorloop/dist/lib/log-sink.js
Reviewed artifact identity: sha256:7fc64f763c3121af4f473875cab16c8fc1a739b082f88f00f801412700d29b38
Status: changes-requested
Detailed record: reviews/code-review-m2-r2.md
Resolution: review-resolution.md#code-review-m2-r2
Material findings: M2-L1B-F1, M2-L1B-F2, M2-L1B-F3, M2-L1B-F4
Finding ID: M2-L1B-F1, M2-L1B-F2, M2-L1B-F3, M2-L1B-F4
Open findings: none
Recording status: recorded


### Review entry

Review ID: code-review-m1-r16
Stage: code-review
Round: r16
Reviewed artifact path: packages/rigorloop/dist/bin/rigorloop.js
Reviewed artifact identity: sha256:8e1cad39aa8ec5117544fcf70a16e20123e4a4ab6e83b7215bb936415d3bd8dc
Status: changes-requested
Detailed record: reviews/code-review-m1-r16.md
Resolution: review-resolution.md#code-review-m1-r16
Material findings: CLIOBS-M1-L1-F12
Finding ID: CLIOBS-M1-L1-F12
Open findings: none
Recording status: recorded


### Review entry

Review ID: code-review-m1-r15
Stage: code-review
Round: r15
Reviewed artifact path: packages/rigorloop/dist/lib/result-renderer.js
Reviewed artifact identity: sha256:583f76773a7f1451c3644d9cf968e00ab0bf613da2189edd49856b72a804e870
Status: changes-requested
Detailed record: reviews/code-review-m1-r15.md
Resolution: review-resolution.md#code-review-m1-r15
Material findings: CLIOBS-M1-L1-F11
Finding ID: CLIOBS-M1-L1-F11
Open findings: none
Recording status: recorded


### Review entry

Review ID: code-review-m1-r13
Stage: code-review
Round: r13
Reviewed artifact path: packages/rigorloop/dist/lib/result-renderer.js
Reviewed artifact identity: sha256:7e131ea61620ab75ead4dc840f0947a931866b46e7bfe9b76ae3eb3b3176ef57
Status: changes-requested
Detailed record: reviews/code-review-m1-r13.md
Resolution: review-resolution.md#code-review-m1-r13
Material findings: CLIOBS-M1-L1-F10
Finding ID: CLIOBS-M1-L1-F10
Open findings: none
Recording status: recorded


### Review entry

Review ID: code-review-m1-r11
Stage: code-review
Round: r11
Reviewed artifact path: packages/rigorloop/dist/lib/lifecycle-cli.js
Reviewed artifact identity: sha256:e908021bd498242c3773e05ccd0a6613e153a8738553454ed3685101609e326c
Status: changes-requested
Detailed record: reviews/code-review-m1-r11.md
Resolution: review-resolution.md#code-review-m1-r11
Material findings: CLIOBS-M1-L1-F9
Finding ID: CLIOBS-M1-L1-F9
Open findings: none
Recording status: recorded


### Review entry

Review ID: code-review-m1-r8
Stage: code-review
Round: r8
Reviewed artifact path: branch diff against `origin/main`
Reviewed artifact identity: sha256:76ec33a33888a7dabbd44c958646d82a47797d320a18cc6bb007ca45515109a5
Status: changes-requested
Detailed record: reviews/code-review-m1-r8.md
Resolution: review-resolution.md#code-review-m1-r8
Material findings: CLIOBS-M1-L1-F7
Finding ID: CLIOBS-M1-L1-F7
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m1-r6
Stage: code-review
Round: r6
Reviewed artifact path: branch diff against `origin/main`
Reviewed artifact identity: sha256:bb30d63147c2cc41d7ac7f78454de87d7f7609ddd86304baebdb36654416fc2d
Status: changes-requested
Detailed record: reviews/code-review-m1-r6.md
Resolution: review-resolution.md#code-review-m1-r6
Material findings: CLIOBS-M1-L1-F6
Finding ID: CLIOBS-M1-L1-F6
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m1-r5
Stage: code-review
Round: r5
Reviewed artifact path: branch diff against `origin/main`
Reviewed artifact identity: sha256:e459f182874a47ad9f14af61da6d483b36c45923820f3013560f126ceb4240ce
Status: changes-requested
Detailed record: reviews/code-review-m1-r5.md
Resolution: review-resolution.md#code-review-m1-r5
Material findings: CLIOBS-M1-L1-F5
Finding ID: CLIOBS-M1-L1-F5
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m1-r4
Stage: code-review
Round: r4
Reviewed artifact path: branch diff against `origin/main`
Reviewed artifact identity: sha256:5675b1a983f5b2bc6c1b9f7efbfd3521c924779bb6f0fe988f9f0dfefca4859e
Status: changes-requested
Detailed record: reviews/code-review-m1-r4.md
Resolution: review-resolution.md#code-review-m1-r4
Material findings: CLIOBS-M1-L1-F1, CLIOBS-M1-L1-F2, CLIOBS-M1-L1-F3, CLIOBS-M1-L1-F4
Finding ID: CLIOBS-M1-L1-F1
Finding ID: CLIOBS-M1-L1-F2
Finding ID: CLIOBS-M1-L1-F3
Finding ID: CLIOBS-M1-L1-F4
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m1-r3
Stage: code-review
Round: r3
Reviewed artifact path: branch diff against `origin/main`
Reviewed artifact identity: sha256:bf617a9ddd1acbfaa3b1ffaf72cd50e7829711bca6c1340535c33b9b9fab6bf0
Status: inconclusive
Detailed record: reviews/code-review-m1-r3.md
Resolution: review-resolution.md#code-review-m1-r3
Material findings: none
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m1-r2
Stage: code-review
Round: r2
Reviewed artifact path: branch diff against `origin/main`
Reviewed artifact identity: sha256:a30866ff83c85ebe0eb0425c203cb2b401386ba99266d23daa8a0b983e7380e4
Status: changes-requested
Detailed record: reviews/code-review-m1-r2.md
Resolution: review-resolution.md#code-review-m1-r2
Material findings: CLIOBS-M1-CR7, CLIOBS-M1-CR8, CLIOBS-M1-CR9
Finding ID: CLIOBS-M1-CR7
Finding ID: CLIOBS-M1-CR8
Finding ID: CLIOBS-M1-CR9
Open findings: none
Recording status: recorded

### Review entry

Review ID: test-spec-review-r1
Stage: test-spec-review
Round: r1
Reviewed artifact path: specs/cli-observability-and-token-efficient-results.test.md
Reviewed artifact identity: sha256:1d4d6dfb266b986ba9e66a0fa44816a77e26e02e5b721524737621c88f98e86a
Status: changes-requested
Detailed record: reviews/test-spec-review-r1.md
Resolution: review-resolution.md#test-spec-review-r1
Material findings: CLIOBS-TSR1, CLIOBS-TSR2
Finding ID: CLIOBS-TSR1
Finding ID: CLIOBS-TSR2
Open findings: none
Recording status: recorded

### Review entry

Review ID: code-review-m1-r1
Stage: code-review
Round: r1
Reviewed artifact path: branch diff `bcc7ef14ae45e8df737d8a97e72eff3a3823446b..ca112ac6028ad265cbf111f99ad2d4618b6a72b6`
Reviewed artifact identity: sha256:7fdc8b445397922feba03da3f5ff6aefffd01d38a6d137a6ed78d366c64a6d7d
Status: changes-requested
Detailed record: reviews/code-review-m1-r1.md
Resolution: review-resolution.md#code-review-m1-r1
Material findings: CLIOBS-M1-CR1, CLIOBS-M1-CR2, CLIOBS-M1-CR3, CLIOBS-M1-CR4, CLIOBS-M1-CR5, CLIOBS-M1-CR6
Finding ID: CLIOBS-M1-CR1
Finding ID: CLIOBS-M1-CR2
Finding ID: CLIOBS-M1-CR3
Finding ID: CLIOBS-M1-CR4
Finding ID: CLIOBS-M1-CR5
Finding ID: CLIOBS-M1-CR6
Open findings: none
Recording status: recorded

### Review entry

Review ID: proposal-review-r1
Stage: proposal-review
Round: r1
Reviewed artifact path: docs/proposals/2026-08-25-cli-observability-and-token-efficient-results.md
Reviewed artifact identity: sha256:c33ad29a273199cf5a5d8065f3415e71dfbe449764f7535fea91806beecc61ea
Status: changes-requested
Detailed record: reviews/proposal-review-r1.md
Resolution: review-resolution.md#proposal-review-r1
Material findings: CLIOBS-PR6, CLIOBS-PR7, CLIOBS-PR8
Open findings: none
Recording status: recorded

### Review entry

Review ID: plan-review-r1
Stage: plan-review
Round: r1
Reviewed artifact path: docs/plans/2026-08-25-cli-observability-token-efficient-results.md
Reviewed artifact identity: sha256:3cd6a1937d40b653244eb72deb3e8b277872e011a626d63e1d502a08ed8cc6ff
Status: changes-requested
Detailed record: reviews/plan-review-r1.md
Resolution: review-resolution.md#plan-review-r1
Material findings: CLIOBS-PLR1, CLIOBS-PLR2
Open findings: none
Recording status: recorded

### Review entry

Review ID: proposal-review-r2
Stage: proposal-review
Round: r2
Reviewed artifact path: docs/proposals/2026-08-25-cli-observability-and-token-efficient-results.md
Reviewed artifact identity: sha256:70652f4e5afb34bb3272f73be339a43661a14ef6d4c69bf02585091a6592f47c
Status: approved
Detailed record: reviews/proposal-review-r2.md
Resolution: review-resolution.md#proposal-review-r2
Material findings: none
Open findings: none
Recording status: recorded

### Review entry

Review ID: proposal-review-r3
Stage: proposal-review
Round: r3
Reviewed artifact path: docs/proposals/2026-08-25-cli-observability-and-token-efficient-results.md
Reviewed artifact identity: sha256:5b087daae14c102670a2aab515fa5fd402bfe0202ff7096e781c81a1110cbfe0
Status: approved
Detailed record: reviews/proposal-review-r3.md
Resolution: review-resolution.md#proposal-review-r3
Material findings: none
Open findings: none
Recording status: recorded

### Review entry

Review ID: proposal-review-r4
Stage: proposal-review
Round: r4
Reviewed artifact path: docs/proposals/2026-08-25-cli-observability-and-token-efficient-results.md
Reviewed artifact identity: sha256:bda70adc597cc43f9799c34db57010c2fb626e1eea4abdeada88ea716d8429ae
Status: approved
Detailed record: reviews/proposal-review-r4.md
Resolution: review-resolution.md#proposal-review-r4
Material findings: none
Open findings: none
Recording status: recorded

### Review entry

Review ID: spec-review-r1
Stage: spec-review
Round: r1
Reviewed artifact path: specs/cli-observability-and-token-efficient-results.md
Reviewed artifact identity: sha256:60a0fc29fd429e56673d87b32bccd6d2411dbfaba7f2bf5f5d6b02f38e6e612a
Status: changes-requested
Detailed record: reviews/spec-review-r1.md
Resolution: review-resolution.md#spec-review-r1
Material findings: CLIOBS-SR1, CLIOBS-SR2, CLIOBS-SR3
Open findings: none
Recording status: recorded
### Review entry

Review ID: code-review-m1-r9
Stage: code-review
Round: r9
Reviewed artifact path: packages/rigorloop/dist/lib/lifecycle-cli.js
Reviewed artifact identity: sha256:1ce482bf439da26133aa378328087d67d41a75ae47f1fefbaef0556ca9c899f7
Status: changes-requested
Detailed record: reviews/code-review-m1-r9.md
Resolution: review-resolution.md#code-review-m1-r9
Material findings: CLIOBS-M1-L1-F8
Finding ID: CLIOBS-M1-L1-F8
Open findings: none
Recording status: recorded
