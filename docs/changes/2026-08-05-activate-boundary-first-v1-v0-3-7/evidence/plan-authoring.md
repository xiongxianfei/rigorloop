...
narrow activation transition T. It separates candidate-R, evidence-C,
readiness-H, strict-H, detached-T,
...
The R4 revision aligns execution with the approved non-circular identity model.
M1 emits `P/B/T/R`; M4 commits the exact result in immediate child `C` before
later lifecycle evidence settles `H`; and M2 publication mode reruns readiness,
retains exact full `H` in memory, and uses that same SHA in the atomic refspec.
