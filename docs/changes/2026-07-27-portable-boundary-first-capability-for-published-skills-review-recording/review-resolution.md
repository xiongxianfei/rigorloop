...
- Findings resolved: 64
- Unresolved findings: 0
- Final result: PBF-M4-CR1 and PBF-M4-CR2 are resolved by authoritative active validation, CLI output, and direct containment/non-mutation proof.

## Resolution Overview

| Finding ID | Disposition | Status | Resolution summary |
| --- | --- | --- | --- |
| PBF-M4-CR1 | accepted | resolved | Bind rollback selection to the fixed validated activation manifest and production CLI. |
| PBF-M4-CR2 | accepted | resolved | Prove authoritative-path containment and complete non-mutation. |
...
Validation evidence: Active CLI success and missing-metadata failure tests pass; pending validation remains independent.
...
paths.
Validation target: code-review M4 R2.
Validation evidence: Six missing/non-regular/symlink path cases preserve the complete fixture tree and outside sentinel.
...
#### PBF-M3-CR18 - Baseline is not bound to the activation transition parent
...
#### PBF-TSR3 - Parent-revision activation proof is not executable
...
manifest, inventory, exclusion, uniqueness, and ordering assertions.
...
before-and-after governed-file bytes, and a no-action sentinel.
...
No material findings.
R3 confirms PBF-PLR1 through PBF-PLR3 resolved and approves test-spec revision.
...
Validation evidence: The governing artifact list names the accepted
ADR-20260728 decision.
...
Validation target: Plan-review R3.
Validation evidence: M3 owns isolated schema fixtures; M4 owns an exact
active-release integration test and the sequencing table records that proof.
...
Validation target: Plan-review R3.
Validation evidence: M4 recovery retains or restores pending repository bytes,
excludes external operations, and uses a release-readiness commit identity.
