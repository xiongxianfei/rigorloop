...
| RTF-CR8 | accepted | resolved | Added release-level integration coverage proving invalid governed token-cost metadata blocks through token-cost validator delegation. |
...
Status: resolved
Owner: implementer
Owning stage: implement M5
Chosen action: Added release-level integration coverage that forces governed `v0.1.1` release validation through a failing token-cost validator path for invalid token-cost metadata.
Rationale: The approved T16 test spec requires proof that governed invalid token-cost evidence blocks release validation, not only proof that missing token-cost metadata blocks and valid repository metadata passes.
Validation target: `validate_release_output("v0.1.1", ...)` or `scripts/validate-release.py --version v0.1.1` rejects invalid token-cost report metadata through `scripts/validate-token-cost-report.py` delegation.
Validation evidence: `python scripts/test-adapter-distribution.py`; `python scripts/test-token-cost-report-validation.py`; `python scripts/validate-release.py --version v0.1.1`; `python -m py_compile scripts/validate-release.py scripts/validate-token-cost-report.py scripts/adapter_distribution.py`.
