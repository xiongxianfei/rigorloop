...
| Selector and architecture ownership support | Recorded eight exact owner-approved deferrals for one-change ledgers and fixtures, then retired four stale prior `artifact_states` claims on the canonical architecture path. | Keep registration debt visible without permanent simplicity routing and preserve exactly one current architecture owner. | Existing owner-deferral and single-owner lifecycle contracts; T7-T9, CMD7, MP1, PR lifecycle validation |
...
The governed profile reduction is intentionally modest because deterministic identity and failure procedure remain explicit. Active legacy plans with incomplete `planned_work` still require explicit workflow-owned migration rather than automatic repair. The first final PR-mode selector correctly stopped on eight unsupported one-change evidence paths. Exact repository-maintainer deferrals preserve T7-T9, CMD7, and MP1 and change no selector or check catalog. The first PR gate then exposed four stale historical ownership entries for the canonical architecture path; those entries were retired while their durable review evidence remained intact, leaving this architecture-changing `change.yaml` as the single current owner. Final verification and hosted CI remain downstream gates; this explanation does not claim branch readiness or PR readiness.
