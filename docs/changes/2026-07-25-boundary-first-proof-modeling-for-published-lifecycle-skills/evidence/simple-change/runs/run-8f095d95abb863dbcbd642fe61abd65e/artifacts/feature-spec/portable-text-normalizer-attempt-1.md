...
Then the returned text excludes that leading and trailing Unicode whitespace.
...
edge.trim.empty: In `trim` mode, text with no non-whitespace content yields the result implied by removing its leading and trailing Unicode whitespace under R2.
...
| criterion.trim.output | R2 | Leading and trailing Unicode whitespace is absent from the returned text in `trim` mode. |
