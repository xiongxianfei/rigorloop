...
### T1. Trim boundary Unicode whitespace and preserve interior text

- Covers: R1, R2, example.trim-whitespace, EC1, EC2
- Level: unit
- Command IDs: none
- Fixture/setup: Use the complete, deterministically enumerated set of code points having the Unicode `White_Space` property. For every code point in that set, construct cases placing it at each boundary, a whitespace-only case, and a retained-text case placing it between non-whitespace code points.
- Steps: Normalize every constructed input using mode `trim`.
- Expected result: Every leading and trailing Unicode `White_Space` code point is removed; every Unicode whitespace code point between retained non-whitespace code points remains unchanged; whitespace-only text returns empty text.
- Failure proves: The accepted `trim` mode does not remove the complete Unicode `White_Space` boundary set, removes interior whitespace, or otherwise violates R1 or R2.
