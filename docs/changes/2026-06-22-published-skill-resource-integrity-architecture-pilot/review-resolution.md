...
Owner: implementer
Owning stage: implement
Chosen action: Preserve recorded-source release compatibility without dropping release metadata and release-surface validation. The resolution should skip only current canonical skill or archive-content checks that are invalid for historical source while retaining release metadata, release notes, required validation evidence, security, npm-publication, and adapter artifact metadata checks that still apply to the recorded release.
