...
- `UBR-R021` makes lifecycle authority select one canonical boundary marker placement: stage-owned specs use the owning-change section, while historical specs without that authority retain status placement.
...
| PR-gate corrections | `boundary_first_validation.py`, `validation_selection.py`, boundary/selector tests, and CLI package fixtures now normalize lifecycle authority, reject malformed release profile paths, route the canonical research artifact, and preserve historical `v0.3.3` fixture identity. | Resolve integration assumptions exposed only by the complete PR diff while preserving the approved release and compatibility contract. | `UBR-R021`, `T24`, `EC11`; PR full-gate R1-R5 and PR-readiness R1 |
...
| Lifecycle marker authority (`T24`) | Exact quoted/unquoted and repository-spaced stage authority selects owner placement; absence retains historical status; unknown, malformed, and duplicate values fail before placement consistency. | Path-aware integration fixtures exercise the feature spec and its owning change record together. |
...
| --- | --- |
| `python scripts/test-boundary-first-validation.py` | 65 passed after lifecycle-authority corrections |
| `python scripts/validate-boundary-first.py --check` | active snapshot; `v0.4.0` intent; `v0.3.6` rollback |
| `python scripts/test-select-validation.py` | 150 passed after selector and release-profile corrections |
| `npm test --prefix packages/rigorloop` | 117 passed after current/historical fixture identity separation |
...
| Review-artifact and change-metadata validation after PR full-gate R5 | passed; 36 reviews, 35 resolved findings, no open findings |
...
The durable [review resolution](review-resolution.md) closes 35 material findings, all accepted and resolved, with no `needs-decision` or open review-log entry.
Every milestone ended in a clean independent review.
PR full-gate R5 additionally closes the lifecycle-authority parser corrections after four finding rounds, including reciprocal marker placement, closed-vocabulary scalar handling, and repository-style mapping-key normalization.
...
- PR-mode preflight exposed one selector omission for the canonical research artifact produced during release framing. The focused [PR-readiness bug-fix evidence](evidence/pr-readiness-research-selector-bugfix.md) records the failing reproduction, test-first correction, and scoped routing result. This does not change the release contract; it makes the existing research output path participate in its owned documentation checks. Independent PR-readiness code review R1 approved the correction with no material findings and confirmed that unsupported and mixed unknown paths remain fail-closed.
- The first complete PR-mode execution then exposed four stale integration assumptions after the release version and activation model changed. The [full-gate fix evidence](evidence/pr-readiness-full-gate-fixes.md) records the boundary-record normalization, single architecture owner, exact release profile parsing, and `v0.4.0` CLI fixture update. Broad smoke passed in that run; only the four named diff-scoped checks required correction.
- Independent PR full-gate reviews then found and closed six material findings across five themes: marker-placement authority, malformed profile routing, historical fixture identity, reciprocal status placement, and fail-closed lifecycle scalar/key parsing. The final R5 review is clean-with-notes and the [resolution evidence](evidence/pr-full-gate-review-resolution.md) records the test-first correction sequence.
...
The release implementation and all PR-gate corrections are independently reviewed.
Final verification must now refresh branch readiness; none of this authorizes tagging, publication, merge, or a public-release claim.
