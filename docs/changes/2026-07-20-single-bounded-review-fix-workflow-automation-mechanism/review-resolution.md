...
- Current result: Code-review M6 R5 classified `BRF-M6-CR10` as a failed remediation and opened residual `BRF-M6-CR11`. Replacement proposal-review authority still trusts the new recovery invocation's unbound `derived_at`. M6 is resolution-needed.
...
| BRF-M6-CR11 | needs-decision | open | Replacement proposal-review authority changes when only the fresh recovery invocation's unbound `derived_at` changes. |
...
| BRF-M4-CR7 | accepted | resolved | Canonical correction recipes compile to closed exact operations that alone govern the proposal bytes written. |
...
| BRF-M5-CR1 | accepted | resolved | Integrated closed reviewer-owned implementation correction through exact capabilities, prepared receipts, bounded paths, SHA-256 validation, convergence, rollback, and same-milestone rereview. |
| BRF-M5-CR2 | accepted | resolved | Verification readiness now rehashes and semantically validates the closed plan, latest final review, explanation, promotion, branch, and command evidence instead of accepting caller booleans. |
...
#### BRF-M6-CR1 - Cross-spec validation has no independent closed selector registry
...
R3 confirmed `BRF-M6-CR5` and `BRF-M6-CR6` resolved.
It classified `BRF-M6-CR7` as a failed remediation and recorded the residual findings below.
...
R4 confirmed `BRF-M6-CR8` resolved.
It classified `BRF-M6-CR9` as a failed remediation and recorded the residual finding below.
...
R5 classified `BRF-M6-CR10` as a failed remediation and recorded the residual finding below.
...
Owning stage: review-resolution
Decision owner: implementation author
Decision needed: Accept or reject the recorded durable-timestamp remediation before implementation changes begin.
...
Rationale: Proposal bytes and affected paths are now restart-safe, but the fresh capability still uses `coordination.derived_at`, which is neither compared with the persisted correction capability nor transition-key-bound.
Required outcome: Every fresh capability field must be derived from durable transaction evidence, and recovery-time timestamp substitution must not change canonical state.
Safe resolution path: Reuse the original correction capability's validated `derived_at` or another approved bound source, reject or ignore unbound recovery timestamps, and add altered/omitted timestamp recovery contrasts with canonical-state equality assertions.
...
Validation evidence: A direct fresh-process-style reproduction changed only recovery `derived_at` from `2026-07-22T00:01:00Z` to `2099-01-01T00:00:00Z`; the same receipt completed and the fresh proposal-review capability persisted the `2099` value. Both existing focused recovery tests still pass.
auto_fix_class: none
...
#### BRF-M5-CR14 - Exact-command guard accepts equality-spoofing tuple subclasses
...
Rationale: `command != expected_command` runs before the `isinstance` check, and tuple subclasses remain accepted. A tuple subclass can spoof equality while its actual elements name a broader Git operation.
Required outcome: Reject every command that is not an exact built-in tuple before equality evaluation, then compare only the built-in tuple with the canonical-root probe.
...
Validation evidence: The proof-first exact-allowlist regression failed before the helper existed and now passes. Twelve code-state tests, six verify-selected tests, 58 full automation-engine tests, six implementation-selected tests, fourteen correction-selected tests, five milestone-selected tests, five non-public-selected tests, 104 review-artifact tests, 156 lifecycle tests, and 259 skill-validator tests pass. Final synchronized lifecycle and broad-smoke evidence is recorded in the active plan and change metadata.
auto_fix_class: none

### code-review-m5-r5

#### BRF-M5-CR12 - Canonical anchor remains substitutable
...
Decision owner: implementation author
Decision needed: none
Rationale: The resolver returns an injected provider before Git membership detection when `<root>/.git` is absent, even when Git identifies that root as inside a worktree. It also resolves the final-review commit from arbitrary Git revision syntax, so unchanged review evidence such as `Reviewed commit: HEAD` can retarget newer code.
Required outcome: Determine Git membership and canonical repository-root ownership before provider selection, and require the final-review record to name the exact immutable commit object identity used by the anchor.
...
Validation evidence: `GitCodeStateAnchorResolver` discovers the repository-owned default target ref, derives its merge base with the exact commit named by canonical final-review evidence, and binds change/review/target/evidence paths into an immutable anchor. `GitCodeStateProvider` consumes only that anchor and rejects target-ref drift. `resolve_canonical_code_state` rejects test-provider substitution in Git repositories. Branch evidence must project the anchor identity. The 7 provider regressions, 56 engine tests, 16 policy tests, 60 state tests, 65 automation-validator tests, Python compilation, direct tracked-branch merge-base proof, and diff checks pass. The plan-required repository broad-smoke suite passes all 11 checks in 402 seconds on the final synchronized M5 R4 resolution source.
auto_fix_class: none
...
Validation evidence: `scripts/workflow_code_state.py` derives added, modified, deleted, and renamed paths from trusted Git revisions and rejects unapproved committed, dirty tracked, and untracked drift; code paths cannot be exempted as post-review lifecycle evidence. `resolve_verification_readiness` now requires an injected provider and rejects incomplete branch path/revision/identity projections. The 4 provider regressions, 56 engine tests, 16 policy tests, 60 state tests, 65 automation-validator tests, lifecycle, metadata, guide, review-structure, compilation, and diff checks pass. The plan-mandated broad smoke passes all 11 checks in 388 seconds on the final synchronized source.
auto_fix_class: none

### code-review-m5-r2

#### BRF-M5-CR6 - Correction contract and policy authority are compressed
...
Decision owner: implementation author
Decision needed: none
Rationale: The implementation accepts only one mechanical kind and one path despite the preserved mechanical and declared-safe contract, and capability-wide mutation categories bypass the review-resolution stage's recorded category.
Required outcome: Project the complete preserved correction vocabulary and require every mutation category to be authorized by the bound stage policy, capability, and parent.
...
Decision owner: implementation author
Decision needed: none
Rationale: The plan hash is used as the final diff identity, source drift is unobserved, and the positive verify transaction trusts a report field instead of installing the required external-action traps.
Required outcome: Bind verification authority to one canonical final code identity and directly prove every prohibited external-action surface remains uncalled.
...
Decision owner: implementation author
Decision needed: none
Rationale: A later review of the same M2 occurrence is not considered applicable when the milestone title changes, so an earlier approval can remain canonical.
Required outcome: Compare latest-review applicability by structured milestone occurrence identity.
...
Decision owner: implementation author
Decision needed: none
Rationale: The correction path resolves only its bound findings but unconditionally marks the entire review-resolution document closed without checking unrelated entries.
Required outcome: Keep global closeout open until the canonical full-document predicate proves every contributing finding is closed.
...
Validation evidence: The integrated T13 transaction test proves a prepared receipt, exact reviewer recipe, bounded repository mutation, SHA-256 validation, capability consumption, and same-milestone rereview; engine, state, policy, schema, lifecycle, review, skill, selector-selected, and final 12-check broad-smoke validation pass.
auto_fix_class: none
...
Validation evidence: Repository-backed readiness tests prove current hashes and exact semantic fields for the closed plan, latest final holistic review, explanation, promotion, branch state, and verification commands; transaction tests no longer accept caller readiness booleans and still stop before PR.
auto_fix_class: none
...
Validation evidence: Direct duplicate-field, conflicting-result, duplicate-milestone, and handoff/body contradiction regressions pass in the state and engine suites; the unknown correction-operation validator regression also fails closed.
auto_fix_class: none
...
Validation evidence: Direct external-symlink and stale-later-review tests pass, along with the complete state/recovery and engine suites.
auto_fix_class: none
...
Validation evidence: Direct clean/no-resolution, material/open, canonical closed, stale, and invalid resolution contrasts pass in the state suite and full review-artifact validation.
auto_fix_class: none
...
Chosen action: Source-sorted the combined detailed and clean-table parser projection, replaced durable occurrence index with the canonical entry line, and exercised both mixed-format source orders plus the receipt-level rewind and duplicate proof.
...
Validation evidence: Mixed-format parser and receipt tests failed before correction and now pass. Final validation passed 104 review-parser, 57 state, 22 query, 64 automation-validator, 44 engine, 15 policy, 156 lifecycle, 259 skill-validator, and 53 metadata-validator tests, Python compilation, diff checks, and 12 broad-smoke checks in 241 seconds.
auto_fix_class: none
...
Chosen action: Made the state adapter's single parsed snapshot authoritative for every query, introduced a typed unified-automation validation error, and allowed missing or mismatched canonical change ID only for non-unified legacy/compact query snapshots.
...
Validation evidence: Valid and invalid no-automation-first contrasts failed before correction and now pass with byte stability. Unified-first valid/invalid, malformed generic metadata, legacy/compact compatibility, structural precedence, and full query/metadata suites pass. The complete 774-test selected set and 12-check broad smoke pass.
auto_fix_class: none
...
Chosen action: Added one state-adapter-owned repository semantic validator for every completed proposal-review receipt. Canonical read and replacement validate the tracked formal review and exact canonical occurrence before reporting or writing state; status and `query-change-record` inherit that boundary. Completed recovery reuses the same receipt-semantic helper while retaining its stricter current-log identity check and stable recovery reason codes.
...
Validation evidence: Proof-first read/status and query regressions reproduced the coordinated review-ID and all-known-outcome bypass before the correction. After correction, 56 state/recovery, 44 engine, 64 automation-validator, 20 query, 15 policy, 156 lifecycle, 103 review-artifact, 259 skill-validator, and 53 change-metadata tests pass. The append-only two-review history and post-review proposal-correction paths pass. The explicit selector reports only expected manual routing for the two automation-state paths, which the plan-owned suites cover directly; the final repository broad-smoke run passes all 11 checks in 231 seconds.
auto_fix_class: none
...
#### BRF-M4-CR15 - Cancellation reconciliation terminalizes proposal review without its route binding
...
Decision owner: implementation author
Decision needed: none
Rationale: `cancel()` reconciles a valid prepared proposal-review receipt directly to completed state without invoking the route projection used by normal finalization. The validator conditions route checks on `latest_review_result` already existing, so the omission passes canonical validation, and the new terminal-receipt immutability rule prevents later repair.
Required outcome: Every supported path that reconciles proposal review to completed state must preserve the exact verifier-derived route decision and occurrence identity while cancellation still becomes terminal and revokes authority.
Chosen action: Factor proposal-review completion into one state-writer projection used by normal finalization and cancellation reconciliation; require every completed proposal-review receipt to retain immutable route evidence and a source-linked latest occurrence; validate cancelled-run evidence, authority shutdown, and pause-reason absence; and extend focused state and validator regressions.
Safe resolution path: Factor one proposal-review completion projection across normal finalization and cancellation reconciliation; require every completed proposal-review receipt to carry its exact route binding and source-linked occurrence; define canceled-run validation explicitly; and extend the existing cancellation test with missing-binding, explicit-absence, immutability, authority-revocation, and terminal-run assertions.
Validation target: Normal finalization, cancellation reconciliation, exact route binding, explicit absence, canceled-run semantics, terminal receipt immutability, full state/engine/validator suites, CMD14-CMD20, and code-review M4 R9.
Validation evidence: Proof-first cancellation and missing-occurrence regressions failed before correction with a missing `proposal_review_route` and zero validator errors respectively. The corrected implementation passes 7 cancellation state tests, 6 focused proposal-review validator tests, all 44 engine, 15 policy, 51 state/recovery, 58 automation-validator, 156 lifecycle, 103 review-artifact, and 53 change-metadata tests, plus Python compilation and diff checks. The required repository broad-smoke result is recorded in the change validation ledger and active plan.
auto_fix_class: none
...
Chosen action: Keep the active-only selector for route-time authorization, add a persisted-route validator keyed by the recorded correction capability and exact review basis, and prove both active-to-consumed and absent-to-active route stability.
...
Validation evidence: Proof-first active-to-consumed and absent-to-active regressions failed before correction and now pass. CMD15-CMD20 pass with 7 proposal-review, 8 proposal-correction, 4 authoring, 4 non-public, 156 lifecycle, 103 review-artifact, and 259 skill tests; all 44 engine, 15 policy, 51 state/recovery, 56 automation-validator, and 53 metadata tests pass. Change metadata, review structure, guide-system, explicit lifecycle, Python compilation, and diff checks pass. Repository broad smoke passed all 11 checks in 838 seconds.
auto_fix_class: none
...
Chosen action: Compile canonical reviewed recipes into closed exact operations, remove arbitrary correction callbacks, and prove wrong-result rejection.
...
Validation evidence: The proof-first wrong-recipe regression failed before implementation. The focused proposal-correction suite now passes all 8 cases, including unsupported-recipe rejection and exact-operation execution; all 41 engine tests and the 12-check broad-smoke suite pass.
auto_fix_class: none
...
Chosen action: Remove arbitrary correction execution and apply only the internally compiled bounded operation to the exact proposal path.
...
Validation evidence: The callback-isolation regression failed before implementation. The focused proposal-correction suite now proves caller callbacks cannot execute, symlink escape attempts cannot occur, an injected atomic-replace failure leaves the proposal unchanged with no temporary file, and a rejected postcondition restores the original proposal bytes; all 42 engine tests and broad smoke pass.
auto_fix_class: none
...
Chosen action: Introduce one pure proposal-review projection and require validator equality with its concrete identity and run-state contract.
...
Validation evidence: Empty identities and contradictory known-value projections failed proof-first before the validator correction. All 55 automation-validator tests now pass, including concrete identity, gate/route/pause, target, correction-capability, and run-state consistency contrasts.
auto_fix_class: none
...
Chosen action: Use the shared projection during atomic finalization and bind the selected active correction capability for the correction-loop route.
...
Validation evidence: The transactional authorized-correction-loop test failed before the shared finalizer projection was connected. All 5 proposal-review tests, 51 state/recovery tests, and 55 automation-validator tests now pass; the persisted route carries the exact active correction-capability ID while absent or invalid authority pauses.
auto_fix_class: none
...
Chosen action: Removed the unconditional review-log requirement, returned stage facts in verified completion proof, routed without raw-path rereads, and added completed-recovery proof for authored, assessment, plan, and review stage families.
...
Validation evidence: Completed recovery passes for receipt-backed spec, architecture assessment, plan, and formal review proof; canonical review-log drift retains its specific pause; CMD14-CMD20, 36 engine tests, 51 state tests, 54 automation-validator tests, and the final 12-check broad-smoke run in 253 seconds pass.
...
Chosen action: Bound correction to canonical review, review-log, review-resolution, persisted finding/classification, and exact budget evidence; added independent post-mutation reread, strict shrinkage, deterministic validation, historical-review preservation, and an atomic finalization that either consumes correction authority while activating fresh proposal-review authority or pauses and invalidates the spent capability.
...
Validation evidence: Regressions reject empty, unknown, over-limit, stale-content, forged-finding, false-validation, non-shrinking, historical-review-drift, and missing-fresh-authority cases; failed postconditions atomically pause the receipt and invalidate correction authority, while successful finalization consumes old authority and activates exactly one current proposal-review capability in the same state write. CMD14-CMD20 and the final 12-check broad-smoke run in 253 seconds pass.
