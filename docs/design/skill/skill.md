...
Subsequent bounded authoring direction: [Design and Plan simplification](../../proposals/2026-09-15-design-plan-simplification.md), realized by [Plan](authoring/plan.md#authoring-presentation). Only that capability's presentation is newly selected; Design retains its existing package and authoring contract.
...
| Composition/path | SKL-SR-08, SKL-SR-09, SKL-SR-12, SKL-SR-17, SKL-SR-20, SKL-SR-31, SKL-SR-32, SKL-SR-33, SKL-SR-34, SKL-SR-35 | Entry classification reaches the complete selected reference and correct existing asset in supported candidates. Moving the body section does not bypass profile/placement validation; an unrelated resource containing required words does not satisfy the selected path. The implement-to-code-review handoff retains scope, direct proof and authority through relocated recording references; all selected package readers inspect those exact destinations. The complete refinement preserves each native classification and output; changed recording locations cannot bypass validation or drop applicable procedure. |
...
| Failure/recovery | SKL-SR-09, SKL-SR-11, SKL-SR-14, SKL-SR-17, SKL-SR-29, SKL-SR-31, SKL-SR-32, SKL-SR-33, SKL-SR-34, SKL-SR-35 | Missing required methods stop without invention; missing untriggered resources need not stop unrelated runtime work but fail package validity. A redundant convenience fallback is distinguishable from a missing normative method. Unknown profile selection or escaped paths fail explicitly. Changed proof, a concurrent CI target, partial area registration, lost vision manifest or conflicting learn result stops the affected mutation. Removing a pilot body heading while omitting its recording reference fails validation; a rejected or interrupted write remains visibly incomplete and leaves neighboring records intact. The complete refinement preserves each native classification and output; changed recording locations cannot bypass validation or drop applicable procedure. |
| Compatibility/migration | SKL-SR-01, SKL-SR-13, SKL-SR-19, SKL-SR-20, SKL-SR-21, SKL-SR-23, SKL-SR-28, SKL-SR-29, SKL-SR-30, SKL-SR-31, SKL-SR-32, SKL-SR-33, SKL-SR-34, SKL-SR-35 | Both improved pilots coexist with unchanged skill bodies and their validators. Current obligations resolve once, mixed sources keep bounded authority, Git history remains recoverable, and the original pilot leaves the other 17 skills outside its procedure-improvement claim. The later complete repository retirement reconciles their current contracts below rather than deferring their remaining spec authority. Retirement removes pilot metrics and historical counts while preserving plan assets and current specialist outcomes. The later inventory initiative retains each skill's public values, output fields, invocation and policy applicability; pilot-specific layout validation does not force other skills into that layout. A retained package has an assessed no-change rationale. The complete refinement preserves each native classification and output; changed recording locations cannot bypass validation or drop applicable procedure. |
| External/environment | SKL-SR-05, SKL-SR-10, SKL-SR-14, SKL-SR-15, SKL-SR-31, SKL-SR-32, SKL-SR-33, SKL-SR-34, SKL-SR-35 | Installed package instructions work without this internal checkout; supported candidate resources preserve identities. Missing local authority/tooling is distinct from a package defect. No live customer install, target-agent accuracy or release authorization is inferred. Both supported candidate packages supply the relocated pilot guidance without checkout-only links; semantic review reports the concrete reading-path improvement and its limits, not a runtime or token-saving claim. The complete refinement preserves each native classification and output; changed recording locations cannot bypass validation or drop applicable procedure. |
...
### Complete refinement round

The [complete refinement proposal](../../proposals/2026-09-15-refine-skills-and-retire-stale-support.md) extends assessment across the existing inventory and its support. SKL-SR-33–35 select the concrete presentation changes here; earlier pilot applicability and judgments are unchanged. This section records intended realization, not completed package assessment.

| Skill | Selected treatment and preserved responsibility |
| --- | --- |
| route | Refine: keep the quick operating guide, explicit routing/automation classification and essential stops first; move full recording construction into existing `references/governed-lifecycle-routing.md`. Preserve bootstrap, stateless commands, manual routing, unsupported automation and late-governed-context behavior. |
| verify | Refine: classify scoped, branch and governed-final outcomes before storage procedure; place complete recording in new local `references/governed-verification-recording.md`, triggered by selected adopted recording authority before a write. Keep isolated checks usable without a lifecycle store, success-only explanation and conditional seven-field Git basis. |
| pr | Refine: resolve intent, exact target and the three readiness owners before detailed governed inspection. Put read-only record-consumption procedure in existing `references/governed-pr-readiness.md`; preserve no lifecycle writes, all remote rechecks, partial effects, refresh authority and unknown-state stops. Reconcile the body asset under HAND-SR-04. |
| design-review | Refine: present engineering judgment and exact package before recording. Move complete inline recording procedure into existing `references/design-review-recording-and-settlement.md`, retaining every durable/formal trigger. Assess living-model outputs and relied-on examples under Design; retained feature, architecture and ADR sources remain read-only authority/evidence inputs. Unsupported output requests stop without source mutation; separately authorized scoped adoption reconciles their meaning into living Designs. |
| delivery-review | Refine: present indivisible sequencing/proof judgment before recording. Move complete inline construction into existing `references/delivery-review-recording-and-settlement.md`, preserving durable/formal triggers, exact primary-plan scope and correction ownership. |
...
| ci-maintenance | Retain nine selected assemblies and conditional provider/coverage resources; no permission or coverage-policy change. |
| explore | Retain optional option-space expansion and destination-owner handoff; no mandatory discovery stage. |
| research | Retain bounded factual uncertainty, attributable evidence and confidence; no generic research prerequisite. |
| vision | Retain strategic authoring versus README synchronization, existing markers and conditional assets. |
| constitution | Retain self-contained principle authoring; no reference file is required merely to match other skills. |
| project-map | Retain observation versus future intent, area coordination and follow-up ownership. |
| learn | Retain route-result versus session operation, collision handling and owned lesson follow-up. |

Retention is the selected engineering treatment, conditional on complete current package assessment during delivery. A discovered defect requires correction under its owner and affected independent review; it cannot be hidden by the retention row. All 19 packages remain allocated in this round.

#### Reading paths and consumer reconciliation

For the five selected entrypoints, retain `Resource map` and `Expected output`, a clear purpose and local authority boundary, and one coherent procedure. Equivalent combined scope/input/stop sections are permitted for this named population; this does not impose the original pilot heading regime or new metadata on every skill. Before a conditional action, resolve the native classification and load the mapped resource. False triggers do not load it; late true triggers load before the dependent operation. Missing, escaped, contradictory or mixed-package guidance stops, never falls back to remembered inline content.

Reuse the existing resource path in four skills; only Verify needs a new recording reference. Do not introduce a shared generic recording manual, cross-skill dependency or a new lifecycle predicate/value. PR's resource remains read-only. Its record consumption does not authorize the mutation procedures used by authors/reviewers. Shared policy applications retain their present adoption tests and complete portable copies; moving a procedure does not adopt a policy in a customer project.

Validation must resolve each selected entrypoint's declared recording destination and inspect the complete required profile there. Positive valid package and negative missing, wrong, escaped and incomplete resource cases must distinguish extraction from an early return that bypasses checks. Retain rejection of unsupported interfaces and unknown values before consistency checks. Existing packaging and installed-resource readers must observe the same destination and current bytes. Description and asset changes preserve native required output groups and truthful claim limits.
...
Apply [System's selection procedure](../test-design/rules.md#select-requirements-and-proof). The strategy accounts for all 35 requirements through recurring groups and explicit scoped-adoption dispositions. Its Implementation coverage includes complete IP0/IP1/IP2 handoffs, Bugfix authority/feasibility/original-proof identity, CI's unique conditional assemblies, guarded writes, atomic-group stops and bounded PR repair. Grammar checks cannot prove those semantic decisions; the catalog contains concrete independent procedures with explicit pending realization.
...
Child-local coverage remains with [Workflow](workflow.md#test-design), [Assessment](assessment.md#test-design), [Authoring](authoring/authoring.md#test-design), [Project Foundations](project-foundations/project-foundations.md#test-design), [Discovery](discovery/discovery.md#test-design), [Learning](learning.md#test-design) and [Delivery Handoff](delivery-handoff.md#test-design). Parent scenarios add actual recording/resource and capability handoff failures, rather than repeat each child's local conditions. The existing capability/resource/recording overview and supporting views already identify these relationships and unchanged package deployment; complete coverage accounting adds no runtime component.
...
| SKL-DEC-09 | Extend classification-before-recording presentation to the five named skills using existing local references, adding only Verify's missing recording resource; retain specialist layouts elsewhere and broaden Code Review description to its existing final-review duty. | A uniform rewrite or generic recording manual would add dependencies and hide specialist differences. Current complete-package assessment, not historical approval, justifies retention. |
...
Design DES-SR-25/26 owns the changed public method. Implement it in canonical `skills/design/SKILL.md`, `references/model-authoring.md`, related system-composition/test-quality guidance and `assets/design-skeleton.md`: the common procedure requires lasting coverage intent, the reference explains proportional model/parent ownership and the template exposes the section. Detailed case code and current results remain outside the generated Design. Apply DES-SR-14/23/24: preserve source interpretation, compact boundary reasoning and separately authorized scoped adoption into living Designs. Feature/proof and standalone architecture/ADR outputs remain unsupported, with source bytes and historical judgments preserved; retired output methods and their exclusive conditional resources must not be restored by this adoption.
