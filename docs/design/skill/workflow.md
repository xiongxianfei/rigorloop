...
Workflow protects correct owner selection and authorized continuation using current evidence. Its important surface is the Route decision and handoff, not an automatic CLI state machine. Apply [System's selection procedure](../test-design/rules.md#select-requirements-and-proof) to the requirement groups below. Use fresh synthetic decision packets containing project authority, exact current subjects, selected context and candidate next actions; inspect the complete applicable Route guidance independently. Fictional identities W0/W1 assert equality or difference only. The expected action comes from the named ownership and assessment contracts, independently of whatever action the candidate proposes.
...
| --- | --- | --- | --- |
| Scope, independent approval and continuation; WF-SR-01, WF-SR-02, WF-SR-03, WF-SR-09 | Given an isolated Design request, a successful save and an author's self-approval, reject a candidate that starts implementation. Select the authorized authoring/review action and retain missing independence as a limitation. A valid independent judgment can support only its exact current scope; mutable progress remains in the change record. | Contrast self-authored, independent and absent review packets under identical user authorization; inspect allowed next actions and preserved artifact roles. | Proposed independent Route walkthrough. [Route guidance](../../../tests/skill/skill_route_guidance_tests.py) and [authority tests](../../../tests/skill/skill_authority_tests.py) protect instructions, not the decision. Assessment owns the judgment itself. |
| Changed basis and correction owner; WF-SR-04, WF-SR-07, WF-SR-08, WF-SR-17 | After W0 was reviewed, change one model or relied-on example to W1. A candidate that reuses W0 approval or ignores a sibling consumer fails. Separate fault packets omit intended behavior, omit an allocated command, or contain a defective assertion: route respectively to Design, Plan and implementation, without requiring the owner to appear in an earlier pending set. | One changed subject or fault kind per packet; include independently named producer/consumer relationships and the exact previous judgment. | Proposed independent procedure; [Authoring correction scenarios](authoring/test-design/cases/correction-reconciliation.json) own sibling authoring interactions. Actual subject/conflict mechanics belong to CLI. |
| Post-completion failure and disposition; WF-SR-05, WF-SR-06, WF-SR-13 | With work marked complete, Verify discovers a required failure and reports blocker B1. The selected path records failure and reopens the correct owner; it creates no success report. After repair, return for independent reassessment and verifier-owned B1 disposition. A different reviewer's approval cannot silently close B1 or another unresolved finding. | An ordered correction sequence in one packet, preserving B1 origin and separate review-finding identity/current fields; compare each proposed action with its actor's authority. | Proposed walkthrough of the [correction-cycle example](examples/workflow/correction-cycle.mmd). [Record-store interaction tests](../../../packages/rigorloop/test/record-store-interactions.test.js) support executable persistence of actor decisions, not their semantic justification. |
| Scoped observations and explicit recording; WF-SR-11, WF-SR-12, WF-SR-14, WF-SR-15 | Supply a partial context omitting required Verify rationale and a save receipt with drift observations. A sufficient decision expands the missing basis before reliance, reads the exact final explanation/material decisions through their normal selectors and supplies explicit applicability. A recorded check alone cannot change activity, disposition or completion; a conflict/recovery-required result is not a save. | Real-shaped selected v3 responses with explicit omissions and distinct operation/status values; independent walkthrough of requests and proposed decisions. No whole-store reconstruction or invented hash computation. | [V3 read tests](../../../packages/rigorloop/test/record-store-v3-reads.test.js) and [targeted operations](../../../packages/rigorloop/test/record-store-targeted.test.js) provide mechanical proof. Full-basis interpretation and authority preservation remain proposed semantic review. |
| Final-review dependency; WF-SR-09, WF-SR-16 | With every milestone reviewed but no fresh whole-change review, reject a candidate that routes directly to successful Verify. A later in-scope correction after final review requires renewed applicable assessment before closeout. Even a one-milestone documentation change retains the dependency. | Plan plus milestone/final review packets with distinct scopes and subjects; explicit dependency walkthrough. | Proposed procedure; [shared-policy](../../../tests/skill/skill_shared_policy_tests.py) and [authority tests](../../../tests/skill/skill_authority_tests.py) provide bounded instruction checks. Assessment owns actual final-review sufficiency. |
| Current support and historical preservation; WF-SR-10, WF-SR-18 | For the original selected retirement, inspect its work/recovery/governance disposition and shared-protection transfer; an owner-confirmed baseline cannot be reported as an executed inventory. In current work, historical judgment remains attached to its original bytes and cannot supply v3 authority or an operational retired-format path. | Read-only scoped source/disposition packet plus a current v3 correction request. Historical retirement acceptance is conditional on that original scope, not a new recurring suite. | Proposed provenance/reliance review when applicable. Current format and safe recording are covered by [Records](../cli/records.md#test-design) and [CLI](../cli/cli.md#test-design); preserve the original no-new-retirement-tests constraint. |
| Follow-up ownership and resumption; WF-SR-02, WF-SR-19 | A fresh actor without chat receives a map risk already owned by a change, an accepted unowned cross-change task and a vague suggestion. Keep the first with its change, place only the second in the optional register with owner/source/action/disposition, and do not invent a commitment for the third. Unknown register status is rejected rather than treated as open. | Separate concrete action packets and current authoritative artifacts only; independently trace owner and next action, including an existing entry transferred to a plan without duplicate active ownership. | Proposed independent procedure. [Project-map static tests](../../../tests/skill/skill_project_map_tests.py) inspect present register fields and closed statuses, but skip when the register is absent; a controlled unknown-value regression and semantic ownership assessment remain alignment gaps. |

These groups account for all WF-SR requirements and section-owned follow-up placement. CLI and Records supply safe storage; Assessment supplies independence, applicability and closeout policy. Workflow adds the actual decision to select the right owner, adequate basis and authorized continuation across those boundaries. Semantic procedures remain proposed until performed against exact subjects and recorded with their expected/observed actions and limits; they are not target-agent runtime tests. The existing overview and Context, Building Block and Runtime views describe these same relationships, so this coverage refinement adds no component or deployment view.
...
Workflow consumes Design DES-SR-25/26 and System TEST-SR-21–23. A current Design package includes lasting coverage intent, explicit requirement-selection rationale and gaps; Plan allocates executable proof without becoming its sole historical home. Missing intended behavior, observation boundary or model ownership returns to Design; missing commands, timing or milestone allocation returns to Plan; faulty fixtures/assertions return to implementation. Assessment retains judgments and applicability, and CLI remains storage-only.
