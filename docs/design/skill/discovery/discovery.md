...
Apply [System's selection and proof rules](../../test-design/rules.md#select-requirements-and-proof). DISC-SR-01–03 concern selecting and composing support without transferring decision authority. [Explore](explore.md#test-design) owns useful alternatives and its artifact; [Research](research.md#test-design) owns qualified factual answers and its artifact. This parent adds the selection and return path, where two individually sound artifacts can still produce an unauthorized adoption or unnecessary lifecycle step.

| Behavior group and requirement basis | Concrete fixture and faulty candidate | Action and independently expected observation |
| --- | --- | --- |
| Select the actual uncertainty — DISC-SR-01, DISC-SR-03 | Independent packets describe an unsettled product direction, a selected direction blocked by one platform fact, an option comparison materially affected by that fact, and a settled direction with adequate facts. Faulty candidates always invoke both capabilities or label incidental thought a completed explicit invocation. | Review the decision and proposed routing. Select Explore, Research, both with an explicit bounded question, or neither respectively. Resources/artifacts follow the actual explicit request; discovery is optional support and creates no mandatory stage. |
| Return evidence without adopting the option — DISC-SR-01, DISC-SR-02 | Explore compares local and hosted operation and asks Research about a specified offline requirement; Research supplies qualified evidence under an exact source/version. The compliant handoff preserves confidence and names the direction owner; the faulty one promotes a favored option to approved direction. | Trace evidence, inference and limits through the handoff and inspect the receiving decision. The owner may adopt, reject or request more evidence; the support artifact cannot mutate the proposal, Design, approval or stage status. Child conclusions retain their actual scope. |
| Changed framing, resources and scope limits — DISC-SR-02, DISC-SR-03 | After the question is framed, the owner changes the supported deployment environment; the available source addresses only the old one. Independent variants remove required shared resources or supply unsafe artifact placement. The faulty candidate reuses the old conclusion unchanged or falls back to an unrelated writable target. | Inspect reassessment and proposed writes. Revise the bounded question or report the owned blocker/uncertainty before reliance; preserve source attribution, privacy and project placement. Do not claim missing evidence, a missing resource or historical record conversion is successful completion. |

Use fresh request/evidence packets, a named decision owner and explicit current authority. Source dates/versions and the expected routing are supplied independently of the candidate. Paired compliant/faulty handoffs must differ in their decision or effect, not merely their wording. No external calls, decision writes or workflow mutations are required for these reviews.

Existing [Discovery guidance tests](../../../../tests/skill/skill_discovery_guidance_tests.py) check routing phrases, optional invocation guidance, required files and exact shared-policy bytes. Its temporary-file cases detect source-copy drift and an unknown consumer; they do not establish useful research, correct routing or owner adoption. The groups above are proposed independent walkthroughs, with concrete packets and actual assessment still to be allocated by Delivery. [Skill's support-consumption scenario](../test-design/cases/capability-composition.json) owns the wider receiving-capability interaction. Existing overview/runtime views already show selection and owner handoff; test detail adds no component or lifecycle stage.
