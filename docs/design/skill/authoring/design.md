...
Input is an approved direction or an explicitly scoped authorized correction, affected model identities and relevant current contracts. Output is the smallest reconciled set of model changes, preserved decisions/references, declared consumer impacts and reviewable acceptance intent. A retained customer feature specification may remain a scoped output under its own contract. Standalone architecture documents and ADRs are source inputs, not supported authoring outputs; their authority remains unchanged until an explicitly authorized adoption reconciles the selected responsibility.
...
| DES-SR-14 | Scoped work on a retained feature specification MUST retain its applicable contract and historical reference identities unless separately approved for migration. Architecture/ADR source preservation and supported output follow DES-SR-24. New features MUST update existing responsibility owners where appropriate; repository-wide conversion MUST NOT become a prerequisite for a small change. |
...
| DES-SR-23 | Repository model-only authoring and retained customer feature-format methods MUST remain distinct after specs retirement. The portable compact core, feature authoring and proof method retain their scoped grammar and behavior; repository rollout activation and grandfathering replay retire under Validation without retroactively invalidating customer artifacts. |
| DES-SR-24 | Architecture and decision authoring MUST produce living Designs, not create, rebuild or amend standalone architecture documents or ADRs. Existing documents MUST remain readable authority/evidence inputs; required old-format output MUST stop without project-file mutation, implicit conversion or reconstructed legacy procedures. An explicitly authorized scoped adoption MUST reconcile all affected obligations and decision meaning into the current owner before displaced authority is retired. Separate customer feature-spec/proof support remains unchanged. |
...
| `references/legacy-source-reconciliation.md` | Retained source inputs or feature-spec amendments: select authority, distinguish supported output, preserve IDs and load the applicable feature method; explicitly authorized scoped adoption | Automatic conversion, blanket deletion or historical approval rewriting |
...
The customer project supplies its authoritative legacy requirements, approved decisions, local format/version and any mandatory project-specific template or schema. Packaged methods explain supported feature-spec amendment or source reconciliation into a living Design; they cannot invent authority or supply standalone architecture/ADR authoring. A missing packaged method is a distribution defect that stops the dependent invocation. Missing or contradictory project authority instead requires the project owner to resolve that bounded gap; installing more generic guidance or migrating the document does not settle it. The author may proceed with independently authorized unaffected work. A project-specific required feature scaffold must come from that project. An architecture/ADR scaffold cannot re-enable unsupported output; the project owner must choose an authorized scoped adoption or another authoring tool.
...
5. Preserve references and current ownership. For a selected migration, use the exact displacement map; for a retained feature-spec amendment, preserve its declared contract and boundary format. A required standalone architecture/ADR output stops before authoring; source inspection alone grants no conversion authority.
...
The repository uses living Designs and Delivery proof allocation. It no longer authors or retains its own feature specifications or separate test specifications. The compact boundary scan remains routine; a model uses its requirement-linked eight-dimension scenario table without a separate ID series. Customer projects may explicitly retain their feature-format contract and substantive-amendment classification under their own authority. A customer's unknown or conflicting adoption basis stops that dependent authoring decision; installing a package or deleting this repository's specs establishes no customer policy.

The unchanged portable `boundary-first-v1` compact core owns its closed dimension vocabulary, record fields, identifier grammar, example classification and interaction-selection method; feature-authoring and proof references supply their respective method detail. These remain packaged application of this Design and Validation, not owners of lifecycle settlement, placement or permissions. The current consumer set is route, design, design-review, plan, delivery-review, implement, code-review and verify, with specialized references loaded only where mapped. Packaging owns the maintained source locations and byte-preserving projections.

Retire this repository's historical activation-state parser, release rollback lookup and Git-derived grandfathered-file inventory with their exclusive readers. The earlier grandfathered-spec observation paragraph describes the replaced repository profile, not a requirement for a customer Git database or new repository feature files. Structural checking of explicitly supplied feature/proof records remains possible under Validation, independent of current v3 record storage. The generic `7ad33e1b1827c84dfa4e9fbbfc8b52a204b5139e:specs/feature-template.md` scaffold is removed; living model assets remain the normal repository authoring shape. Accepted old IDs and review judgments retain their original meaning in recoverable history.

Existing authoring views remain sufficient: this changes source and applicability selection within the same model/legacy input boundary, not the reconciled-authoring responsibility or independent review path. No new submodel or authoring stage is introduced.
...
| Portable source reconciliation; DES-SR-14/15/23/24 | Supported feature-format amendments retain authority and IDs; unsupported standalone architecture/ADR output leaves files unchanged; authorized scoped adoption preserves meaning. | Customer-like private sources and installed guidance without the internal checkout. | [Skill portability tests](../../../../tests/skill/skill_portability_tests.py), resource checks and specialist assessment retain their distinct claims. |
...
DES-DEC-07 supersedes only the architecture/ADR authoring portion of DES-DEC-04 and earlier complete-package retention decisions. Customer feature-spec/proof contracts and unrelated compatibility responsibilities remain current. Missing adoption authority leaves existing documents and their authority unchanged; a skill installation supplies none. A request to edit an ADR or rebuild an old architecture receives an unsupported-output explanation and the bounded owner decision, not a substitute artifact.
...
| Compatibility/migration | DES-SR-01, DES-SR-06, DES-SR-13, DES-SR-14, DES-SR-17, DES-SR-20, DES-SR-21, DES-SR-23, DES-SR-24 | Supported packages supply design and omit retired authoring entries; Installation handles actual candidate conflicts and explicit replacement under its current contract. A small retained feature-spec amendment preserves its owner/format; standalone architecture/ADR output is unsupported without modifying existing sources, while selected migrated clauses have exactly one replacement and old approvals remain unchanged. A customer-selected feature/proof record remains assessable after the repository activation ledger and feature template are absent. |
| External/environment | DES-SR-05, DES-SR-12, DES-SR-15, DES-SR-17, DES-SR-24 | A clean installed package contains every triggered method without the internal repository checkout, including living test-design authoring, supported feature-format methods, source reconciliation and technical guidance; retired standalone scaffolds are absent. Other target roots and user files remain untouched; context/structure checks make no target-agent performance or publication claim. |
...
| DES-DEC-07 | Retire standalone architecture/ADR authoring and its exclusive resources; retain existing-source interpretation, explicit scoped adoption and customer feature-spec support. | Maintaining two architecture output methods increases guidance and validation duties without an established present need. Compatibility withdrawal is explicit: an old-format requirement stops rather than silently converting a project. Preserve engineering meaning in living owners instead of shipping old output scaffolds. |
