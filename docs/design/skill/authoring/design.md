...
| DES-SR-25 | Each model MUST own a proportionate living test design linking its requirements to behavior groups, important functions/interfaces, meaningful scenarios, expected observations, fixture strategy and implementation locations. Account for every current supported requirement and section-owned responsibility using System's explicit selection procedure; risk determines depth and priority without waiving required proof. Parents own integrated coverage and reference child details. Use the shared System criteria without duplicating them or requiring one case per function; guidance-only responsibilities use justified review or walkthrough methods. Unknown coverage and proposed tests MUST remain distinguishable from observed existing coverage. |
...
| --- | --- |
| Purpose and basis | Account for each current supported requirement and section-owned responsibility, then identify its behavior group, plausible violation, consequence and selection rationale. Explicitly identify existing proof, a proposed realization, an unresolved gap or a justified scoped applicability disposition; a missing row is not an omission decision. |
...
The Design skill's common reconciliation and handoff must include this section, with detailed method in model-authoring and an adaptable table/prose scaffold in design-skeleton. Apply System's [requirement and proof selection procedure](../../test-design/rules.md#select-requirements-and-proof) through the project's declared shared rules before expanding a catalog. Account for supported obligations, explain the material failures and selected depth, group related variations, preserve distinct authority/persistence/recovery observations and keep known gaps explicit. A reviewable stopping decision identifies what protection is sufficient and why remaining possible variations add no distinct required observation; it cannot silently dispose of a missing requirement. Native discovery owns the method inventory; no per-method catalog entry or case-count target is required. A smaller catalog does not establish executable redundancy. Shared guidance can have a declared supporting directory and one existing owner, while scenarios stay with their models. The packaged skill must convey this procedure through its local methods and examples without requiring this repository's documentation paths; adopted customer policy still governs its use.
...
Delivery allocates the exact models, groups and consumers selected for each change. The earlier focused application selected Release, Skill and Authoring; the separately approved complete-model direction now accounts for every current model and section-owned responsibility. Existing adequate scenarios remain usable and catalogs remain optional outside their selected owners. Missing or contradictory coverage rationale remains with its model owner, and proposed realization is never presented as demonstrated proof.
...
| --- | --- | --- | --- |
| Coherent engineering owner and credible direction; DES-SR-01, DES-SR-02, DES-SR-03, DES-SR-04, DES-SR-09 | For Authoring's atomic batch-import goal, inspect a Design that validates/writes each row in sequence: expose the partial-write counterexample and require reconciliation with the all-or-none goal. Contrast a Design with one importer owner, explicit failure state and credible validate-before-write realization. A material feasibility conflict returns to the direction owner rather than quietly weakening atomicity. | Use the [shared Authoring fixture](test-design/test-design.md#synthetic-artifact-fixture); independently derive the destination after a bad middle row. Review the complete model and source intent, not keyword presence. | Proposed independent method walkthrough; AUTH-RF-001 covers goal transfer across stages. [Shared-policy tests](../../../../tests/skill/skill_shared_policy_tests.py) supply bounded wording/traceability support only. |
| Architecture and composition; DES-SR-05, DES-SR-07, DES-SR-08, DES-SR-22 | Add a writer child and a shared validation-to-write interface. Inspect a candidate parent that repeats child rules but omits their agreement: require an owned integrated atomicity observation. An overview naming a store without an owner, a necessary Runtime view left undrawn, or inconsistent authorization edges fails reconciliation. A pure leaf can justify no additional deployment diagram without inventing services. | Leaf/parent model packets with explicit consumers, necessity reasons and one conflicting edge per variation; independent graph-to-contract inspection. | Proposed walkthrough using [architecture-view examples](../../../../skills/design/references/architecture-view-examples.md) and actual model outputs. [Design resource tests](../../../../tests/skill/skill_design_resource_tests.py) prove resource presence, not architectural adequacy. |
| Model grammar; DES-SR-11, DES-SR-19 | Starting with a valid model, independently introduce an unknown marker/dimension, duplicate requirement, missing table or undeclared reference. Observe the owning diagnostic; unknown closed values reject before consistency inference. Unsafe or symlinked paths reject with relevant bytes unchanged. | Real validator with minimal private model trees, independently declared valid IDs and visible single mutations. Include competing unknown/consistency defects only for rejection-order protection. | Existing [model-record tests](../../../../tests/engineering/validation/boundary_model_tests.py) and [public command tests](../../../../tests/engineering/validation/boundary_command_tests.py); structural success makes no credibility claim. |
| Examples and exact handoff; DES-SR-08, DES-SR-11, DES-SR-12, DES-SR-16 | Contrast an unindexed example, invalid complete JSON record and a before/after pair that silently changes a preserved neighbor. None establishes the illustrated invariant. Change only a relied-on example E0 to E1 while the parent is unchanged: the handoff contains E1 and the parent; its old approval is not replayed. | Minimal complete record and declared excerpt packets, independently known unchanged fields; actual parsed JSON plus independent invariant/subject review. | [Model examples](../../../../packages/rigorloop/test/record-store-model-examples.test.js) and document tests support current concrete fixtures/shape. Semantic invariant and example-only review applicability remain proposed procedures. |
| Meaning and scoped source disposition; DES-SR-06, DES-SR-13, DES-SR-18, DES-SR-20, DES-SR-21 | For an authorized source move, preserve a requirement, decision alternatives and failure constraint at named current owners; a filename-only mapping or unresolved mixed-source obligation blocks removal. Review the changed consumers with original subject identities preserved. DES-SR-20 accounts for the original first slice only: its handoff cannot assert whole-repository consolidation. | Independently listed source obligations, changed current destinations and read-only original bytes/identities; compare meaning and live references, not line counts. | Proposed scoped adoption review. Historical first-slice acceptance stays with its original change; no recurring replay or archive corpus is created. Current link/shape checks provide narrower support. |
| Portable source and output boundary; DES-SR-14, DES-SR-15, DES-SR-17, DES-SR-23, DES-SR-24 | Given existing customer feature/architecture sources, an old-format output request stops with source and user-modified neighbors unchanged. The same source may support separately authorized living-model adoption preserving decisions and IDs. A replacement package with retired authoring entries is not coherent; removing a triggered surviving method stops dependent authoring. | Fresh customer-like private roots, exact before/after file manifests and complete installed-package reading paths without the internal checkout. | [Portability](../../../../tests/skill/skill_portability_tests.py), [Design resources](../../../../tests/skill/skill_design_resource_tests.py), and [command rejection](../../../../tests/engineering/validation/boundary_command_tests.py) protect bounded automated surfaces. Actor preservation/adoption decisions remain proposed review; actual installer preservation belongs to Installation. |
| Requirement selection and durable proof; DES-SR-10, DES-SR-16, DES-SR-25, DES-SR-26 | Give a leaf two supported requirements and a prior regression missing from its catalog. An author who selects only cheap cases, treats the catalog as a whitelist or leaves the second requirement unexplained fails the method. A sufficient output accounts for both, preserves the regression until its protection is assessed, separates current tests from gaps and explains why another equivalent input adds no observation. A guidance-only model chooses concrete independent review. | Compliant and faulty model/test-design outputs under [System's worked selection procedure](../../test-design/rules.md#select-requirements-and-proof); parent variation adds an interaction whose failure passes both children. Expected protection is derived from the contract before reading existing tests. | Proposed semantic procedure here and AUTH-RF-003/004 for transfer into Plan. [Design resources](../../../../tests/skill/skill_design_resource_tests.py) and [shared-policy checks](../../../../tests/skill/skill_shared_policy_tests.py) remain structural support; published method/template reconciliation and actual walkthrough evidence require Delivery allocation. |
...
These groups account for all DES-SR requirements while retaining the original scope of adoption duties. Every semantic walkthrough records exact source/package/output subjects, the compliant and faulty candidate, observed decision and evidence limits when performed. Selection policy changes the existing reconciliation and handoff content; the overview and necessary Context, Building Block and Runtime views still identify its actual owners. No new deployment boundary or structural document marker is needed.
...
Path selection accepts portable `docs/design/M/M.md` regular files and this repository's exact [System directory map](../../system.md#repository-directory-layout), including its explicitly declared flat System root. Generic flat `docs/design/M.md` files and absent historical aliases are unsupported; neither presence nor a deletion/rename history grants admission or receiver substitution. Stable model IDs remain independent of mapped filenames. Examples retain their declared owner; mismatched portable IDs, paths into examples, extra normative nesting and symlinks reject. Existing customer sources keep project authority and bytes: unsupported validation supplies no automatic relocation, conversion or retargeted approval. Feature/proof validation remains unsupported; document markers are independent of runtime record formats. Existing views remain applicable because this narrows the input boundary without introducing a responsibility or deployment.
