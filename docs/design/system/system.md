...
These candidates outline potential responsibility boundaries before new model files are authored. They are not adopted owners, deployed components, or a mandatory final directory inventory. Existing models remain the first choice; Design may merge, split or reject a candidate after reconciling its actual obligations. The present necessary-design cleanup still creates no new model. Candidate definition here does not expand its implementation scope.
...
| Release — `docs/design/release/release.md` | Release identity, transaction profiles, composition of package proof, publication permissions, release evidence and recovery. This recurring external operation is distinct from packaging and engineering approval. | [Release process](../../../specs/release-process-contract.md); [release transactions](../../../specs/release-transaction-automation.md); [process ADR](../../adr/ADR-20260523-release-process-contract.md); [profile ADR](../../adr/ADR-20260629-release-transaction-profile.md). Published-skill-first R7/R8 and release portions of “Standing release-process flow”, “Release and adapter evidence” and “Public npm package boundary”. | Consumes Distribution package identities and Installation smoke where applicable; does not inherit publication authority from tests or reviews. FU-013: distinguish standing release obligations from version-specific rollout evidence, then select the receiving Design. |
