...
The [Release-model direction](../../proposals/2026-09-09-release-model-source-consolidation.md) selects [Release](../release/release.md) and its exact source/consumer dispositions under the [Release owning change](../../changes/2026-09-09-release-model-source-consolidation/change.json). This is prospective ownership until reviewed coordinated implementation and successful Verify; it neither repeats prior consolidation nor authorizes publication.
...
| Release/publication and release evidence | [Release](../release/release.md), prospectively at its coordinated adoption; its displacement map identifies the still-operational legacy sources until then | Maintainer, release tooling, package/installation proof and public evidence; local success grants no publication permission. Mixed package and historical release responsibilities retain their owners. |
...
Release is now a separately selected model; the other candidates remain prospective. The remaining-area row is a boundary against blind conversion, not an assertion that every historical file is current. Known component owners above are the starting inventory; a previously unclassified responsibility needs explicit ownership before reliance.

### Candidate models and legacy source ownership

These candidates outline potential responsibility boundaries before new model files are authored. They are not adopted owners, deployed components, or a mandatory final directory inventory. Existing models remain the first choice; Design may merge, split or reject a candidate after reconciling its actual obligations. The earlier necessary-design cleanup created no new model. The subsequent Release initiative selects only Release, with its separate adoption boundary; candidate definition does not expand either initiative.
...
| --- | --- | --- | --- |
| Distribution — `docs/design/distribution/distribution.md` | Canonical-to-target generation, invocation aliases, declared transformations, package inventory and archive identities. Skill owns content; distribution owns the materialized target package. | [Adapter invocation](../../../specs/skill-invocation-commands-for-adapters.md); [archive install surface](../../../specs/stop-tracking-generated-public-adapter-skill-bodies.md); [generated adapter ADR](../../adr/ADR-20260424-generated-adapter-packages.md); [archive-surface ADR](../../adr/ADR-20260513-v0-1-3-adapter-release-archive-install-surface.md). Also the package portions of published-skill-first R4/R5/R27/R28 and mixed architecture “Validation and Generation Scripts” / “Generated output”. | Consumes Skill resources; supplies archives and identities to Installation and Release. FU-013: maintainer selects whether generation warrants this separate owner and maps complete transformation/package obligations before adoption. |
| Installation — `docs/design/installation/installation.md` | Verified archive acquisition, target-root materialization, managed project state, conflict/drift handling, upgrades and recovery. Existing CLI owns recording mechanics; that does not make it the installer contract. | [Target-native init](../../../specs/target-native-init.md); [multi-adapter init/download](../../../specs/multi-adapter-init-and-proxy-aware-download.md); [lockfile](../../../specs/rigorloop-cli-lockfile.md); [init-state ADR](../../adr/ADR-20260524-target-native-init-state-boundary.md); [lockfile ADR](../../adr/ADR-20260516-rigorloop-cli-lockfile.md). Mixed architecture “CLI target-native init, download, state, and release-smoke flow” and installation/state portions of “CLI package, project scaffold, and lockfile boundary”. | Consumes Distribution identities and release metadata; preserves authorization and local-state safety. FU-013: reconcile command generations and later amendments, including TNI-DES-01–06, before selecting the final model and transferring authority. |
| Release — [selected model](../release/release.md) | Release identity, transaction profiles, composed package proof, publication permissions, public evidence and recovery. | Release's exact displacement map selects the two standing specifications, matching test specifications, two ADRs and bounded architecture prose; version-specific and mixed remainders stay explicit. | FU-013's Release portion is selected for coordinated adoption; Distribution/Installation remain separate. No actual release is authorized. |
...
Release owns the standing operation and its [exact source displacement](../release/release.md#source-displacement-and-retirement); Design owns the scoped DES-SR-21 retention extension. The mixed published-skill-first source transfers only R7/R8 through its reciprocal amendment. Shared generation, installation and release-specific contracts remain dependencies, not extra model migrations.

For SYS-SR-02/04/06/07/08/09, assess the new Release model, this System amendment, Design's retention extension and the R7/R8 source amendment as one package. Demonstrate complete local meaning and current references after the six selected source removals, retained historical/operational baseline identity, and an unmodified release consumer using its existing contracts. If a packaged dependency changes, trace its current candidate metadata and actual consumers; source-only proof cannot cover that change.

The composed failure examples are local package success with a mismatched hosted publication identity, a public write with delayed registry visibility, and source retirement with an unresolved reader. Release owns their precise operation outcomes; System requires producer/consumer consistency without duplicating them. Final whole-change Code Review and distinct Verify establish adoption; neither authoring nor source deletion establishes publication. FU-013 retains Distribution/Installation and any explicitly retained release remainder; Route reconciles its progress at implementation handoff.
