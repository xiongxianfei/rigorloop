...
Engineering owns the section-level Development responsibility and additional interactions among Validation, Packaging and Release. Apply [System's selection procedure](../test-design/rules.md#select-requirements-and-proof) to these outcomes; child designs own their local correctness. The principal integration risks are confusing the development tool with the candidate, treating structural/local passes as independent approval, assembling mutually incompatible products, losing protection during cleanup and optimizing execution against an incomplete population.

| Requirement group and plausible defect | Selected scenario and independent expected observations | Boundary, fixture, realization and limit |
| --- | --- | --- |
| Development authority and independent closeout — ENG-SR-01, ENG-SR-02, ENG-SR-03, ENG-SR-04, ENG-SR-09, ENG-SR-12 | Walk an exact candidate through approved direction/Design/Delivery, bounded implementation, milestone review, fresh whole-change Code Review and distinct Verify. Introduce a changed relied-on subject or behavioral gap after a local pass: return the affected issue to its owner, retain actual partial work and reassess before renewed reliance. Record installed/checkout tool identity separately from the artifact under test; neither self-use nor a non-final review grants completion. | Proposed independent walkthrough of the actual plan, subjects, reviews and evidence, with [Workflow](../skill/workflow.md) and [Assessment](../skill/assessment.md) as semantic owners. A small synthetic valid/invalid handoff pair can exercise the procedure; it cannot approve the real change. CLI interaction and record tests support storage only. |
| Canonical source to usable products — ENG-SR-01, ENG-SR-06, ENG-SR-07, ENG-SR-11, ENG-SR-16 | Generate supported archives and pack the actual CLI from the same candidate. Independently inspect mapped resources and execute installed recording/installation commands from a fresh consumer root. Missing nested implementation/resource or stale bundled metadata must fail, while no repository executor or internal change identity is required for customer use. Read generated instructions against actual supported invocation forms. | Real source→archive/tarball→installed consumer, using [Packaging's composed proof](packaging.md#test-design), [packed recording](../../../tests/engineering/packaging/npm_recording_tests.py) and [Skill portability tests](../../../tests/skill/skill_portability_tests.py). Independent guidance walkthrough remains necessary for semantic usability; artifact membership alone is insufficient. |
...
| Qualification versus publication authority — ENG-SR-06, ENG-SR-07, ENG-SR-08, ENG-SR-09 | Feed the exact built candidate and check identities into Release. An altered artifact or missing approval prevents the external write; a lost provider response remains uncertain and recovery observes before retry. Local success cannot supply public identity/smoke evidence. The same product checked during development cannot silently be replaced during qualification. | [Release's candidate/publication scenarios](release/test-design/test-design.md#candidate-and-publication) own the actual coordinator and provider-fault realization; Packaging supplies real bytes. Engineering independently assesses their identity/evidence handoff. Future actual publication observations apply only to an authorized release, never to a benchmark run. |
...
These groups account for ENG-SR-01–16 without duplicating each child's scenarios. Mechanical supporting suites do not complete the Development, transfer or authority judgments: Delivery must allocate the independent walkthroughs against actual subjects and retain their limits. Native discovery and assertion/fixture audits establish what proof is implemented; evidence records establish what ran. The existing Context, Building Block, Runtime and Deployment views already express these ownership and artifact handoffs, so no new component or diagram is needed for this coverage refinement.
...
Development reconciles executable groups, imported/generated cases, actual direct and aggregate callers, and inline/file/resource fixtures through bounded Delivery slices after independent Design Review. Use native discovery to account for the supported executable population; documentation or broad-smoke membership alone cannot establish it. Capture baseline timings before suite changes and compare the affected population afterward under [Validation's execution-cost contract](validation.md#execution-cost-reporting), including worker/environment conditions, incomplete results and the difference between total elapsed and summed case cost. Expensive proof remains required until an adequate retained or replacement observation is established. Future operational assessments remain applicability-scoped and are not executed merely to fill a timing row.

The selected guidance includes Design and its authoring/template resources, independent review, planning, implementation and verification consumers. Published skills remain independent of this repository's internal paths. Independent review and Verify assess the exact complete package and integrated consumers; a completed scenario catalog or faster run alone does not establish test/fixture alignment.
