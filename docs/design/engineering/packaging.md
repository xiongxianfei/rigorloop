...
| DIST-SR-09 | Current candidates MUST use `rigorloop-tree-hash-v2` with the exact representation below, including normalization, regular-file membership, path ordering and independent counts. Consumers MUST retain the existing `rigorloop-tree-hash-v1` verification path for historical trusted metadata; missing algorithm fields retain their historical v1 meaning. Unknown algorithms MUST reject. Target root safety MUST be checked separately; excluding symlinks from a hash does not authorize writing through them. |
...
| Hash algorithm | Regular files only; exclude directories, symlinks, metadata/times/ownership, absolute paths, the lockfile and temporary files. Relative UTF-8 POSIX paths have no leading `./` or trailing `/`. For v2, sort their UTF-8 byte sequences lexicographically, case-sensitively, without locale collation, case folding or Unicode normalization. Normalize generated text to UTF-8/LF and remove a BOM without trimming whitespace or semantic normalization; binary files retain raw bytes under the existing deterministic classification. Each file digest is SHA-256 of those bytes. For v2, hash UTF-8 `rigorloop-tree-hash-v2\n` followed by sorted `<relative_path>\t<file_sha256>\n` rows. |
...
The v2 transition is candidate-scoped: generate new metadata and its consuming CLI together, and qualify that exact candidate before publication. Existing released metadata, archives and v1 verifier semantics remain unchanged. An older CLI cannot consume v2 metadata; it must reject the unknown algorithm. Recovery restores the coherent pre-transition candidate without rewriting any published identity. The legacy profile-driven public-filesystem observation retains v1 representation, with its required text normalization, version prefix and symlink exclusion; current immutable-candidate public smoke compares the algorithm declared by its approved artifact metadata. This change adds no target, deployment component or publication authority; the existing overview and supporting views remain sufficient.
...
| DIST-DEC-07 | Use a new `rigorloop-tree-hash-v2` identifier for exact UTF-8 byte ordering across producer, CLI and current public-smoke observation. Preserve the existing v1 producer helper and CLI verifier behavior for historical evidence and trusted metadata; do not recompute or rewrite published artifacts. | Python case folding and locale-dependent CLI collation disagree for admitted resource names. Silently changing v1 would also break previously agreeing Unicode inputs. V2 makes new candidate digests intentionally distinct, keeps old-consumer rejection explicit and requires coordinated builder/metadata/packed-CLI qualification. A legacy v1 mismatch remains a historical limitation, not permission for a fallback that accepts an arbitrary digest. |
