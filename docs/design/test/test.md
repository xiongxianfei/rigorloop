...
The change uses rigorloop-records-v2. The model-validation marker above versions document structure independently of stored records. No schema change, CLI judgment selector, automated deletion, history ledger, migration, new gate or mandatory separate test specification is introduced. Public guidance must not require internal model IDs, repository-maintainer details or an internal Design checkout.
...
| --- | --- | --- |
| Input domain | TEST-SR-01, TEST-SR-03, TEST-SR-06 | Representative partitions and justified generated inputs contribute explicit protection without exhaustive combinations. |
...
| Composition/path | TEST-SR-04, TEST-SR-05, TEST-SR-10 | Helper proof cannot substitute for public-path behavior; a selector omission cannot produce a valid coverage claim. |
| Temporal/retry | TEST-SR-03, TEST-SR-06, TEST-SR-09 | Retry and race protection remains distinct; generated failures expose reproducible or investigable conditions. |
...
| Compatibility/migration | TEST-SR-02, TEST-SR-08, TEST-SR-13 | Unlabelled historical tests remain valid candidates for retention; retirement needs explicit authority and adoption preserves old records. |
