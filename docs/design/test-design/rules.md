...
TEST-SR-23 makes the selection procedure explicit. Apply it to every current requirement and section-owned responsibility in the selected model scope, including nonfunctional guarantees, public failures and prohibited effects. For a complete-model adoption, the scope is the current hierarchy; for a change, include affected producers, consumers and parent interactions. A CI path selector chooses existing checks to execute; it cannot decide whether the required proof exists.

1. Identify the supported outcome and governing requirement or justified engineering obligation. Split a compound requirement into the outcomes whose violations need different observations, while retaining its existing identity. Account for scoped migration obligations at their actual adoption or correction point; completed historical activity does not become a perpetual replay test. Preserve an unexplained regression while its owner settles the intended behavior.
2. Describe the plausible violation and consequence: under which condition could the system produce a wrong result, lose state, cross authority or hide failure, and what observation would expose it? Start from the contract before inspecting how the existing suite happens to be organized. Include known defects and cross-owner interactions that a requirement checklist alone would miss.
3. Explain risk using consequence, plausible triggering conditions, exposure and recovery. Unauthorized writes, corrupted records, false completion or irreversible publication warrant strong failure and transition proof even when infrequent. Complex decisions, concurrency, recent changes and prior defects justify additional distinctions. Risk chooses depth and priority; cost and low frequency cannot waive required proof. Qualitative reasoning is sufficient; this adds no numerical score or closed risk vocabulary.
4. Choose a repeatable automated observation for executable behavior and a concrete independent inspection or walkthrough for semantic judgment. Select the smallest boundary that can expose the violation, preserving any explicitly required broader observation. Name what a substitute dependency leaves unproved. Refine an unobservable or contradictory requirement with its owner instead of inventing an expected result.
5. Select representative outcome partitions and relevant limits. Use decision alternatives when combinations change the rule, transitions when state changes the result, and controlled interruption or faults when partial effects or recovery matter. Group equivalent variations; keep different failure mechanisms independently understandable. Properties may describe a generated domain and invariant without enumerating every input.
6. Connect the scenario to an independent expected result, minimal realistic fixture and existing or proposed realization. Inspect actual assertions and native discovery before claiming existing coverage. Requirements, scenarios and executable cases may relate many-to-many. An unlinked useful regression stays protected; a link or passing method does not prove that every claimed outcome is asserted.
7. Review the selected group against a concrete faulty candidate or inspected failure mechanism. Stop adding variations when each supported outcome and material hazard has sufficient observation, and another case supplies neither distinct detection nor useful diagnosis. Stop a completeness claim when a required outcome, owner, realization or observation remains unresolved.

Keep this reasoning in the existing group table or prose: requirement/outcome, plausible violation and consequence, selected conditions/actions/observations, fixture strategy, realization and limits. No new per-requirement record, mandatory field schema or per-method inventory is introduced. A current obligation is either covered by sufficient proof, explicitly awaiting proof, or subject to a responsible owner's bounded disposition. A disposition explains retained proof, actual non-applicability or authorized retirement; documenting a gap does not make it satisfied. Actual execution and assessment remain separate evidence.

### Worked selection: Installation

This example applies [Installation's destination and partial-failure contract](../cli/installation.md); the owner retains the complete requirements and scenarios. Start with a verified small candidate archive, a private destination containing identifiable old bytes, and an unrelated sentinel file. Expected files and contents come from the independently specified candidate, not the installer's own inventory calculation.

| Plausible violation | Selected condition, action and observation | Why this observation is needed |
| --- | --- | --- |
| An existing skill is silently overwritten without permission | Invoke the real installer on an existing candidate destination without force; observe the conflict diagnostic and unchanged old bytes and sentinel. | Argument parsing alone cannot prove absence of filesystem writes. |
| Forced replacement merges stale files or damages an unrelated destination | Repeat with explicit force and a valid candidate; observe the complete replacement unit, absence of obsolete files within it and preservation of unrelated files. | Success and default refusal protect different required outcomes. |
| A preflight failure still permits installation | Corrupt the candidate's integrity while preserving valid command setup; observe the intended integrity failure and no destination mutation. | An unrelated setup failure or nonzero exit alone would not expose this defect. |
| A later write failure is reported as complete success | Inject a failure after a completed installation unit; observe the owner's permitted partial state, accurate incomplete result and bounded retry guidance. | Installation does not promise whole-operation rollback; inventing that expectation would test the wrong contract. |

Similar malformed values within one integrity or input rule can share named variations. Different target/platform cases are justified when their path or permission mechanism changes, not merely because a target name differs. Packaging owns archive production proof; this example consumes that boundary rather than reproducing its complete catalog.

### Worked selection: Design authorship

Use a small proposal requiring an atomic batch import, a compliant living Design and a separate faulty candidate that silently permits partial import. An independent reviewer compares the approved outcome, requirement, technical behavior and coverage intent and identifies the weakened guarantee and responsible correction owner. A second candidate that retains the guarantee but allocates only a parser check exposes insufficient observation of persistence. Keep these failures distinct: one changes behavior, the other fails to prove it. Matching headings or a requirement ID can pass for both faulty candidates and therefore supplies only structural support. Review records identify the inspected candidate and observed decision; preparing this example is not an executed assessment or universal agent-behavior certification.
