...
[verify-report.json](verify-report.json) is a complete proposed v3 Verify record under [RF-SR-05/07/09](../../../records.md#requirements). It contains the required explanation fields and the success-only outcome, but its evidence_refs and review_refs arrays are empty. No verification_basis object is needed for the illustrated non-Git scope.
