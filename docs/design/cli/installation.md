...
Select cases under [System's shared procedure](../test-design/rules.md#select-requirements-and-proof) for Codex and Claude Code, network and local archive acquisition, default conflict and explicit replacement. These dimensions need representative composed paths, not their Cartesian product for every parser value. The high-impact failures are trusting altered packages, writing outside the selected units, deleting concurrent user work and reporting a partial operation as complete. Preserve the actual packed-CLI proof required by Packaging DIST-SR-18; narrower helpers add fault control and useful diagnosis without replacing it.

| Requirement group and selection reason | Conditions, action and independent expected observations | Fixture, boundary and supporting realization |
| --- | --- | --- |
| Admission and truthful dry-run — DIST-SR-02, DIST-SR-11, DIST-SR-15 | Invoke both supported targets, then missing/unknown/retired targets, removed adapter/state syntax and pinned/local/dry-run variations. Rejection precedes acquisition/extraction/writes and names supported forms. Dry-run with and without force reports preliminary checks and unperformed full verification without claiming installed bytes. Human/JSON status, exit and replacement warning agree. | Real CLI dispatch with counted external acquisition and private destination snapshots. [CLI tests](../../../packages/rigorloop/test/cli.test.js) and [packed smoke](../../../tests/engineering/packaging/test-npm-package-publication.py) supply support. A returned no-write label alone is insufficient; align filesystem and acquisition observations with each claimed boundary. |
| Trusted acquisition and extraction — DIST-SR-07, DIST-SR-08, DIST-SR-16 | From valid bundled metadata/index and a matching archive, independently alter metadata digest, target/release/URL, archive digest/size, roots/counts or tree algorithm. Tampered bytes reject at their intended verification stage. Local input cannot replace trust metadata; network uses exactly the trusted URL. Archive traversal/absolute/drive-letter/empty names, symlinks and out-of-root members reject before installed-file mutation. The allowed support entrypoint is not installed. | Small real ZIPs and package metadata with independently computed expected digests/counts, private roots and controlled provider responses. [CLI acquisition tests](../../../packages/rigorloop/test/cli.test.js) exercise actual acquisition/verification; Packaging supplies shared independent representation proof. Safe substitutes prove local decisions, not public endpoint availability. |
| Whole-candidate default conflict and bounded force — DIST-SR-10, DIST-SR-12, DIST-SR-13, DIST-SR-15 | A candidate has existing identical, empty, changed and regular-file units plus one absent unit. Default installation reports every conflict, changes none and does not install the absent subset. Explicit force replaces only selected complete units, removes obsolete interior files and installs absent units; unrelated skills, retired entries, mirrors and all state files remain untouched. Invalid/unreadable state cannot affect admission because it is not read. | Real filesystem and actual packed CLI for both roots; independently compare installed inventory and preservation bytes. [CLI conflict tests](../../../packages/rigorloop/test/cli.test.js), [replacement tests](../../../packages/rigorloop/test/installer-replacement.test.js) and [Release candidate composition](../../../tests/engineering/release/release_candidate_tests.py) supply support; the last exercises actual packed repeated conflicts and force on both targets. Byte preservation alone does not prove absence of state reads; allocation must retain an observation capable of detecting those reads. |
| Containment, race and retained original — DIST-SR-08, DIST-SR-12, DIST-SR-13, DIST-SR-14 | Preflight valid units, then substitute an ancestor, modify an existing unit or create a destination between detach and publish. Stop without escaped writes or overwriting the competing object. Symlinked/inaccessible units and unsafe retention placement fail in both modes. Force detaches originals outside all discovery roots; a late write through an open original handle remains recoverable there. Unavailable anchored primitives stop before detachment. | Real private directories, symlinks, open handles and controlled checkpoints around the actual replacement engine. [Replacement tests](../../../packages/rigorloop/test/installer-replacement.test.js) supply direct support; packed smoke establishes public safety composition. The oracle inspects the external sentinel, competing object, active candidate and retained original separately, never assumes all outside writes were excluded. |
| Partial failure and retry — DIST-SR-13, DIST-SR-14, DIST-SR-15 | Fail after one unit completes and while another publishes, leaving a third untouched. Report completed, failed/partial, untouched and retained-original locations consistently with actual bytes; stop further writes. Retry performs verification/preflight again, conflicts by default on completed/partial destinations and requires a new explicit force choice. Retained names cause no automatic restore/delete or eligibility bypass. | A three-unit candidate and actual replacement faults. [Replacement tests](../../../packages/rigorloop/test/installer-replacement.test.js) include mid-publication interruption and retry; extend/confirm the public result projection for the same partial-state distinctions. This is per-unit bounded preservation: no whole-operation rollback is promised or tested as required behavior. |
| Safe network diagnostics and fallback — DIST-SR-07, DIST-SR-15, DIST-SR-16 | Simulate DNS/TLS/timeout/HTTP/proxy/unknown failure using a valid selected source and private proxy sentinels. Diagnostics retain only admitted names/values, target/release and trusted public URL, reveal no raw credential/path/private-host data, and cause no additional probes. A subsequent local-archive fallback still rejects bad integrity and succeeds only with trusted matching bytes. | Controlled network boundary with the actual error renderer and local verifier; private environment per process. [CLI proxy tests](../../../packages/rigorloop/test/cli.test.js) supply support. Unknown diagnostic vocabulary must reject before consistency checks; neither runtime guessing nor a mocked success can establish verification. |

The assertion and fixture audit must establish the no-read/no-write and public partial-result observations above, reusing adequate existing proof where present, and account for generated target variants through native discovery. The inspected packed candidate case covers repeated identical conflicts and force replacement, but does not establish empty-unit conflicts or unsafe-destination rejection in both modes at that same packed boundary; complete or identify that required composed proof before claiming DIST-SR-18 alignment. Supporting paths are existing proof locations, not a claim that every scenario is fully realized or has passed in this change. Shared immutable archives may be copied into private workspaces; mutable destinations and retained originals are case-owned and cleaned up by the test, never by deleting a caller's installation. [Validation](../engineering/validation.md#execution-cost-reporting) owns attributable timing; optimize repeated archive construction while retaining real packed installation and race boundaries. The existing context, building-block, runtime and deployment views remain sufficient because the selected behavior, trust roots and per-unit write protocol are unchanged.
