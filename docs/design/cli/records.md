# CLI Records Model Design

Model validation contract: model-document-v1

Parent model: [CLI — Records](cli.md#records).
...
| --- | --- | --- |
| Actor responsibility, decision meaning, justified progression and independence | [Workflow](../skill/workflow.md) | Supplies the semantic obligations represented by records |
...
The [finding correction example](examples/records/v3-finding-correction/README.md) shows a complete stored and updated Review plus its linked CLI operation. It exercises RF-SR-13: the same finding ID retains an editable current account without an origin snapshot.

The [complete five-kind collection](examples/records/v3-complete-store/README.md) provides a linked [change](examples/records/v3-complete-store/change.json), [Review](examples/records/v3-complete-store/reviews/final-code-review.json), [evidence](examples/records/v3-complete-store/evidence.json), [material decisions](examples/records/v3-complete-store/material-decisions.json) and [Verify](examples/records/v3-complete-store/verify-report.json). Each is a complete record under RF-SR-01/02/03/07/09–12, with a consistent virtual registry, explicit applicability and resolvable EntryRefs. Its README defines the synthetic identities, omitted external subjects and incomplete lifecycle-proof scope. The collection illustrates relationships without claiming actual review, execution or justified completion.

The [V3 review before a limitations update](examples/records/v3-review-limitations-update/before.json) and [after the update](examples/records/v3-review-limitations-update/after.json) are complete v3 Review objects using synthetic identities. Only limitations differs; findings and all other values remain identical, with no v3 finding origin. They illustrate RF-SR-09/10/13, not executed byte-preservation proof. The [Verify example](examples/records/v3-verify-without-evidence/verify-report.json) is a complete non-Git v3 report with explicit empty supporting-reference arrays to illustrate recordability rather than justified success. The [conditional-basis example](examples/records/v3-verify-limitations-update/before.json) adds the complete optional object for an explicitly synthetic Git/PR claim; its branch labels and revisions are illustrative, not resolved repository facts. None is a registered record or new judgment. Executable schema conformance and transition proof belong to implementation evidence; JSON parsing or Design inspection alone cannot establish them. Earlier v2 examples are historical illustrations only.

The [Verify limitations-update example](examples/records/v3-verify-limitations-update/README.md) pairs a complete after record with the indexed conditional-basis before record. It illustrates RF-SR-09/10/11 by changing only limitations and retaining the complete optional basis. The corresponding CLI requests and projections are owned by the [CLI example index](examples/README.md).
...
The supported population is the five current v3 record kinds and their shared typed references, explanation values and preservation transitions. Apply [System's selection procedure](../test-design/rules.md#select-requirements-and-proof): closed-shape partitions detect accidental admission, paired before/after records detect lost meaning, and the complete candidate catches reference defects invisible to standalone schemas. [CLI's test design](cli.md#test-design) owns command grammar, byte transport, filesystem safety and recovery execution; the Records design supplies the independent data invariants those tests observe.

| Requirement group and plausible defect | Concrete scenario and independent observation | Fixture, realization and evidence limit |
| --- | --- | --- |
| Closed current representation — RF-SR-01, RF-SR-09, RF-SR-11 | Validate a complete five-kind store, then independently vary unknown kind/member/enum/version, missing explanation, whitespace-only value, dual body, duplicate key and mixed registered version. The intended structural diagnostic occurs before reference/consistency work. Repeated ordered rationale strings and multiline Unicode round-trip unchanged. Absent optional verification basis is valid; null, partial or extra-member basis rejects without Git access. | Fresh small objects plus literal malformed JSON where parser behavior matters. [V3 contract tests](../../../packages/rigorloop/test/record-store-v3-contract.test.js), [format tests](../../../packages/rigorloop/test/record-store-format.test.js) and the [complete-store examples](examples/records/v3-complete-store/README.md) supply supporting cases. The v3 contract helper currently accepts several error categories; strengthen cases requiring a specific first rejection so an unrelated reference defect cannot satisfy them. |
| Registry and typed references — RF-SR-02, RF-SR-08 | Begin with one review, check, decision and Verify; validate each allowed reference field against its permitted target. Contrast missing ID/file, wrong collection, unregistered path, duplicate applicability and a review-root/finding or manifest collection ID collision. They reject even when a colliding object is unreferenced. The same ID in different files is unambiguous; a new target/reference in one final candidate succeeds. Extra physical files are not authority. | Hand-authored ID/path matrix over the complete candidate, with private files only for publication composition. [V3 contract tests](../../../packages/rigorloop/test/record-store-v3-contract.test.js) and [targeted reference tests](../../../packages/rigorloop/test/record-store-targeted.test.js) are supporting realizations. Alignment must account for every field-specific target class and disjoint namespace, not merely demonstrate one broken reference. |
| Finding correction versus blocker origin — RF-SR-03, RF-SR-04, RF-SR-08, RF-SR-13 | Correct every admitted non-ID finding field and withdraw a mistaken finding with a resolved rationale; retain its ID. Removal/rename and origin/basis additions reject. Replace review assessment and observe all finding bytes unchanged. Correct/dispose a blocker and later replace its supporting review: copied origin and supporting judgment remain exactly the original basis, independently of current fields. | A review finding and a blocker with distinctive original/current subjects and rationale. Independently inspect before/after objects and bytes through targeted and advanced writers; reference CLI recovery proof for exact snapshots. [V3 contract](../../../packages/rigorloop/test/record-store-v3-contract.test.js), [v3 mutation](../../../packages/rigorloop/test/record-store-v3-mutations.test.js) and [targeted origin](../../../packages/rigorloop/test/record-store-targeted.test.js) tests supply support. Actor labels are data, not proof of attribution or independent assessment. |
| Correctable incomplete work and faithful reading — RF-SR-03, RF-SR-05, RF-SR-07, RF-SR-10, RF-SR-11 | Record a failed check/new blocker while activity remains completed and a historical subject has changed. Structure remains recordable, observations disclose drift/failure, and no approval, applicability or next-stage value is invented. Read full Verify/decisions and finding/blocker accounts without unrelated bodies; projections disclose omissions and leave storage unchanged. Optional basis absence does not become a branch-readiness claim. | Private current store with independently known neighbors and a changed external subject. [Interaction tests](../../../packages/rigorloop/test/record-store-interactions.test.js), [v3 reads](../../../packages/rigorloop/test/record-store-v3-reads.test.js) and [queries](../../../packages/rigorloop/test/record-store-queries.test.js) provide mechanical support. Proposed semantic walkthrough: an independent reader uses those exact records without prior chat, identifies the current problem, retained blocker basis, proof limits and missing authority; record the assessed subjects and any ambiguity under Assessment. |
| Current consumer agreement and historical preservation — RF-SR-06, RF-SR-08, RF-SR-12, RF-SR-14 | Inspect schema/template/runtime imports and the packed candidate: v3 stays usable without a retired stored codec, independent transport versions stay admitted, and old records are neither converted nor retargeted. In an interrupted current update, CLI's exact complete/restore proof retains registration and preservation rules. Assess actual consumers against the current support boundary; do not execute historical workflows to establish current correctness. | [V3 adoption tests](../../../packages/rigorloop/test/record-store-v3-adoption.test.js), [public persistence tests](../../../packages/rigorloop/test/record-store-cli.test.js) and [packed recording](../../../tests/engineering/packaging/npm_recording_tests.py) supply current-contract support. Completed retirement dependency dispositions remain scoped to their original changes; their static/no-new-retirement-tests allocation is not a new historical replay requirement. Future support changes need their own authorized disposition. |

Design completeness means all requirement groups have selected observations; the supporting links do not establish a whole-suite assertion audit, completed semantic assessment or successful run. The specific diagnostic-precedence and typed-reference matrix work above remains proof alignment work. Full record validation must establish the valid baseline before a defining mutation, while before/after preservation expectations come from captured independent bytes and the permitted field set. CLI shares the composed realization instead of requiring a second transaction suite here. No architecture view changes are needed: the model still defines data consumed by the same external persistence and assessment owners, without a new stored schema, sidecar or deployment.
