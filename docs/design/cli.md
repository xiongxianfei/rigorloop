...
| `verify` | No filters; where must be empty | Complete Verify metadata plus body, which carries the final assessment and explanation |
| `decisions` | No filters; where must be empty | Complete material-decisions metadata, including all decisions, plus the shared body |
...
| Context/show/status inspected (except observations.show) | change_id, revision, data, scope | Nonempty observation_summary |
| Observations inspected | change_id, revision, data, scope | None; detail scope replaces observation_summary |
...
CLI-SR-02/03/09/10/11 own this interface. `--root` is an explicit existing repository directory and `--change` is a Record Format change ID. Inputs and outputs are not inferred from the current working directory. Supported authoritative paths relative to that root are exactly `docs/changes/<id>/change.yaml`, `docs/changes/<id>/reviews/<review-id>.md`, and the three conditional files `evidence.yaml`, `material-decisions.md`, `verify-report.md` in that change directory. Registry entries cannot expand this allowlist. The review filename and metadata ID must agree.
