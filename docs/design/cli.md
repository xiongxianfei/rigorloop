...
### Retained-assessment amendment boundary

Workflow's [retained judgments design](workflow.md#retained-judgments-for-unresolved-findings) identifies a storage-model gap in the v1 review update below: preserving findings while replacing the current assessment can lose their supporting judgment. It proposes review schema version 2 with named immutable assessments and explicit finding references. That revised stored shape is not accepted by the current v1 compatibility path or implemented by merely adding an undocumented request field.

For the revised representation, CLI construction will preserve stored assessments and finding support links independently of which assessment the actor selects as current. The actor supplies a new assessment ID and its full judgment/provenance/rationale, the intended current-assessment selection, and explicit supporting IDs for affected findings. The engine supplies only deterministic construction, reference checks and preservation. No automatic archive-selection decision, latest-judgment inference or approval-driven finding closure is permitted.

The existing operation table remains the v1 compatibility design; it is not sufficient implementation authority for the revised review shape. Completing this amendment requires an exact version-dispatch decision and matching mappings for review.record, finding.add/set, review/finding show, context/status judgment projection, advanced writes and limits/recovery. Those mappings must follow Workflow's immutable-assessment and append-only-reference rules. No command may advertise retained supporting judgments for a v1 review simply because its findings array survived replacement. This design refinement changes neither the existing schema file nor executable recording behavior.
