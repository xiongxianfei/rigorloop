...
| TEST-SR-23 | Test selection MUST apply the [requirement and proof selection procedure](test-design/rules.md#select-requirements-and-proof) to every current obligation in the selected scope and material parent interaction. Model coverage MUST explain plausible violations, risk-based depth, representative scenarios, sufficient observation boundaries and realization or explicit gaps. The complete-model adoption MUST account for all current document and section-owned model responsibilities. Risk, cost, catalog presence and passing checks MUST NOT silently waive required proof or turn an unresolved gap into completion. |
...
| SYS-DEC-06 | Extend the selected living test-design method to the complete current model hierarchy, with explicit obligation accounting and risk-based scenario selection under TEST-SR-23. Keep proportional inline sections and the three existing catalog packages. | Catalog creation for every model duplicates method inventories without establishing sufficient proof. Limiting refinement to the earlier three packages leaves the user's all-model scope unmet. Shared selection examples improve consistency while model owners retain their scenarios, fixtures and semantic judgments; Design completeness remains separate from implemented and executed proof. |
...
The TEST-SR-01–23 rows in [Requirements](#requirements) are the single normative owner of shared testing criteria. Earlier IDs and protective meaning are preserved from Validation; scoped cleanup clauses keep their existing limits. TEST-SR-23 adds the explicit selection procedure and complete-model adoption. Validation retains the VAL-SR execution contract, concrete source layout, selector/document admission and reporting. Design owns the portable authoring method, Delivery owns concrete allocation, and Assessment owns independent judgment and evidence applicability. This composition adds no model, runner, universal case catalog, mandatory test layer or lifecycle gate.
...
The focused application reconciled its selected catalogs, executable groups and guidance consumers. The [complete-model direction](../proposals/2026-09-17-complete-model-test-coverage.md) now selects every current model and section-owned responsibility for coverage reconciliation. The [navigation](test-design/README.md#model-test-designs) links each owner's durable account; it carries no duplicate scenarios or completion state. Adequate existing coverage remains usable, and known implementation or semantic-assessment gaps remain explicit until the responsible proof and judgment exist. Scope does not depend on a separate JSON package, permanent per-method ledger or numerical test quota.

Design authorship and its templates consume the shared selection procedure; Plan allocates its outcomes; Assessment judges sufficiency and actual reliance. Skill owns portable instruction/resource integrity, and Engineering owns implementation of changed guidance and reporting consumers. Reconcile those sources in their delivery slices; authoring this Design does not claim that published resources or execution interfaces already implement the refinement. Existing Context, Building Block, Runtime and Deployment relationships remain valid: this adds detail to existing coverage and reporting responsibilities, not a new component or trust boundary.

The earlier testing change did not withdraw feature/proof compatibility. The subsequent feature-format retirement changed only that support and its direct consumers; Records schemas, installation targets and release permissions retain their existing contracts. The historical-path retirement withdraws predecessor routing and generic flat-model support under [Validation](engineering/validation.md#current-path-support-and-retirement); exact declared paths, including this System root, remain supported. Complete-model coverage adds no further compatibility retirement.

### Test design

System owns the integrated observations below and the application of its shared policy. Leaf scenarios stay with Skill, CLI and Engineering. Each row identifies a plausible mismatch that independently passing children could miss. Existing modules supply bounded support; proposed walkthroughs require recorded observations before they establish proof.

| Group and basis | Conditions, action and independent observation | Fixture and boundary | Realization and limits |
| --- | --- | --- | --- |
| Responsibility and architecture agreement; SYS-SR-01, SYS-SR-02, SYS-SR-10, SYS-SR-11 | Compare the declared hierarchy and producer/consumer views against their detailed owners. A candidate that assigns approval to the executor or places customer adoption inside installation is rejected; valid ownership retains one detailed authority and consistent edges. | Current affected models plus a private contradictory candidate; independent architecture walkthrough and real document-link/grammar validation. No fabricated runtime service is needed. | [Model validation](../../tests/engineering/validation/boundary_model_tests.py) supplies structural support. Proposed independent walkthrough assesses actual ownership and view adequacy. |
| Intent, coverage and delivery composition; SYS-SR-03, SYS-SR-06, SYS-SR-08; TEST-SR-01–06, TEST-SR-11–16, TEST-SR-18, TEST-SR-21, TEST-SR-23 | Follow the shared batch-import example from approved direction through model requirements, local/integrated scenarios, planned proof and assessed evidence. Compare a coherent package with weakened behavior, parser-only proof of atomicity, a missing model and a planned case presented as passed. Each faulty candidate receives the appropriate upstream correction; unrelated valid proof remains bounded. | Independent compliant and single-fault artifact copies with explicit subject identities; review actual outcomes and traces, using [Authoring's fixture](skill/authoring/test-design/test-design.md#synthetic-artifact-fixture). | Authoring and Assessment own detailed procedures. This proposed cross-owner walkthrough supplies the additional whole-chain observation; structural wording checks do not perform it. TEST-SR-14 keeps its cleanup-only scope within applicable allocation. |
| Published resources and actual consumer agreement; SYS-SR-04, SYS-SR-05, SYS-SR-09; TEST-SR-13 | Generate the maintained skill package and use the packed CLI in a private project. Required resources resolve, documented recording shapes work and installation preserves unrelated project state. Removing a required selection resource or substituting an incompatible command must be exposed at its actual consumer. | Real generated archive and packed CLI, independently inspected inventory, private project and unrelated-file sentinels. Package mechanics and semantic method judgment remain distinct observations. | [Adapter resource tests](../../tests/engineering/packaging/adapter_resources_tests.py), [npm recording tests](../../tests/engineering/packaging/npm_recording_tests.py) and Installation proof supply existing mechanics. Updated resource parity and composed semantic selection walkthroughs remain implementation/assessment work. |
| Concurrent correction and honest reliance; SYS-SR-06, SYS-SR-09 | Two actors use one prior record revision; after the first update the second is rejected without replacing it. A corrected engineering subject then requires reassessment, and a storage save or green suite cannot revive the earlier approval. | Actual CLI and fresh private record store for conflict; matching artifact copies and independent Assessment/Workflow review for reliance. Expected preserved bytes are captured before mutation. | [Targeted record tests](../../packages/rigorloop/test/record-store-targeted.test.js) and Records persistence proof cover mechanics. Proposed composed walkthrough covers the semantic decision after the conflict; a retry alone proves no approval. |
| Test maintenance and reporting composition; TEST-SR-07–10, TEST-SR-17, TEST-SR-19, TEST-SR-20, TEST-SR-22, TEST-SR-23 | Move or consolidate a behavior group using established replacement protection. Compare normal discovery, actual callers and intended observations; a lost generated case, stale fixture, shared mutable state or duplicate cost attribution defeats the affected claim. A slow required case stays protected until adequate replacement exists. | Existing native entrypoints and small isolated Python/Node fixture groups, before/after discovery identities, one deliberate assertion/selection fault. Execution timing stays with Validation. | [Validation's test design](engineering/validation.md#test-design) owns executable adapter/report cases; independent review of actual changed source establishes retained detection and complete model-to-proof alignment. No full implementation audit is claimed by this table. |
| Scoped source transfer and current reliance; SYS-SR-07, SYS-SR-12, SYS-SR-13; TEST-SR-14 | At a selected retirement or correction, compare displaced obligations with current owners and consumers. An unmapped still-supported obligation or live dependency on removed material blocks retirement; historical approval bytes retain their original meaning. | Bounded source disposition and actual reader/reference inspection under Engineering; use private malformed current fixtures for surviving protection. | [Engineering](engineering/engineering.md#repository-retirement) owns the scoped audit. Completed migration activity does not require permanent tests for historical report presence or repeated retirement. Current shared-safety proof remains in its model. |

Design review judges all current obligations accounted for and the scenarios capable of detecting their named violations. Implementation alignment and final coverage claims additionally require the real test/fixture audit, applicable walkthrough results, caller reconciliation and resolution of required gaps. Evidence and mutable progress remain in the owning change records.
