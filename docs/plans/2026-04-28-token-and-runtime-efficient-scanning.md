...
- Validation commands:
  ...
  - Selector inspection / manual-routing proof:
    ...
    - Expected selector result: this may block on `scripts/test-adapter-distribution.py` as `script-unsupported` / `manual-routing-required`; the manual route is the pass-gate command `python scripts/test-adapter-distribution.py`.
...
- Validation commands:
  ...
  - Selector inspection / manual-routing proof:
    ...
    - Expected selector result: this may block on `scripts/test-adapter-distribution.py` as `script-unsupported` / `manual-routing-required`; the manual route is the pass-gate command `python scripts/test-adapter-distribution.py`. Do not pass directory path `skills`; pass each changed canonical `skills/<name>/SKILL.md` path explicitly.
...
- 2026-04-28: after plan-review revision, `python scripts/select-validation.py --mode explicit --path scripts/build-adapters.py --path scripts/adapter_distribution.py --path scripts/test-adapter-distribution.py --path scripts/validation_selection.py` blocked only on `scripts/test-adapter-distribution.py` as `manual-routing-required`; the plan records `python scripts/test-adapter-distribution.py` as the manual pass-gate route.
- 2026-04-28: after plan-review revision, the M4 selector inspection with concrete `skills/<name>/SKILL.md` paths had no unclassified `skills` directory path and blocked only on `scripts/test-adapter-distribution.py` as `manual-routing-required`; the plan records the direct manual route.
