...
- Required verification: TG-05 and TG-06 — TG-05: change.create/link, activity.set, work.add/set, review.record, finding.add/set, blocker.add/set, evidence.record, applicability.set, decision.record, verify.record, batch; omitted fields/neighbors/whitespace/body preserved, missing decisions rejected, v2 body token round-trip, required v2 origin construction with supporting judgment explicitly absent, supplied directly, or copied from an explicitly selected review; missing or null v2 origin rejects while v1 retains its compatibility absence of origin, immutable origin under every update path, ID collisions, overlapping edits, allowed same-batch references, all-or-none rejection, no-op and retries, preview writes nothing/reserves nothing. TG-06: after completed activity Verify records failure+blocker, Route later records correction, reviewer approval cannot close Verify blocker, stale review identities are retained; role labels never authenticate and successful save never establishes approval.
- Evidence expectations: Focused automated positive/negative checks at the owned boundary, named command results, exact implementation/design subjects, and a concise explanation of any manual assessment.
- Implementation steps: Add failing contract cases; implement the bounded slice; run focused commands; reconcile the integrated candidate; hand off for independent Code Review.
- Validation commands: C2, C4, C5, resolved exactly in the command table below.
...
- Completion criteria: All required adoption rows have exact reviewed diffs or justified unaffected dispositions; no required dependency deferred. Full integration and generated-output checks pass before activation is proposed. TG-08 supports the claimed qualitative benefit with an adequate decision basis, or the owning Design decision has explicitly dispositioned unfavorable/inconclusive evidence and any resulting approved design/plan changes have received the required rereview. No unqualified adoption recommendation follows merely from completing the benchmark.
- Required evidence: Registered evidence.yaml check entries under this v1 change with subjects, procedure/result/summary; independent milestone code-review record with explicit applicability. Do not create routine standalone implementation or review logs.
- Review handoff: Independent Code Review of M5's implementation and its full required proof. Before M6, final Code Review must also assess the complete integrated implementation against the approved design and delivery plan with current subjects and no unresolved material findings. The same final invocation may satisfy both scopes if its recorded subjects and assessment explicitly cover the whole change; an M5-only review does not.
...
- Dependencies: M1–M5 reviewed and required corrections reassessed; final independent Code Review has assessed the complete integrated change against the approved design and delivery plan with current subjects and no unresolved material findings; current evidence and review applicability explicitly established.
...
Token comparison is a controlled manual/automated hybrid under TG-08: run the same finding append, evidence+blocker, applicability/reassessment and final-read tasks through both interfaces against isolated equal fixtures. Save totals, tool/tokenizer versions and interpretation in registered evidence; do not commit transcripts or operation requests. A count without loaded skill content and required follow-up reads is insufficient. Reviewer assesses whether observations support the stated benefit, not an invented threshold. Unfavorable or inconclusive evidence returns to the owning Design decision before adoption is recommended. No fixed percentage, improvement for every operation, or safety relaxation is required.

M5 implements named reusable fixtures and a documented invocation/procedure alongside ordinary recording tests: finding-with-neighbors, failed-verify-correction, applicability-reassessment and final-explanation-read. Use these stable scenario names in test data and registered results. Document fixture construction/reset, both command sequences, exact guidance/help loading, tokenizer/version, environment, retries and inclusion rules so another contributor can reproduce the comparison. C2/C4 execute the automated portion; human assessment records decision-basis adequacy and the qualitative conclusion. Synthetic model examples alone do not replace runnable benchmark fixtures, and full interaction transcripts or per-operation request histories are not committed.
...
M1 → M2 → M3 → M4 → M5 → M6. Each implementation milestone receives independent Code Review before its dependent milestone. Between M5 and M6, a final independent Code Review assesses the complete integrated implementation and its current subjects against the approved design and delivery plan. This may share the final milestone review invocation if whole-change scope is explicit; it is not satisfied by an M5-only review or integration tests. Delivery Review approves this exact plan against the exact current Design Review; later model or material plan changes require the appropriate rereview. Milestone reviews assess increments; final Code Review assesses the integrated implementation; Verify evaluates the complete current evidence. Final Verify does not substitute for either review scope or missing milestone proof.
...
See the owning change record for current workflow state. Remaining completion gates are independent Delivery Review, implementation and Code Review for M1–M5, final whole-change Code Review with current subjects and no unresolved material findings, required correction reassessment, qualitative TG-08 acceptance or owning-Design disposition before adoption recommendation, and successful final Verify. This plan does not claim those outcomes.
