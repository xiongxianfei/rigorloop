...
- PR #68 hosted CI initially failed because the CI workflow did not set up Node and the runner's default `/usr/local/bin/node` rejected `--use-env-proxy`. The fix aligns CI with the release workflow by pinning Node 24 before running repository CI.
...
- 2026-05-18: PR #68 hosted CI failed before the fix in `rigorloop_cli.test`; `CR-M4-R1-F1 node_env_proxy_status reports enabled with --use-env-proxy` exited 9 because `/usr/local/bin/node` reported `bad option: --use-env-proxy`.
- 2026-05-18: `npm test --prefix packages/rigorloop` passed after pinning CI to Node 24.
- 2026-05-18: `bash scripts/ci.sh --mode explicit --path .github/workflows/ci.yml --path packages/rigorloop/test/cli.test.js --path packages/rigorloop/dist/bin/rigorloop.js --path docs/plans/2026-05-18-multi-adapter-init-and-proxy-aware-download.md --path docs/changes/2026-05-18-multi-adapter-init-and-proxy-aware-download/change.yaml` passed selected checks: `artifact_lifecycle.validate`, `change_metadata.regression`, `change_metadata.validate`, `selector.regression`, `rigorloop_cli.test`, and `npm_package_publication.test`.
