...
- Validation commands:
  ...
  - `ADAPTER_OUTPUT_DIR="$(mktemp -d)" && python scripts/build-adapters.py --version 0.0.0-verify-simplification --output-dir "$ADAPTER_OUTPUT_DIR" && python scripts/validate-adapters.py --version 0.0.0-verify-simplification --adapter-root "$ADAPTER_OUTPUT_DIR" --clean-install-smoke --skill verify`
  - `python scripts/validate-skills.py skills/verify/SKILL.md`
  - `python scripts/test-skill-validator.py`
