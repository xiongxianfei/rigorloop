...
Recovery terminology follows the approved [CLI removal dependency contract](../design/cli/cli.md#removal-dependency-contract): the existing journal `version` is a stored-format selector, not an independent bookkeeping schema. Supported v2 recovery uses selector 2 and matching recorded snapshots; advanced results remain schema 1. This plan introduces no new journal shape or version and never treats transport schema 1 as a reason to reject a supported v2 transaction.
...
- Implementation steps: Establish v2 and rejection proof first; remove v1 branches/fixtures only after identifying transport-schema-1 cases that must remain. Update current discovery consumers and selectors with the same cutoff. Validate the recorded recovery basis: the journal format selector, saved before/candidate manifests, declared writes and read set must agree under the retained recovery contract. Recover supported v2 transactions, including initial creation with no current manifest; do not classify from the current manifest alone or from an independent transport schema number. Retired, unknown, inconsistent or ambiguous bases stop before restore/complete writes and preserve available recovery evidence.
...
- Risks: Deleting shared schema1 transport validation; hiding corrupt v2 as history; accidentally rejecting supported v2 recovery because its advanced result uses schema 1, or accepting a retired/inconsistent recorded format basis.
