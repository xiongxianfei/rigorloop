# Boundary-First v1 v0.3.7 Activation Release
...
the `v0.3.7` activation. Non-conflicting requirements in those contracts remain
in force.

## Goal and context

Activate the complete `boundary-first-v1` package in stable release `v0.3.7`
...
`v0.3.7` identifies the earlier pending-to-active transition commit. Candidate
...
- `reviewed base`: the exact remote `main` commit that is the transition commit's
  first parent.
...
- `atomic publication`: one remote transaction that fast-forwards `main` to the
  reviewed head and creates `v0.3.7` at the transition commit, or changes neither.

## Examples first

Example E1: candidate validation succeeds before the tag exists
Given one active transition from reviewed base `B` to transition commit `T`
And reviewed head `H` contains `T` in its first-parent history
And `v0.3.7` does not exist locally or remotely
When candidate validation runs for `v0.3.7` at `H`
Then it validates `B`, `T`, `H`, rollback `v0.3.6`, the complete resource bundle,
and tag absence
And it reports candidate-ready without reporting an active published release.

Example E2: default validation remains strict
Given the same active tree but no `v0.3.7` tag
...
And local immutable tag `v0.3.7` points to `T`
When strict boundary-first validation and the full release gate run from the
tagged tree
Then both require `v0.3.7` to resolve to `T` and `v0.3.6` to be its immediate
...
Given candidate evidence was recorded against reviewed base `B`
...
Then it fails and requires a new coherent candidate transition and rereview.
...
BFA-R001. The activating release MUST be stable release `v0.3.7`, npm package
version `0.3.7`, and npm dist-tag `latest`.
...
`python scripts/validate-boundary-first.py --check --activation-candidate v0.3.7`.
...
activating release to be `v0.3.7`, and the rollback release to be `v0.3.6`.

BFA-R007. Candidate mode MUST fail if `v0.3.7` already exists locally or is
observable on the configured remote. An unreachable remote tag check MUST be an
explicit blocking result, not a pass.

BFA-R008. Candidate mode MUST discover exactly one first-parent pending-to-active
transition `T` and MUST record its full commit identity.

BFA-R009. Candidate mode MUST require the activation manifest's grandfathering
baseline to equal transition `T`'s full first-parent identity `B`.
...
does not logically require the absent `v0.3.7` tag, including exact resource,
manifest, projection, governed-skill, grandfathering, rollback-package, and
canonical-path identities.

BFA-R012. Candidate success MUST report `candidate_release`, `reviewed_base`,
`transition_commit`, `reviewed_head`, `rollback_release`, and `tag_state` in a
...
tag `v0.3.7` at `T` and MUST rerun ordinary strict boundary-first validation
from repository head `H` and full release verification from tagged tree `T`.

BFA-R019. Strict validation at `H` MUST require `v0.3.7` to resolve to `T`.
...
`main` to `H` by fast-forward and creates `v0.3.7` at `T`.

BFA-R021. The atomic update MUST use compare-and-swap evidence that remote
`main` still equals reviewed base `B` immediately before the update.
...
BFA-R023. Sequential ref-update fallback MUST NOT be used. A failed candidate
must be rebased or regenerated, revalidated, and rereviewed before another
publication attempt.
...
gate for `v0.3.7` before GitHub release or npm publication.
...
`v0.3.7`. Package recovery MUST use standing failed-release evidence,
...
MUST NOT construct a mixed resource bundle or overwrite `v0.3.7`.
...
release, reviewed base, transition commit, reviewed head when available,
...
- exact candidate release `v0.3.7`;
- activation record and its Git first-parent history;
- current `HEAD`, reviewed base, and local/remote tag namespaces;
...
- Public capability state is `pending` until remote `v0.3.7` exists and strict
  tag-context release validation succeeds.
- Candidate-ready is not published, active-public, tag-ready, or release-ready.
- `B` is the first parent of `T`; `T` is in the first-parent history of `H`.
- Remote publication maps `main -> H` and `v0.3.7 -> T` atomically.
...
- Base drift or ref conflict blocks both remote updates.
...
The new candidate flag is additive and valid only for the named `v0.3.7`
...
Grandfathered specifications are computed from reviewed base `B` under the
existing boundary-first rules. New specifications introduced after `B` are not
grandfathered.

## Observability

Candidate success emits one concise JSON result with mode, release, base,
transition, head, rollback, tag state, and validated bundle identity.
...
Boundary model scope: BFA-R001 through BFA-R034
...
| identity-authority | applicable | BFA-R008, BFA-R010, BFA-R018, BFA-R020, BFA-R021 | BND-AUTH-001 | - |
| composition-path | applicable | BFA-R005, BFA-R018, BFA-R025, BFA-R026, BFA-R033 | BND-COMPOSE-001 | - |
| temporal-retry | applicable | BFA-R015, BFA-R021, BFA-R022, BFA-R023 | BND-TEMPORAL-001 | - |
| failure-recovery | applicable | BFA-R022, BFA-R027, BFA-R028, BFA-R030 | BND-RECOVERY-001 | - |
...
| --- | --- | --- | --- | --- | --- | --- |
| BND-INPUT-001 | input-domain | BFA-R004, BFA-R006, BFA-R007, BFA-R031 | absent flag; exact v0.3.7 flag; malformed or other release; local tag absent/present; remote reachable/unreachable | Candidate mode is explicit, named, and tag-absent only. | Exact input validates; unknown, conflicting, or unreachable input stops with bounded diagnostics. | BFA-R004 |
...
| BND-AUTH-001 | identity-authority | BFA-R008, BFA-R010, BFA-R018, BFA-R020, BFA-R021 | reviewed base B; transition T; reviewed head H; rollback tag; activating tag; remote main | B is T's first parent, T is in H's first-parent history, main maps to H, tag maps to T. | Exact identities pass; drift, ambiguity, mismatch, or stale authority stops. | BFA-R020 |
| BND-COMPOSE-001 | composition-path | BFA-R005, BFA-R018, BFA-R025, BFA-R026, BFA-R033 | candidate CLI; default validator; strict local tag gate; tag workflow; package/archive checks; public closeout | Candidate mode changes only absent-tag authority; every sibling gate retains ownership. | Complete composed path publishes; bypass or omitted gate blocks. | BFA-R033 |
| BND-TEMPORAL-001 | temporal-retry | BFA-R015, BFA-R021, BFA-R022, BFA-R023 | transition before evidence; repeated candidate check; base drift; local tag retry; atomic push retry | Release-gated payload is fixed at T; retry never overwrites refs or reuses stale evidence. | Idempotent read checks pass; drift or failed atomic update requires regeneration/rereview. | BFA-R023 |
| BND-RECOVERY-001 | failure-recovery | BFA-R022, BFA-R027, BFA-R028, BFA-R030 | pre-push failure; atomic-ref rejection; failed-during-publish; failed-after-publish; delayed public evidence | Pre-push leaves remote unchanged; post-push never rewrites immutable artifacts. | Abandon local candidate, retry after rereview, or use standing fix-forward/closeout recovery. | BFA-R028 |
| BND-COMPAT-001 | compatibility-migration | BFA-R002, BFA-R003, BFA-R005, BFA-R029 | pending/default strict; active v0.3.7; v0.3.6 rollback; historical fixtures | boundary-first-v1 and default strict behavior remain stable; rollback is one immutable release. | Compatible paths remain valid; mixed or older rollback fails. | BFA-R005 |
...
| INT-002 | BFA-R008, BFA-R010, BFA-R015, BFA-R020 | BND-AUTH-001, BND-TEMPORAL-001 | Later evidence commits conflate H with T or mutate release payload. | Candidate records both identities and rejects post-transition release-gated drift. |
...
| --- | --- | --- | --- | --- | --- |
| E1 | illustration | BFA-R004, BFA-R006, BFA-R008, BFA-R010, BFA-R012 | BND-INPUT-001, BND-AUTH-001 | - | - |
...
| E7 | regression | BFA-R015, BFA-R016 | BND-TEMPORAL-001, BND-COMPOSE-001 | REG-BFA-005 | - |
...
EC2. Candidate release is `v0.3.8`, a prerelease, malformed, or not equal to the
manifest release; validation fails closed.

EC3. `v0.3.7` exists locally but not remotely, or remotely but not locally;
...
`T`; the candidate is invalid and must be regenerated and rereviewed.
...
- No generalized missing-tag candidate mode for releases after `v0.3.7`.
...
| ID | Criterion |
| --- | --- |
| AC-BFA-001 | Exact candidate command validates a complete active v0.3.7 tree without requiring the absent tag and emits non-public candidate status. |
| AC-BFA-002 | Default validation still fails an active tree whose activating tag is absent. |
| AC-BFA-003 | Candidate validation records exact B, T, and H identities and requires one first-parent chain. |
...
| AC-BFA-007 | Local v0.3.7 at T makes strict validation pass at H and the full release gate pass at T before remote mutation. |
| AC-BFA-008 | Base drift, tag conflict, non-fast-forward, or missing atomic capability changes neither remote ref. |
| AC-BFA-009 | Authorized publication atomically maps main to H and v0.3.7 to T. |
...
| AC-BFA-014 | Every BFA-R001 through BFA-R034 requirement has direct automated or explicitly release-owned proof in the matching test specification. |
