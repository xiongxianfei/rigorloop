...
| C12 | `python3 scripts/validate-governed-lifecycle-cli.py` | planned-for-implementation | implement | M7 | M7 code review | nonzero blocks enforcement | not applicable | M7 CI enforcement report | enumerates governed changes and invokes the public non-interactive read-only lifecycle validator with exact approved baseline warnings |
