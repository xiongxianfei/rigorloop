...
| CMD4 | `python scripts/select-validation.py --mode explicit --path scripts/validate-boundary-first.py --path scripts/boundary_first_validation.py --path scripts/test-boundary-first-validation.py --path scripts/fixtures/boundary-first/activation` | existing/configured | implement | M1 | code-review M1 | Block unclassified paths, registration debt, or missing owned checks. | Not applicable; deterministic selector. | `evidence/implementation-m1.md` | Read-only selection; executes no checks. |
