...
| CMD-08 | `bash scripts/release-verify.sh v0.4.1` | release-owned | release validation | M3 | M3 | Any release gate failure blocks cutover. | Not applicable; the named release target must execute its configured checks. | `evidence/m3-publication-parity.md` | Existing release verification; no publish operation. |
...
- Steps: build, drift-check, package, validate archives, select both changed skills, and run release verification without publishing.
- Expected result: supported projections contain the canonical resources and contract; missing, stale, escaped, or mixed resources block; tracked generated bodies are not introduced.
