...
| R11 | A `RIGORLOOP_LOG_DIR` override MUST resolve to an absolute directory and becomes the containment root for the five owned log names and rotation lock. At initialization and before each owned pathname mutation, the root, every existing component from the nearest existing ancestor, and the affected owned path MUST be checked without following a final symlink; observed symlinks, non-files, identity changes, or containment failures MUST degrade observability before that mutation. Every pathname constructed by the CLI MUST remain beneath the resolved root, and events MUST NOT record its absolute path. These checks are not an atomic authorization boundary against another same-user or privileged process that can replace the validated root or owned pathname between check and mutation; that actor is outside the first-release threat model, and the product MUST NOT describe this boundary as race-proof containment. |
...
| R14 | Append and rotation MUST preserve complete JSON Lines records under supported concurrent RigorLoop writers. Lock acquisition MUST use at most 10 attempts and 1,000 milliseconds total per event. Exhaustion, an unverifiable lock, or a stale lock MUST degrade observability rather than block longer, remove the lock, or corrupt an event; retaining that lock MAY cause later attempts to degrade until explicit user removal occurs. |
...
The active file is `rigorloop.jsonl`; archives are `rigorloop.1.jsonl` through `rigorloop.4.jsonl`. Sequence is `1` only for `invocation-start` and `2` only for `invocation-complete`; crossed or unknown event/sequence combinations are invalid.
...
- Rotation owns only the five named files and its bounded lock inside the validated directory. Its checks provide deterministic integrity for supported RigorLoop writers, not an atomic authorization boundary against another process that can replace pathnames concurrently.
...
| BND-ENV-001 | external-environment | R8, R10, R11, R12, R13, R14, R15, R16, R21, R22, R27, R31, R34 | Linux/macOS/Windows; safe/unsafe override; direct/wrapper; writable/unwritable filesystem; supported RigorLoop writer/untrusted concurrent pathname replacer | Logs stay outside the repository, under user control, with bounded resources and no network. Path checks fail closed for observed unsafe state; they are not an atomic authorization boundary against an actor with same-user or privileged pathname-replacement authority. | Supported environments and cooperating writers record safely; observed unsafe state degrades only diagnostics; hostile concurrent replacement is outside the first-release integrity guarantee. | R10 |
