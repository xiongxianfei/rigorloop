...
For explicitly selected new `explicit-recording-v1` changes, the [Workflow model](../docs/design/workflow.md) owns responsibility, correction, review applicability and success-only completion; the [CLI model](../docs/design/cli.md) owns recording and recovery. The matching distribution exposes `record-store` for explicit recording; only an explicitly requested new root uses this profile. Historical changes retain the procedures below without conversion. Its context is the recorded `record-store inspect` snapshot, not historical eligibility projections.
