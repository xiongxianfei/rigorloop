...
For explicitly selected new `explicit-recording-v1` changes, the [Workflow model](../docs/design/workflow/workflow.md) owns responsibility, correction, review applicability and success-only completion; the [CLI model](../docs/design/cli/cli.md) owns recording and recovery. The matching distribution exposes `record-store` for explicit recording; only an explicitly requested new root uses this profile. Historical changes retain the procedures below without conversion. Its context is the recorded `record-store inspect` snapshot, not historical eligibility projections.
...
- `on-demand artifact`: an artifact or action created only when the work depends on option expansion, decision-relevant uncertainty reduction, or another explicit trigger.
...
| On-demand artifacts | Explore (`explore`) and Research (`research`). | Created as standalone Git-tracked artifacts only when explicitly invoked for durable option expansion or decision-relevant uncertainty reduction. | Revised when their assumptions, evidence, or findings are materially outdated. | Absence is not a blocker unless the current work depends on unresolved options or uncertain facts. | Proposal, Design, Delivery, Implementation, Verify, or another named owning stage when its decision depends on the artifact. |
...
| --- | --- | --- | --- | --- | --- |
| `explore` | Expand the decision space with proportional, materially different options. | `on-demand` | Explicit invocation for unclear problem framing, user value, scope, materially different directions, solution-biased framing, or a difficult-to-reverse decision. | `false` | `true` |
| `research` | Reduce bounded decision-relevant uncertainty with sourced findings and confidence. | `on-demand` | Explicit invocation when repository, platform, dependency, compatibility, migration, current-rule, scale, performance, security, or operational facts materially affect a decision. | `false` | `true` |
...
R7bg. Explore and Research MUST remain separate optional support skills. Explore MUST answer what materially different directions are available; Research MUST answer what decision-relevant facts are sufficiently reliable and what remains uncertain. Neither support skill is a lifecycle stage or decision authority.

R7bh. An explicit invocation of Explore MUST produce a standalone Git-tracked artifact at `docs/explorations/YYYY-MM-DD-slug.md`, and an explicit invocation of Research MUST produce one at `docs/research/YYYY-MM-DD-slug.md`. A stage-local incidental fact check or option consideration MUST NOT require either artifact and MUST NOT claim discovery-skill completion.

R7bi. Route MUST select Explore when the material option space is unclear, Research when a material decision depends on an uncertain fact, both when Explore yields bounded research questions that could materially change the option comparison, and neither when direction and decision-relevant facts are sufficiently clear.

R7bj. A discovery artifact MUST name its owning stage or decision when known and recommend a handoff. Proposal, Design, Delivery, Implementation, Verify, or another named owner MAY receive that handoff, but the owning stage MUST explicitly adopt any conclusion that changes its governed decision. Explore and Research MUST NOT edit that owner's artifact, approve its direction, settle its package, or advance lifecycle state.

R7bk. When discovery evidence contradicts an approved decision, the support skill MUST stop and route the contradiction to the stage that owns the affected decision. Repeated, stale, unavailable, unsafe, or out-of-scope support work MUST stop or qualify its result rather than silently broadening scope or overstating confidence.
