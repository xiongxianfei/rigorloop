...
The explicit-recording profiles implement the [Workflow](../docs/design/workflow/workflow.md), [Record Format](../docs/design/record-format/record-format.md) and [CLI](../docs/design/cli/cli.md) models in an adopting project. Normal skills use targeted reads/writes, explicit decision-basis identities and actor-owned applicability, never complete-file reconstruction or historical eligibility gates. Primary creation selects rigorloop-records-v2; existing explicit-recording-v1 roots retain compatibility without fabricated origin. The profile replaces historical package/transition/recording/output procedures and their conditional resources, while retaining stage duties, permissions, independence and proof. Skill structure, self-containment and resource integrity remain in force. The matching CLI, schemas, templates and supported adapters must agree before separately authorized adoption; distribution availability grants no adoption or review authority.
