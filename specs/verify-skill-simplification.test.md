...
| CMD7 | `ADAPTER_OUTPUT_DIR="$(mktemp -d)" && python scripts/build-adapters.py --version 0.0.0-verify-simplification --output-dir "$ADAPTER_OUTPUT_DIR" && python scripts/validate-adapters.py --version 0.0.0-verify-simplification --adapter-root "$ADAPTER_OUTPUT_DIR" --clean-install-smoke --skill verify` | existing/configured | implement | M3 | M3 code-review | Stop on the first failed step; block any generated, archive, or temporary installed target missing byte-identical resources. | Not applicable; direct package selection must produce all supported targets. | `evidence/m3-package-proof.md` | Local temporary output/install trees only; no publication, network, or agent execution. |
