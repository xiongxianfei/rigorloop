...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CMD1 | `python -c 'import json; from pathlib import Path; root=Path("docs/changes/2026-08-13-test-spec-skill-simplification"); load=lambda path: json.loads(path.read_text(encoding="utf-8")); rules=load(root/"test-spec-rule-disposition.yaml")["rules"]; literals=load(root/"test-spec-literal-compatibility.yaml")["literals"]; scenarios=load(root/"fixtures/scenario-contracts.yaml")["scenarios"]; bad_rule=load(root/"fixtures/invalid-rule-disposition.yaml"); bad_literal=load(root/"fixtures/invalid-literal-classification.yaml"); rd={"retained-inline","retained-governed-reference","retained-boundary-reference","asset-owned","removed-duplicate","removed-obsolete-with-approved-contract-change"}; lc={"normative-contract","parser-or-package-contract","test-only-incidental","obsolete","historical-fixture"}; rf={"rule_id","source_locations","behavior","governing_requirements","applicable_profiles","disposition","destination","preservation_proof"}; lf={"literal_id","literal","source_location","consumers","classification","required_semantics","disposition","replacement"}; sf={"scenario","required","forbidden"}; vr=lambda row: ["unknown-disposition"] if row.get("disposition") not in rd else (["missing-required-fields"] if not rf <= row.keys() else (["empty-required-fields"] if any(row.get(field) in (None,"",[]) for field in rf) else [])); vl=lambda row: ["unknown-classification"] if row.get("classification") not in lc else (["missing-required-fields"] if not lf <= row.keys() else (["empty-required-fields"] if any(row.get(field) in (None,"",[]) for field in lf) else [])); expected={"portable-create","portable-bounded-revision","governed-create","governed-revise","governed-candidate-valid","governed-candidate-invalid","creation-entry-only","creation-file-written","creation-evidence-written","creation-already-complete","creation-collision","creation-changed-basis","creation-entry-file-asymmetry","stale-restart-disposable-partial","stale-restart-preserved-partial","stale-restart-blocked-reliance","revision-changes-requested","revision-authorized-upstream-change","revision-reopened-pre-reliance-active","revision-active-implementation-reliance","revision-identical-retry","revision-changed-retry","peer-activation","workflow-settlement-read-only","full-asset-composition","bounded-asset-revision","emitted-placeholder","missing-resource","automated-proof","manual-proof","hybrid-proof","incomplete-manual-proof","unknown-vocabulary"}; assert rules and literals and scenarios; assert all(vr(row)==[] for row in rules); assert all(vl(row)==[] for row in literals); assert vr(bad_rule)[0]=="unknown-disposition"; assert vl(bad_literal)[0]=="unknown-classification"; assert len({row["rule_id"] for row in rules})==len(rules); assert len({row["literal_id"] for row in literals})==len(literals); assert all(sf <= row.keys() and all(row[field] not in (None,"",[]) for field in sf) for row in scenarios); assert len({row["scenario"] for row in scenarios})==len(scenarios); assert {row["scenario"] for row in scenarios}==expected; print(f"rules={len(rules)} literals={len(literals)} scenarios={len(scenarios)} unknown_values=rejected-first")'` | planned-for-implementation | implement | M1 | M1 code-review | Reject unknown dispositions and classifications before field or consistency checks; then reject missing or empty required fields, duplicate IDs, incomplete scenario outcomes, or exact scenario-inventory drift. | Not applicable; every assertion executes. | `evidence/m1-preservation-inventories.md` | Repository-local reads only; no network or target-agent runtime. |
...
M1 creates JSON-compatible YAML ledgers and static scenario records under the owning change root. CMD1 fixes the exact 33-scenario inventory and requires non-empty `required` and `forbidden` outcomes for every row. The inventory covers both profiles, candidate validation, creation interruptions, identical and stale retries, same-entry restart with partial-byte variants, legal and illegal revision states, settlement boundaries, full and bounded composition, missing resources, automated/manual/hybrid proof, forbidden writes, and unknown vocabulary. `invalid-rule-disposition.yaml` and `invalid-literal-classification.yaml` prove that unknown closed values are rejected before missing-field or consistency checks. Existing repository fixtures remain the owners of skill parsing, boundary validation, lifecycle metadata, build output, and adapter packaging.
