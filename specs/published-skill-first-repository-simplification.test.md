...
| CMD2 | `python scripts/validate-artifact-lifecycle.py --mode explicit-paths --path docs/changes/2026-08-10-published-skill-first-repository-simplification/retirement-ledger.json` | existing/configured | implement | M1 | code-review M1 | Block malformed or unregistered ledger lifecycle evidence. | Not applicable; deterministic validator. | `evidence/m1-retirement-ledger.md` | Read-only explicit-path validation. |
