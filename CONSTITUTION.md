...
Authoring skills MUST write only their own governed artifacts and matching authoring-state transitions. As the sole narrow exception, `plan` initializes missing `workflow_state.planned_work` exactly once when it registers a new primary plan; it MUST NOT replace or update existing planned work. Review peers MUST write their own review evidence and only the matching artifact settlement transition. `workflow` MUST write routing state and every later planned-work transition and MUST NOT manufacture review settlement. Downstream and support skills MUST treat upstream governed artifacts and state as read-only and route required corrections to the owning stage.
