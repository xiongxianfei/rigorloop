...
from lib.release.release_candidate import (
...
from lib.release.release_transaction import load_release_profile_file
...
class ReleaseCandidateTests(unittest.TestCase):
    ...
    def test_source_identity_covers_nested_code_resources_and_ignores_bytecode(self):
        from lib.release.release_candidate import script_identity
        scripts = self.root / 'scripts'
        files = ['command.py', 'lib/release/core.py', 'lib/validation/worker.mjs',
                 'resources/boundary-first/manifest.yaml', 'resources/adapter-templates/codex/AGENTS.md']
        for name in files:
            path = scripts / name
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text('original')
        original = script_identity(scripts)
        for name in files:
            with self.subTest(path=name):
                path = scripts / name
                path.write_text('changed')
                self.assertNotEqual(original, script_identity(scripts))
                path.write_text('original')
                self.assertEqual(original, script_identity(scripts))
        cache = scripts / 'lib/release/__pycache__/core.cpython-312.pyc'
        cache.parent.mkdir()
        cache.write_bytes(b'runtime bytecode')
        self.assertEqual(original, script_identity(scripts))

    def test_source_identity_rejects_symlinked_module_directory(self):
        from lib.release.release_candidate import script_identity
        scripts = self.root / 'scripts'
        (scripts / 'lib').mkdir(parents=True)
        outside = self.root / 'outside'
        outside.mkdir()
        (outside / 'helper.py').write_text('loadable outside input')
        (scripts / 'lib/validation').symlink_to(outside, target_is_directory=True)
        with self.assertRaisesRegex(CandidateError, 'symlink'):
            script_identity(scripts)

    def test_candidate_rejects_changed_nested_source_before_build(self):
        import shutil
        import subprocess
        from lib.release.release_candidate import prepare_candidate
        ...
        git('init', '-b', 'main')
        for i, name in enumerate(['lib/release/release_execution.py',
                                  'resources/adapter-templates/codex/AGENTS.md']):
            with self.subTest(path=name):
                ...
                git('-c', 'user.name=Fixture', '-c', 'user.email=fixture@example.invalid',
                    '-c', 'commit.gpgsign=false', 'commit', '-m', 'Changed nested input')
                with self.assertRaisesRegex(CandidateError, 'does not match reviewed source'):
                    prepare_candidate(source, git('rev-parse', 'HEAD'), 'refs/heads/main',
                                      None, self.root / ('candidate-' + str(i)))
    ...
    def test_prepared_ci_failure_cannot_become_source_mode_sentinel(self):
        ...
        for child, expected in [(0, 0), (1, 1), (3, 1)]:
            with self.subTest(child=child), patch('lib.release.release_candidate.run', side_effect=run), \
                    patch('lib.release.release_candidate.prepare_candidate', side_effect=prepare), \
                    patch('lib.release.release_candidate.ci_subject'), \
                    patch('lib.release.release_candidate.subprocess.run', return_value=SimpleNamespace(returncode=child)) as dispatch:
    ...
    def test_ci_range_resolves_main_defaults_and_repeated_head_before_preparation(self):
        from unittest.mock import patch
        from lib.release.release_candidate import ci_arguments
        with patch('lib.release.release_candidate.run', side_effect=lambda command, root: {
    ...
    def test_ci_subject_rejects_changed_runtime_and_source(self):
        import subprocess
        import tarfile
        from lib.release.release_candidate import ci_subject, script_identity
...
class ReleaseCandidateIntegrationTests(unittest.TestCase):
    ...
    def test_ci_candidate_preserves_published_tag_without_release_eligibility(self):
        ...
        import subprocess
        from unittest.mock import patch
        from lib.release.release_candidate import prepare_candidate, run
        repository = ROOT
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('lib.release.release_candidate.run', side_effect=stop_after_real_preflight):
    ...
    def test_actual_candidate_build_and_packed_metadata_chain(self):
        ...
        with tempfile.TemporaryDirectory(prefix='release-candidate-proof-') as temporary:
            ...
            # The real prepared-release path must execute all relocated suites.
            recorded = {check['command'] for check in receipt['checks']}
            self.assertTrue({
...
class CurrentSourceQualificationTests(unittest.TestCase):
    def setUp(self):
        import runpy
        self.module = runpy.run_path(str((ROOT / 'scripts' / 'validate-release.py')))
    ...
    def test_missing_or_mismatched_preparation_preserves_prior_receipt(self):
        from lib.release.release_transaction import _release_notes_generated_block, load_release_profile
        path = self.root / 'docs/releases/profiles/v0.5.1.yaml'
        path.parent.mkdir(parents=True)
        path.write_text(profile_text('v0.5.1'))
