...
- Under adopted test criteria, consume the approved model groups through the [selection method](references/test-quality.md#select-requirements-and-proof). Allocate each affected requirement, section-owned responsibility and parent interaction to concrete proof, input prerequisites, timing and evidence; preserve explicit gaps and useful unlisted regressions. Check that the observation can expose the named violation, including semantic judgment through independent assessment. Measured cost guides efficient realization without dropping required protection; this allocation does not duplicate the living test design or require JSON.
