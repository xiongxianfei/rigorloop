...
Author observable requirements in the assigned living model, including failure, identity and correction behavior. Keep Workflow meaning, Record Format representation and CLI mechanics distinct. Use change link for the exact model subject and matching reads. Record material decisions with decision record and explicit rationale; hand the model package to independent Design Review.
