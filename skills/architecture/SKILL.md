...
Contribute structure, boundaries, compatibility and decision rationale to the one affected model Design file. Reconcile with specification authorship in that same file; do not create mandatory specification/architecture/ADR siblings. Preserve stable requirements and decisions, declare affected review/evidence applicability stale with reasons, and leave independent approval to the reviewer.
